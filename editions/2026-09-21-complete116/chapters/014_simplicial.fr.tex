...
$$
\Delta \longrightarrow \textit{Ensembles},\quad
...
$$
\Delta^{opp} \longrightarrow \textit{Ensembles},\quad
...
$\Delta$, voir Catégories, exemple \ref{categories-example-category-over-X}.
Il existe un foncteur $p : \Delta/[n] \to \Delta$.
La catégorie fibre de $p$ au-dessus de $[k]$, voir Catégories,
...
sur $\Delta$. Voir aussi Catégories, exemple
...
\begin{enumerate}
\item on pose
$\Hom_\mathcal{C}(U, V)_n = \Mor_\mathcal{C}(U_n, V_n)$, et
...
cas, le foncteur
$$
\text{Simp}(\mathcal{C})^{opp} \longrightarrow \textit{Ensembles}, \quad
...
\begin{eqnarray*}
\mathcal{C}^{opp} & \longrightarrow & \textit{Ensembles} \\
X
& \longmapsto &
...
$\mathcal{U}$.
Nous utiliserons Catégories, lemme \ref{categories-lemma-initial}.
...
$\mathcal{U}_{\leq m}$ satisfait à la propriété (1) de Catégories,
...
dans Catégories, définition \ref{categories-definition-initial}, (2).
\end{proof}
...
par le lemme de Yoneda. Voir Catégories, section
...
$U(n') \circ \overline{\varphi} = U(n)$ ci-dessus, par Catégories,
...
vers $\textit{Ensembles}$ qui associe à un objet $T$ l'ensemble
$$
...
calculé dans $\mathcal{C}$. Cela résulte, par exemple, de Catégories,
...
sous $[n]$, voir Catégories, exemple
...
$U(n) \circ \underline{\varphi} = U(n')$ ci-dessus, par Catégories,
...
S(A_\bullet)(\psi \circ \varphi)$. Choisissons $\beta \in I_p$ et posons
$\alpha = \beta \circ \psi$ et $\gamma = \alpha \circ \varphi$
...
$h_{n, i} : U_n \to V_n$, pour $n \geq 0$, $i = 0, \ldots, n + 1$
...
$h_{n, \alpha} : U_n \to V_n$, pour $n \geq 0$, $\alpha \in \Delta[1]_n$
...
pour $i : \text{Mod}_R \to \textit{Ensembles}$ le foncteur d'oubli
et pour $F : \textit{Ensembles} \to \text{Mod}_R$ le foncteur qui associe
...
pour $i : \textit{Alg}_A \to \textit{Ensembles}$ le foncteur d'oubli
et pour $F : \textit{Ensembles} \to \textit{Alg}_A$ le foncteur qui associe
