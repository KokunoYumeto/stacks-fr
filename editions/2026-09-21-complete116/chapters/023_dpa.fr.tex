...
\ref{definition-divided-powers} lorsque $A$ est une $\mathbf{Q}$-algèbre.
...
définition \ref{definition-divided-powers} sont satisfaites.
...
\begin{enumerate}
\item[(a)] les propriétés (1), (3) et (4) de la définition \ref{definition-divided-powers} soient
...
conditions énoncées dans la définition \ref{definition-divided-powers}. Il
...
de $IJ$. D'après la propriété (4) de la définition \ref{definition-divided-powers},
...
la définition \ref{definition-divided-powers} pour la famille
...
les éléments de $I$ ou de $J$, voir le lemme \ref{lemma-check-on-generators}.
...
le lemme \ref{lemma-a-version-of-brown}
...
d'après le lemme \ref{lemma-colimits}). Alors
\begin{enumerate}
...
la remarque \ref{remark-pushout}. En général, l'application
...
Plus précisément, en appliquant le lemme \ref{lemma-need-only-gamma-p}, on
...
\begin{definition}
\label{definition-extends} Étant
...
définition \ref{definition-divided-powers} impliquent donc que
$$
...
la définition \ref{definition-divided-powers}. Les
...
Le lemme \ref{lemma-check-on-generators} montre alors que $\bar\gamma$
...
pour $y \in I'$ d'après la définition \ref{definition-divided-powers}, (4),
...
$n(e - 1) \geq e$.
Cela prouve que $\gamma$ se prolonge à $A/p^eA$; voir le lemme \ref{lemma-kernel}.
L'assertion relative aux limites découle immédiatement de la construction des limites donnée dans
la démonstration du lemme \ref{lemma-limits}.
\end{proof}
...
$$
Par conséquent, d'après le lemme \ref{lemma-two-ideals}, il suffit de montrer qu'il existe
...
lemme \ref{lemma-gamma-extends}, puisque $A \to A\langle x \rangle$ est
plat. Pour la seconde, si $A$ est sans torsion, le
lemme \ref{lemma-silly} (4) montre que $\delta$ existe. En effet, en
...
Soit $(A, I, \gamma)$ un anneau à puissances
divisées. Il existe une variante du lemme \ref{lemma-divided-power-polynomial-algebra}
...
la définition \ref{definition-divided-powers}. Les axiomes (1) et (3) sont
...
\begin{remark}
\label{remark-ryo-suzuki} Dans la
situation $A \supset I$ du lemme \ref{lemma-need-only-gamma-p},
...
les sections \ref{section-divided-powers},
...
\begin{remark}
\label{remark-adjoining-set-of-variables} On
...
dans les exemples \ref{example-adjoining-odd}
et \ref{example-adjoining-even}. Plus
précisément, suivant la remarque \ref{remark-divided-power-polynomial-algebra},
pour $(A,\gamma)$
...
la définition \ref{definition-divided-powers-dga}.
...
la définition \ref{definition-divided-powers-dga}. Soit $f : A \to B$
...
(au sens de la définition \ref{definition-extends}).
...
par le lemme \ref{lemma-gamma-extends}.
...
en $T$, avec $\deg(T) = d$,
construite dans l'exemple \ref{example-adjoining-odd} ou
...
\noindent
Dans le lemme \ref{lemma-compute-cohomology-adjoin-variable},
...
dans l'exemple \ref{example-adjoining-odd} ou \ref{example-adjoining-even}.
...
En appliquant l'exemple \ref{example-adjoining-odd} $m$ fois, on obtient
$$
...
d'après le lemme \ref{lemma-extend-differential}.
...
l'exemple \ref{example-adjoining-odd} ou
\ref{example-adjoining-even} $t$ fois, on obtient
$$
...
\begin{proof}
La démonstration est exactement la même que celle du lemme \ref{lemma-tate-resolution}. Le
...
lemme \ref{lemma-divided-power-polynomial-algebra}, il
...
D'après le lemme \ref{lemma-base-change-div}, les algèbres différentielles graduées $B$ et
...
puissances divisées cherchée sur $H(B)$ par le lemme \ref{lemma-dpdga-good}.
...
lemme \ref{lemma-uniqueness-tate-resolution}
...
\begin{lemma}
\label{lemma-compute-theta} On
conserve les hypothèses et les notations du lemme \ref{lemma-get-derivation}.
...
\medskip\noindent
La construction de $\theta$ dans la démonstration du lemme \ref{lemma-get-derivation}
...
dans le lemme \ref{lemma-tate-resolution}.
Soient $\xi\in H_2(A/IA)$ et $\theta : A/IA \to A/IA$ l'élément et la
dérivation obtenus dans les lemmes \ref{lemma-get-derivation} et
...
homomorphisme local plat. La proposition \ref{proposition-avramov}
...
le lemme \ref{lemma-quotient-regular-ring-by-regular-sequence}.
\end{proof}
...
intersection complète au sens de la définition \ref{definition-lci}, et
...
intersection complète locale au sens de la définition \ref{definition-lci}.
\end{enumerate}
\end{lemma}
...
\begin{remark}
\label{remark-no-good-ci-map} Il
...
dans le premier paragraphe de la démonstration du lemme \ref{lemma-ci-map-well-defined}.
\end{proof}
...
d'après le
lemme \ref{lemma-well-defined-if-you-can-find-good-factorization}
et
...
\begin{proof}
Il s'agit d'une reformulation de la proposition \ref{proposition-avramov-map} au
moyen du lemme \ref{lemma-finite-type-lci-map}.
Nous omettons les détails.
\end{proof}
...
\begin{proof}
Cela résulte du lemme \ref{lemma-local-perfect-diagonal} et des
...
lemme \ref{lemma-local-perfect-diagonal} pour démontrer le
présent lemme.
\end{proof}
...
proposition \ref{proposition-regular-ideal} (et
...
sens de la définition \ref{definition-divided-powers-dga}) ; voir
l'exemple \ref{example-adjoining-odd}. Pour $m > n$, l'application de transition
...
du lemme \ref{lemma-tate-resolution}. Nous aurons donc
$$
...
du lemme \ref{lemma-tate-resolution}
...
et $A_i \to K_{n_{i - 1}}$ comme dans le lemme \ref{lemma-lift-tate-to-koszul}.
...
$A\langle T \rangle$ comme dans le lemme \ref{lemma-extend-differential}.
\begin{enumerate}
...
appliquer le lemme \ref{lemma-compute-cohomology-adjoin-variable},
