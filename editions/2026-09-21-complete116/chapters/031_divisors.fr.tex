...
être plus pratique d'utiliser les points faiblement associés, voir la section
 \ref{section-weakly-associated}. Relions la notion schématique à la notion algébrique sur les
...
\begin{proof}
Ceci résulte du Lemme  \ref{algebra-lemma-criterion-no-embedded-primes} d'Algèbre, combiné au Lemme  \ref{lemma-associated-affine-open}
ci-dessus.
\end{proof}
...
\begin{proof}
Nous affirmons que le Lemme  \ref{lemma-check-isomorphism-via-depth-and-ass}  s'applique à l'application affichée dans le
...
\text{WeakAss}_S(f_*\mathcal{F}) \subset f(\text{WeakAss}_X(\mathcal{F}))
$$
d'après le Lemme  \ref{lemma-weakly-ass-reverse-functorial}. Les ensembles extrêmes sont égaux d'après le Lemme
...
Pour tout morphisme  $g : T \to S$ , nous voyons d'après les lemmes  \ref{lemma-base-change-fitting-ideal}  et
 \ref{lemma-fitting-ideal-generate-locally}  que  $\mathcal{F}_T$  est localement engendré par un nombre  $\leq r$  de sections si
...
\noindent
Malgré le Lemme  \ref{lemma-locally-free-rank-r-pullback}, le lemme suivant n'est plus vrai si
...
\begin{proof}
Utilisons le Lemme  \ref{lemma-check-rank-on-affines}, Morphismes, Lemme  \ref{morphisms-lemma-dominant-between-integral}, et Compléments
...
\noindent
Il découle du Lemme  \ref{lemma-dual-reflexive}  que l'enveloppe réflexive est un
...
Soit  $X$  un schéma intègre localement noethérien. Grâce au Lemme
 \ref{lemma-dual-reflexive} , nous savons que l'enveloppe réflexive  $\mathcal{F}^{**}$  d’un
...
\begin{proof}
En utilisant le Lemme  \ref{lemma-check-reflexive-on-affines} , l'équivalence de (1) et (2) découle de Compléments
...
non-diviseur de zéro  $f \in A$  (voir le Lemme  \ref{lemma-characterize-effective-Cartier-divisor}). Rappelons que  $\dim(U)$  est
...
\begin{proof}
Choisissons  $a_i$  comme dans le Lemme  \ref{lemma-codim-1-part}  et définissez
...
d'après le Lemme  \ref{lemma-finite-trivialize-invertible-upstairs}. Choisissons des sections génératrices  $s_j$ 
...
sur  $\det_L(f : K \to K)$  comme dans la démonstration du Lemme  \ref{lemma-finite-locally-free-has-norm}. Observons que
...
(Lemme  \ref{lemma-difference-effective-Cartier-divisors}) est un diviseur de Cartier effectif relatif sur  $X/S$.
\end{lemma}
...
relative, voir le lemme  \ref{lemma-regular-quasi-regular-immersion}. Voir le lemme  \ref{lemma-flat-relative-H1-regular}  (ou
...
fermé et nulle part dense  $T \subset X$  selon le Lemme  \ref{lemma-regular-meromorphic-ideal-denominators}. Alors, il existe des
...
irréductibles de  $X \setminus U$. Le Lemme  \ref{lemma-components-locally-finite}  donne donc le résultat
souhaité.
\end{proof}
...
Définition  \ref{definition-order-vanishing}. Cette somme est bien définie grâce au Lemme  \ref{lemma-divisor-locally-finite}.
\end{definition}
...
Cartier effectif d'après le Lemme  \ref{lemma-weil-divisor-is-cartier-UFD}. Dans ce cas, la section canonique
...
(\ref{equation-c1}) est surjective, il existe un faisceau inversible  $\mathcal{L}$, une
...
d'après le Lemme  \ref{lemma-pullback-meromorphic-sections-defined}. Soit  $\mathcal{N} \subset \mathcal{K}_Y$  le  $\mathcal{O}_Y$-module dont les
...
\medskip\noindent
Dans le cas noethérien, nous pouvons utiliser le Lemme  \ref{lemma-extend-invertible-module}  pour nous ramener à
...
Remarque  \ref{remark-tensor}  avant de procéder à la démonstration. Choisissons un
...
de  $X \setminus U$ ait codimension $\geq 2$ dans $X$  et que
 $\mathcal{F}|_U$  soit inversible, voir le Lemme  \ref{lemma-reflexive-over-normal}. Remarquons que
...
un  $U$  qui convient pour les deux et l'application (\ref{equation-c1})
...
Puisque  $\mathcal{L} = \mathcal{F}|_U$  pour un certain module cohérent réflexif de rang  $1$  sur
 $\mathcal{O}_X$, que nous notons  $\mathcal{F}$, le Lemme  \ref{lemma-reflexive-S2-extend}  montre que
...
Sous les hypothèses et avec les notations du Lemme  \ref{lemma-structure-sheaf-Xs}, si  $s$  est non
...
$$
d'après le Lemme  \ref{lemma-Xs-codim-complement}. De plus, si  $M^{[n]}$  désigne l'enveloppe
réflexive de  $M \otimes_A \ldots \otimes_A M$  ($n$ facteurs), alors
$$
\Gamma(U, \mathcal{O}_U) = \colim M^{[n]}
$$
d'après le Lemme  \ref{lemma-structure-sheaf-Xs}.
\end{remark}

\begin{lemma}
\label{lemma-affine-Xs}
Sous les hypothèses et avec les notations du Lemme  \ref{lemma-structure-sheaf-Xs}, les conditions suivantes
sont équivalentes
\begin{enumerate}
...
\begin{proof}
D'après le Lemme \ref{lemma-closed-subscheme-proj}, il existe un faisceau quasi-cohérent
...
\begin{proof}
On voit que  $b : X' \to X$  est un objet de  $\mathcal{C}$  d'après le Lemme  \ref{lemma-blowing-up-gives-effective-Cartier-divisor}.
...
d'après le Lemme  \ref{lemma-complement-effective-Cartier-divisor}. Ainsi, le morphisme  $Y \to X'$  est unique d'après
...
s'agit d'un homomorphisme de  $A$-algèbres. Cela donne un morphisme
 $\Spec(\mathcal{O}_{X', x"}) \to \Spec(A') = U'$. Par la propriété universelle de l'éclatement (Lemme  \ref{lemma-universal-property-blowing-up}), il
...
\begin{proof}
Immédiat d'après la propriété universelle des éclatements (Lemme  \ref{lemma-universal-property-blowing-up}).
\end{proof}
...
\begin{proof}
Il suffit de combiner le Lemme  \ref{lemma-blowing-up-affine}  avec Algèbre, Lemme  \ref{algebra-lemma-blowup-reduced}.
\end{proof}
...
\begin{proof}
Il suffit de combiner le Lemme  \ref{lemma-blowing-up-affine}  avec Algèbre, Lemme  \ref{algebra-lemma-blowup-domain}.
\end{proof}
...
méromorphes sont définies pour  $b$  (Définitions  \ref{definition-pullback-effective-Cartier-divisor}  et
...
définit un diviseur de Cartier effectif, voir le Lemme  \ref{lemma-sum-closed-subschemes-effective-Cartier}. Il existe donc un
...
quasi-compact de  $X$. D'après le Lemme  \ref{lemma-closed-subscheme-proj-finite-type} , il existe un
...
même que l'éclatement de  $X'$  relativement à  $b^{-1}\mathcal{J} \mathcal{O}_{X'}$, voir le Lemme
 \ref{lemma-blowing-up-two-ideals}. Il suffit donc de montrer que l'éclatement de  $X'$  relativement à
...
pouvons utiliser les Lemmes  \ref{lemma-blow-up-effective-Cartier-divisor}  et  \ref{lemma-blowing-up-two-ideals}.
...
factorise en passant par  $b_i$  d'après le Lemme  \ref{lemma-blowing-up-two-ideals}.
\end{proof}
...
\medskip\noindent
Nous utiliserons la description de l'éclatement donnée dans le Lemme  \ref{lemma-blowing-up-affine}.
...
D'après la définition de l'idéal des dénominateurs dans la Définition
 ...
 $\mathcal{O}_{X'}(D - E)$ , voir la Définition  \ref{definition-invertible-sheaf-effective-Cartier-divisor}. L'image réciproque
 $b^*s$  est définie par les lemmes  \ref{lemma-blow-up-pullback-effective-Cartier}  et  \ref{lemma-meromorphic-sections-pullback}.
...
quasi-cohérent de type fini. Soit  $Z_k \subset S$  le sous-schéma fermé découpé par
 $\text{Fit}_k(\mathcal{F})$, voir section  \ref{section-fitting-ideals}. Soit  $S' \to S$  l'éclatement de
...
si et seulement si  $\text{Fit}_k(\mathcal{F}') = \mathcal{O}_{S'}$, voir le Lemme  \ref{lemma-fitting-ideal-generate-locally}. Ainsi ce
