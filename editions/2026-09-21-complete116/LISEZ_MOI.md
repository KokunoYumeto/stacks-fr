...
2. `02_stacks_project_french_complete_116.tex` : texte LaTeX cumulatif intégral,
   directement téléchargeable, correspondant exactement au lecteur.
3. `03_stacks_project_french_complete_source.zip` : sources modifiables des
   116 chapitres, source cumulative, classe, bibliographie, licence, liste des
   contributeurs, manifestes, outils d’assemblage et instructions de compilation.
4. `04_stacks_project_french_provenance_evidence.zip` : registres des choix de
   traduction, sources consultées, dossiers de relecture, contrôles des sources,
...
reconstruction. Les dépendances externes sont précisées dans ses instructions.
