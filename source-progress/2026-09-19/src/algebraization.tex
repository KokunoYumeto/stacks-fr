...
\begin{proof}
Appliquer Complexes dualisants, lemme \ref{dualizing-lemma-completion-local},
...
\begin{proof}
Appliquer le lemme \ref{lemma-pushforward-commutes-with-derived-completion}
au morphisme de topos annelés $(\mathcal{C}, \mathcal{O}) \to (pt, A)$
et prendre la cohomologie pour obtenir la première assertion. La seconde suite spectrale
...
remonte à \cite[Theorem 6.3.1]{lurie-thesis}.
...
\begin{proof}
Appliquer le théorème \ref{theorem-algebraization-formal-sections} avec $s = 1$
...
avec $x \not \in V(f)$ point associé de $\mathcal{F}$.
...
\cite[Exposee XIII, Theorem 2.1]{SGA2}. Il existe dans la littérature une version
appelée théorème de connexité de Faltings ;
nous supposons qu'il s'agit de \cite[Theorem 6]{Faltings-some}.
Énonçons et démontrons la version optimale pour les anneaux locaux
complets donnée dans \cite[Theorem 1.6]{Varbaro}.
...
de \cite[Theorem 2.10 (i)]{Baranovsky}. Il est immédiat de déduire
...
\Pic(U) \to \Pic(U_0)
$$
est injective. C'est \cite[Exposee XI, Lemma 3.16]{SGA2}\footnote{La condition
...
\begin{proof}
Combiner Diviseurs, lemme \ref{divisors-lemma-extend-invertible-module},
...
\item $(\mathcal{F}_n)$ se prolonge canoniquement sur $X$.
\end{enumerate}
...
et
\item[(b)] $(\mathcal{F}_n)$ soit la complétion du module cohérent
associé à $N$.
\end{enumerate}
\end{lemma}
...
modules $\mathbf{G}_m$-équivariants qui sont quasi cohérents sur $\mathcal{O}_X$, voir
...
est la complétion du module cohérent
...
\dim(\overline{\{q\}}) > 2
$$
Alors $(\mathcal{F}_n)$ est la complétion d'un
...
\item Si $X$ est une variété tridimensionnelle de Cohen--Macaulay, alors
$(\mathcal{E}_n)$ est la complétion d'un
...
est la complétion d'un $\mathcal{O}_X$-module cohérent $\mathcal{F}$.
