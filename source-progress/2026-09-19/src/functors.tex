...
aux conoyaux de couples de flèches, voir
Catégories, lemme \ref{categories-lemma-characterize-right-exact}.
Or, si $a, b : K \to L$ sont des applications de $A$-modules, le
conoyau du couple $(a,b)$ est le conoyau de $a - b : K \to L$.
...
et puisque les conoyaux sont un cas particulier de conoyaux de couples de flèches,
...
sont des sommes directes, qui existent, et le noyau du couple $a, b : L \to M$
...
aux noyaux de couples de flèches, voir
Catégories, lemme \ref{categories-lemma-characterize-left-exact}.
Or, si $a, b : L \to M$ sont des applications de $A$-modules, le
noyau du couple $(a,b)$ est le noyau de $a - b : L \to M$.
...
sont des sommes directes, qui existent, et le conoyau du couple $a, b : K \to L$
...
muni de l'action donnée $\kappa$ de $A$. Pour un
...
$$
F(f) = \text{l'application }F(M) \to F(N)\text{ induite par }
...
\medskip\noindent
Soit $f : M \to N$ un morphisme de $\text{Mod}^{fp}_A$. Montrons que l'application
...
$$
soit également commutatif. Notons $F(f)' : F(M) \to F(N)$ l'application
induite par $F(\psi)$ et $F(\chi)$. À la vue des diagrammes
commutatifs, un argument élémentaire d'algèbre commutative fournit une application
...
que l'application $F(A^{\oplus n_M}) \to F(M)$ égalise $F(f)$ et $F(f)'$.
...
Ils induisent donc l'application voulue, ce qui exprime précisément
...
commute aux conoyaux de couples de flèches, voir
Catégories, lemme \ref{categories-lemma-characterize-right-exact}.
...
dans $\mathcal{B}$. En tout cas, on obtient une application
...
conoyau de $F(\varphi_M)$, on trouve une application
$F(M) \to \Coker(F(K) \to F(L))$ inverse à droite
de l'application $\Coker(F(K) \to F(L)) \to F(M)$. Il en résulte d'abord
...
\item $F$ commute à toutes les colimites ;
...
à droite), comme requis pour obtenir l'application
...
M \to 0
$$
En utilisant les applications construites ci-dessus, nous obtenons un diagramme commutatif
$$
\xymatrix{
...
à toutes les colimites et admet un adjoint à droite (lemme \ref{lemma-functor}).
\end{example}
...
de $t$ en $A$. Cette application est $A \otimes_R B$-linéaire d'après la
...
\begin{proof}
Puisqu'une équivalence commute à toutes les colimites, nous voyons que le
...
K \to B^{\oplus n} \to K
$$
en tant qu'application de $B$-modules. Il s'ensuit que $K$ est un
...
qui commutent à toutes les colimites.
\end{remark}
...
qui commute aux colimites filtrantes.
\end{lemma}
...
du fait que les produits tensoriels de modules commutent aux colimites
filtrantes.
\end{remark}
...
qui commute aux colimites filtrantes. Si $F$ est additif, alors
$F'$ est additif et commute aux sommes directes quelconques.
...
$a \mapsto F(a \cdot \text{id})$. Ainsi, $\mathcal{G}$ est un faisceau de modules sur\par\endgroup
$$
...
pour tout morphisme $f : X' \to X$ dont la source est un schéma affine.
\end{lemma}
...
Nous obtenons donc l'application souhaitée
...
est une suite exacte telle que, pour tout $x \in X$, la suite sur les germes
...
qu'il existe un $\mathcal{M}$ dans
...
présentation finie.\par\endgroup
\end{lemma}
...
Si $\mathcal{F}$ est de présentation finie, alors il est compact au sens
...
et un morphisme $\mathcal{F} \to \mathcal{F}_i$ qui est inverse à droite de
l'application donnée $\mathcal{F}_i \to \mathcal{F}$.
...
alors il existe une application non nulle $\mathcal{F} \to \mathcal{E}$.
\end{lemma}
...
\medskip\noindent
Supposons maintenant qu'il existe une application non nulle
...
existe une application non nulle $\mathcal{G} \to \mathcal{E}$ avec
...
application non nulle $\mathcal{F}|_U \to \mathcal{E}|_U$.
Elle correspond à une application
$$
...
D'après (2), toute application $\mathcal{F}_x \to \kappa(x)$ se relève en
une application $\mathcal{F}_x \to \mathcal{O}_{X, x}$ (puisqu'elle se
relève même en une application globale $\mathcal{F} \to \mathcal{O}_X$).
...
que celui calculé dans $\textit{Mod}(\mathcal{O}_X)$. Nous concluons donc que\par\endgroup
$$
H^1(X, \mathcal{H}) =
...
\begin{proof}[\small{}Démonstration par reconstruction de Gabriel--Rosenberg]
\begingroup\emergencystretch=2em
Ce lemme est une forme faible des résultats exposés dans le
lemme \ref{lemma-equivalence-coherent} et la
remarque \ref{remark-equivalence-coherent-linear}.\par\endgroup
\end{proof}

\begin{proof}[\small{}Démonstration indépendante de la reconstruction de Gabriel--Rosenberg]
\begingroup\emergencystretch=2em
...
$f = \text{pr}_1 \circ s$. Nous omettons certains détails.\par\endgroup
