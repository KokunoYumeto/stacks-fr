...
de présentation finie sur la base $S$ comme foncteurs préservant les limites.
Nous poursuivons par l'étude des limites de systèmes projectifs filtrants sur des
...
La motivation de la définition suivante vient de
...
Soit $S$ un schéma.
\begin{enumerate}
\item Un foncteur $F : (\Sch/S)_{fppf}^{opp} \to \textit{Sets}$
est dit {\it compatible aux limites} ou {\it localement de présentation finie} si
pour tout schéma affine $T$ au-dessus de $S$ qui est une limite $T = \lim T_i$
d'un système projectif filtrant de schémas affines $T_i$ au-dessus de $S$, on a
$$
F(T) = \colim F(T_i).
$$
Nous disons parfois que $F$ est {\it localement de présentation finie sur $S$}.
\item Soient $F, G : (\Sch/S)_{fppf}^{opp} \to \textit{Sets}$.
Une transformation de foncteurs $a : F \to G$
est {\it compatible aux limites} ou {\it localement de présentation finie}
si, pour tout schéma $T$ au-dessus de $S$ et tout $y \in G(T)$, le foncteur
$$
...
s'applique (nous emploierons autant que possible la terminologie « préservant les limites »
...
\begin{enumerate}
\item $a : F \to G$ préserve les limites ;
\item pour tout schéma affine $T$ au-dessus de $S$ qui est une
limite $T = \lim T_i$ d'un système projectif filtrant de
schémas affines $T_i$ au-dessus de $S$, le diagramme d'ensembles
$$
\xymatrix{
...
est un système projectif filtrant de schémas affines au-dessus de $T_i$. Ainsi (1) implique
que $x_T$ est l'image d'un unique élément $x$ de
$\colim_{i' \geq i} F_{y_i}(T_{i'})$. Ainsi $x$ est l'unique
élément de $\colim F(T_i)$ qui s'envoie sur le couple $(y, x_T)$.
Ceci prouve que (2) est vérifiée.

\medskip\noindent
Supposons (2). Soit $T$ un schéma et $y_T \in G(T)$. Nous devons montrer que
$F_{y_T}$ préserve les limites. Soit $T' = \lim_{i \in I} T'_i$ un
schéma affine au-dessus de $T$, qui est la limite filtrante des schémas affines $T'_i$
au-dessus de $T$. Soit $x_{T'} \in F_{y_T}$. Choisissons $i \in I$, ce qui est possible car
...
Par (2), nous obtenons donc un unique élément $x$ de $\colim_i F(T'_i)$
s'envoyant sur $(y_i, x_{T'})$. Il est clair que $x$ définit un élément
de $\colim_i F_y(T'_i)$ qui s'envoie sur $x_{T'}$, ce qui conclut.
\end{proof}
...
Si $a$ et $b$ préservent les limites, alors
$$
...
$$
Par hypothèse, les deux carrés sont des carrés de produits fibrés. Le
rectangle extérieur est donc lui aussi un diagramme de produit fibré, ce qui prouve le lemme.
\end{proof}
...
Soient $F, G, H : (\Sch/S)_{fppf}^{opp} \to \textit{Sets}$.
Soient $a : F \to G$, $b : G \to H$ des transformations de foncteurs.
Si $b \circ a$ et $b$ préservent les limites, alors $a$
préserve les limites.
\end{lemma}
...
sont des produits fibrés d'ensembles. Le carré supérieur
est donc lui aussi un carré de produit fibré, ce qui prouve le lemme.
\end{proof}
...
$$
Si $a$ préserve les limites, alors le changement de base $a'$ préserve les limites.
\end{lemma}
...
Soit $S$ un schéma contenu dans $\Sch_{fppf}$.
Soient $E, F, G, H : (\Sch/S)_{fppf}^{opp} \to \textit{Sets}$.
Soient $a : F \to G$, $b : H \to G$ et $c : G \to E$
des transformations de foncteurs. Si $c$, $c \circ a$ et $c \circ b$
préservent les limites, alors $F \times_G H \to E$ les préserve aussi.
\end{lemma}
...
$$
car les colimites filtrantes commutent aux produits finis. Notre but est donc de
montrer que
$$
\xymatrix{
...
$$
est un diagramme de produit fibré. Cela découle de l'observation que,
...
Si $F$ préserve les limites, alors sa faisceautisation $F^\#$ préserve les limites.
\end{lemma}

\begin{proof}
Supposons que $F$ préserve les limites.
Il suffit de montrer que $F^+$ préserve les limites, puisque
$F^\# = (F^+)^+$, voir
Sites, théorème \ref{sites-theorem-plus}.
Soit $T$ un schéma affine au-dessus de $S$, et écrivons $T = \lim T_i$
comme la limite d'un système projectif filtrant de schémas affines au-dessus de $S$.
Rappelons que $F^+(T)$ est la colimite de $\check H^0(\mathcal{V}, F)$
où la colimite porte sur tous les recouvrements de $T$ dans $(\Sch/S)_{fppf}$.
Tout recouvrement fppf d'un schéma affine peut être raffiné en un recouvrement
fppf standard, voir
...
$$
Tout $\mathcal{V} = \{T_k \to T\}_{k = 1, \ldots, n}$
dans la colimite peut s'écrire
...
le changement de base pour $i' \geq i$. Alors on voit que
\begin{align*}
...
\check H^0(\mathcal{V}, F)
\end{align*}
Ici la deuxième égalité tient au fait que les colimites filtrantes sont exactes.
La troisième égalité tient au fait que $F$ préserve les limites et que
$\lim_{i' \geq i} T_{i', k} = T_k$ et
$\lim_{i' \geq i} T_{i', k} \times_{T_{i'}} T_{i', l} = T_k \times_T T_l$
par Limites, lemme \ref{limits-lemma-scheme-over-limit}.
Si nous faisons ceci pour tous les recouvrements simultanément, nous obtenons
\begin{align*}
F^+(T)
& =
...
\colim_{i \in I} F^+(T_i)
\end{align*}
L'échange de l'ordre des colimites est permis par
...
$F|_{(\Sch/U_j)_{fppf}}$ préserve les limites.
\end{enumerate}
Alors $F$ préserve les limites.
\end{lemma}

\begin{proof}
Soit $T$ un schéma affine au-dessus de $S$.
Soit $I$ un ensemble dirigé, et soit
$T_i$ un système inverse de schémas affines au-dessus de $S$ tel que
$T = \lim T_i$. Nous devons montrer que l'application canonique
...
le tiré-en-arrière $\{U_j \times_S T_0 \to T_0\}$ du recouvrement fppf donné de $S$.
Pour chaque $i \geq 0$, posons $V_{i, k} = T_i \times_{T_0} V_{0, k}$, et
posons $V_k = T \times_{T_0} V_{0, k}$. Remarquons que
$V_k = \lim_{i \geq 0} V_{i, k}$, voir
Limites, lemme \ref{limits-lemma-scheme-over-limit}.

\medskip\noindent
Supposons que $x, x' \in \colim F(T_i)$ s'envoient sur le même
élément de $F(T)$. Disons que $x, x'$ sont donnés par des éléments $x_i, x'_i \in F(T_i)$
pour un certain $i \in I$ (nous pouvons choisir le même $i$ pour les deux puisque $I$ est dirigé).
Par l'hypothèse (2) et le fait que $x_i, x'_i$ s'envoient sur le même élément
de $F(T)$, ceci implique que
$$
x_i|_{V_{i', k}} = x'_i|_{V_{i', k}}
$$
pour un certain $i' \in I$ suffisamment grand. Nous pouvons choisir le même $i'$ pour chaque
$k$, puisque $k \in \{1, \ldots, m\}$ parcourt un ensemble fini.
Puisque $\{V_{i', k} \to T_{i'}\}$
est un recouvrement fppf et que $F$ est un faisceau, ceci implique que
$x_i|_{T_{i'}} = x'_i|_{T_{i'}}$, comme souhaité. Cela montre que l'application
$\colim F(T_i) \to F(T)$ est injective.

\medskip\noindent
Pour montrer la surjectivité, nous raisonnons de manière similaire.
Soit $x \in F(T)$. Par l'hypothèse (2), pour chaque $k$, nous
pouvons choisir un $i$ tel que $x|_{V_k}$ provienne d'un
élément $x_{i, k} \in F(V_{i, k})$. Comme précédemment, nous pouvons choisir un
seul $i$ qui convienne pour tous les $k$. Par l'injectivité
démontrée ci-dessus, nous voyons que
$$
x_{i, k}|_{V_{i', k} \times_{T_{i'}} V_{i', l}}
=
x_{i, l}|_{V_{i', k} \times_{T_{i'}} V_{i', l}}
$$
pour un $i'$ suffisamment grand. Par conséquent, par la condition de faisceau de $F$,
...
Soit $S$ un schéma contenu dans $\Sch_{fppf}$.
Soient $F, G : (\Sch/S)_{fppf}^{opp} \to \textit{Sets}$ des foncteurs.
Si $a : F \to G$ est une transformation qui préserve les limites,
alors la transformation induite entre les faisceaux
$F^\# \to G^\#$ préserve les limites.
\end{lemma}

\begin{proof}
Supposons que $T$ soit un schéma et que $y \in G^\#(T)$.
Nous devons montrer que le foncteur
$F^\#_y : (\Sch/T)_{fppf}^{opp} \to \textit{Sets}$
construit à partir de $F^\# \to G^\#$ et de $y$ comme dans
la définition \ref{definition-locally-finite-presentation}
préserve les limites.
Par l'équation (\ref{equation-fibre-map-functors})
nous voyons que $F^\#_y$ est un faisceau. Choisissons un recouvrement fppf
$\{V_j \to T\}_{j \in J}$ tel que $y|_{V_j}$ provienne
d'un élément $y_j \in F(V_j)$.
Remarquons que la restriction de $F^\#$ à $(\Sch/V_j)_{fppf}$
est simplement $F^\#_{y_j}$. Si nous pouvons montrer que $F^\#_{y_j}$
préserve les limites, alors
le lemme \ref{lemma-sheaf-finite-presentation}
garantit que $F^\#_y$ préserve les limites et
le résultat est démontré. Cela nous ramène au cas $y \in G(T)$.

\medskip\noindent
Soit $y \in G(T)$. Dans ce cas, nous affirmons que $F^\#_y = (F_y)^\#$.
Cela découle de
l'équation (\ref{equation-fibre-map-functors}).
Ce cas découle donc du
...
\begin{enumerate}
\item Le morphisme $f$ est un morphisme d'espaces algébriques qui est
localement de présentation finie, voir
Morphismes d'espaces,
définition \ref{spaces-morphisms-definition-locally-finite-presentation}.
\item Le morphisme $f : X \to Y$ préserve les limites en tant que
...
$T \times_Y X$ préserve les limites au-dessus de $T$ au sens de
la définition \ref{definition-locally-finite-presentation}.
Nous sommes donc réduits à démontrer que si $X$ est un espace algébrique qui
est localement de présentation finie au-dessus de $S$ en tant qu'espace algébrique, alors il
préserve les limites en tant que foncteur
...
Morphismes d'espaces,
définition \ref{spaces-morphisms-definition-locally-finite-presentation}
que $U$ et $R$ sont des schémas qui sont localement de présentation finie
au-dessus de $S$. Donc, par
Limites, proposition
\ref{limits-proposition-characterize-locally-finite-presentation},
nous avons
$$
...
$$
préserve les limites. Donc, par le
lemme \ref{lemma-sheafify-finite-presentation},
sa faisceautisation $X = U/R$ préserve également les limites.
...
$V \times_Y X \to V$ préserve les limites. Par
Morphismes d'espaces,
...
de présentation finie, donc préserve les limites en tant que
...
la composition $U \to V \times_Y X \to V$ préserve les limites
...
Cela signifie, par définition, que (1) est vérifiée.
...
Limites, proposition
\ref{limits-proposition-characterize-locally-finite-presentation}
nous caractérisons $U \to V$ comme étant localement de présentation finie
si pour {\it tous} les systèmes inverses dirigés $(T_i, f_{ii'})$ de schémas affines
au-dessus de $V$, nous avons $U(T) = \colim V(T_i)$, mais dans le contexte actuel
nous ne pouvons considérer que des schémas affines $T_i$ au-dessus de $V$ qui sont (isomorphes à)
...
Il suffit donc d'observer que, dans la partie « seulement si » de la preuve du
...
Alors $X$ est localement de présentation finie au-dessus de $S$ si et seulement
si $X$, en tant que foncteur $(\Sch/S)^{opp} \to \textit{Sets}$,
préserve les limites. Comparer avec
...
$$
soit surjective chaque fois que $T = \lim T_i$ est une limite dirigée de
schémas affines au-dessus de $S$.
\end{remark}
...
Si, pour toute limite dirigée $T = \lim_{i \in I} T_i$
de schémas affines au-dessus de $S$, l'application
$$
...
$$
Par l'hypothèse du lemme, après avoir augmenté $i$, nous
pouvons trouver un morphisme $c_i : T_i \to X$ tel que
...
Par conséquent, après avoir de nouveau augmenté $i$, nous pouvons trouver le
morphisme $a_i : T_i \to U$ recherché tel que $a = a_i \circ p_i$ et
$b_i = (U \to V) \circ a_i$.
\end{proof}
...
Soit $S$ un schéma. Soit $I$ un ensemble dirigé.
Soit $(X_i, f_{ii'})$ un système inverse sur $I$
...
Choisissons un élément $0 \in I$ (c'est possible puisque tout ensemble dirigé est non vide).
...
définit une relation d'équivalence, car une limite dirigée de relations d'équivalence
est une relation d'équivalence. Par conséquent, le morphisme
...
est une application surjective de faisceaux. Enfin, supposons que
$Z$ soit un schéma et que $a, b : Z \to U/R$ soient deux morphismes
qui sont égalisés par (\ref{equation-isomorphism-sheaves}).
Nous devons montrer que $a = b$.
Après avoir remplacé $Z$ par les membres d'un recouvrement fppf,
nous pouvons supposer qu'il existe des morphismes $a', b' : Z \to U$ qui
donnent $a$ et $b$. La condition que $a, b$ soient égalisés par
(\ref{equation-isomorphism-sheaves}) signifie que
...
$R_i$, disons au moyen d'un morphisme $c_i : Z \to R_i$. Comme
...
Soit $S$ un schéma. Soit $I$ un ensemble dirigé.
Soit $(X_i, f_{ii'})$ un système inverse sur $I$ d'espaces algébriques
...
de systèmes inverses sur $I$ d'espaces algébriques sur $S$.
Supposons que
\begin{enumerate}
...
et parce qu'une limite d'immersions fermées de schémas affines est une
immersion fermée : une colimite filtrante d'homomorphismes d'anneaux surjectifs
...
Soit $S$ un schéma. Soit $I$ un ensemble dirigé.
Soit $(X_i, f_{i'i})$ un système inverse sur $I$
...
est un schéma affine réduit, comme limite de schémas affines réduits :
une colimite filtrante d'anneaux réduits est réduite.
...
\begin{enumerate}
\item[(3)] pour toute limite dirigée $T = \lim T_i$ d'espaces algébriques quasi-compacts
et quasi-séparés $T_i$ sur $S$ à morphismes de transition affines, le diagramme
d'ensembles
$$
\xymatrix{
...
La démonstration est assez formelle et nous encourageons le lecteur à trouver
...
Remarquons que $\Mor(T, X)$ est l'égalisateur des deux applications
$\Mor(U, X) \to \Mor(V, X)$ ; cela résulte par exemple du fait que
$X$, comme faisceau sur $(\Sch/S)_{fppf}$, est le coégalisateur
des deux applications $h_V \to h_u$. De même,
$\Mor(T_{i'}, X)$ est l'égalisateur des
deux applications $\Mor(U_{i'}, X) \to \Mor(V_{i'}, X)$.
Et bien sûr, la même chose vaut en remplaçant $X$ par $Y$.
La condition (2) dit que les diagrammes de (3) sont des produits fibrés
...
et un morphisme \'etale surjectif $U \to T_i$. En répétant le
l'argument du paragraphe précédent, nous obtenons encore la démonstration :
...
Soit $S$ un schéma. Soit $X = \lim_{i \in I} X_i$ la limite d'un système inverse dirigé
...
le lemme \ref{lemma-directed-inverse-system-has-limit}.
Ainsi, si nous pouvons démontrer le lemme pour les systèmes inverses où $X_0$
est quasi-compact, le lemme en résulte dans le cas général. Nous pouvons donc supposer, et
nous supposerons, que $X_0$ est quasi-compact.
...
un point $u \in |U|$ qui s'envoie sur un $x \in |X|$ s'envoyant sur l'élément donné
$(x_i)$ de $\lim |X_i|$.

\medskip\noindent
Injectivité de $|X| \to \lim |X_i|$. Supposons que $x, x' \in |X|$
s'envoient sur le même point de $\lim |X_i|$. Choisissons des relèvements $u, u' \in |U|$
et notons $u_i, u'_i \in |U_i|$ leurs images.
Pour chaque $i$, soit $T_i \subset |R_i|$ l'ensemble des points s'envoyant
sur $(u_i, u'_i) \in |U_i| \times |U_i|$. Il s'agit d'un ensemble fini
par définition des espaces décents
(Espaces décents, définition \ref{decent-spaces-definition-very-reasonable}).
De plus, $T_i$ est non vide puisque nous avons supposé que $x$ et $x'$ s'envoyaient sur
le même point de $X_i$. Donc $\lim T_i$ est non vide, voir
Catégories, lemme \ref{categories-lemma-nonempty-limit}.
Comme précédemment, soit $r \in |R| = \lim |R_i|$ un point correspondant à un
élément de $\lim T_i$. Alors $r$ s'envoie sur $(u, u')$ dans $|U| \times |U|$
par construction et nous voyons que $x = x'$ dans $|X|$, comme voulu.

\medskip\noindent
Remarque entre parenthèses : Un espace algébrique quasi-séparé est décent, voir
...
Les applications $f_i : X \to X_i$ sont des morphismes d'espaces algébriques
et déterminent donc des applications continues $|X| \to |X_i|$.
...
inverse dirigé d'espaces algébriques sur $S$ à morphismes de transition affines
(lemme \ref{lemma-directed-inverse-system-has-limit}). Si chaque $X_i$
...
et comme espaces algébriques lorsqu'ils sont munis de leur structure induite de schéma réduit.
\end{lemma}
