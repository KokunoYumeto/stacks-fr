...
introduit les recouvrements de Zariski, \'etale, fppf, lisse, syntomique et
fpqc de schémas. Dans le présent chapitre, nous examinons les structures sur
les schémas qui peuvent se descendre le long de tels recouvrements. Voir, par
...
réciproques des morphismes $\varphi_{\alpha(i)\alpha'(i)}$ par
...
$$
Notons-la $(A/R)_\bullet$, de sorte que $(A/R)_n$ soit le produit tensoriel de
...
$$
comme l'unique application $(A/R)_\bullet(\beta)$-semi-linéaire telle que
$$
...
Les relations entre composées des $\sigma$ sont évidentes.
Celles entre composées des $\delta$ se ramènent à la condition
de cocycle pour $\varphi$.
On vérifie exactement comme ci-dessus les relations
$\sigma_j \circ \delta_j = \sigma_j \circ \delta_{j + 1} = \text{id}$.
Enfin, les autres relations entre composées des $\delta$ et des $\sigma$
...
$(A/R)_\bullet \otimes_R M$. On obtient donc un complexe de cochaînes prolongé
\begin{equation}
...
Alors, pour tout $R$-module $M$, le complexe de cochaînes prolongé
...
une équivalence d'homotopie ; voir
...
$$
le résultat s'ensuit (le complexe prolongé s'obtient simplement en ajoutant
un terme $M$ au début).
\end{proof}
...
$R' \to A' = R' \otimes_R A$. Il en résulte alors le résultat pour
$R \to A$. En effet, pour tout $R$-module $M$, le module cosimplicial
...
$-1$ et $0$ du complexe prolongé (démonstration omise).
...
des modules, des algèbres et des morphismes qui descendent par
les homomorphismes d'anneaux universellement injectifs.
...
est appelé {\it extension des scalaires suivant $f$}.
...
suivant $f$. Commençons par quelques résultats de \cite{mesablishvili2}.
...
qui est stable par extension des scalaires et descend par les morphismes
fidèlement plats descendrait aussi par les morphismes universellement injectifs.
...
Par image réciproque, on obtient des morphismes
$$
a_k :
...
on remarque que $a_j$ et $a_{j'}$ correspondent tous deux à l'image réciproque
...
Par image réciproque, elle donne des données de descente $\xi_j$
...
$$
correspondant à l'isomorphisme entre les images réciproques
...
Si
\begin{enumerate}
\item $f$ est ouverte en tant qu'application d'espaces topologiques,
...
des modules quasi-cohérents sur nombre des sites associés à $S$
...
satisfait la condition de faisceau pour tout $\tau$-recouvrement
$\{T_i \to T\}_{i \in I}$ d'un schéma $T$ sur $S$.
\end{lemma}

\begin{proof}
Pour $\tau \in \{Zariski, \linebreak[0] \etale, \linebreak[0] smooth,
\linebreak[0] syntomic, \linebreak[0] fppf\}$, un $\tau$-recouvrement
...
On peut interpréter les $s_i$ comme une famille d'applications
...
$(\Sch/S)_{Zar}$, son image réciproque sur, par exemple, $(\Sch/S)_{fppf}$
...
Par transitivité de l'image réciproque (voir
...
est l'image réciproque par le morphisme de sites annelés
...
$$
Cela résulte de (1) et de la compatibilité de l'image réciproque avec
...
les topoï annelés correspondants coïncident).
\end{proof}
...
$\mathcal{O}$-modules quasi-cohérents sur le grand site $\tau$ de $S$.
...
$$
entre la catégorie des faisceaux quasi-cohérents sur $S$ et celle des
$\mathcal{O}$-modules quasi-cohérents sur le petit site $\tau$ de $S$.
\end{enumerate}
\end{proposition}
...
relativement à un recouvrement $\tau$ de $S$. Une fois cette donnée
...
site $\tau$ de $S$. D'après Modules sur les sites, définition
\ref{sites-modules-definition-site-local}, il existe un recouvrement
$\tau$, $\{S_i \to S\}_{i \in I}$, de $S$, tel que chacune des restrictions
...
réunions disjointes sont des recouvrements $\tau$.
\end{proof}
...
recouvrement $\tau$, l'homomorphisme $R \to A$ est fidèlement plat.
...
recouvrement $\tau$, $\mathcal{U} = \{U_i \to U\}$, de $U$ dans
...
recouvrements ouverts sont des recouvrements $\tau$. On obtient un
complexe
$$
...
dérivées, lemme \ref{derived-lemma-morphisms-lift}, il existe donc une
...
R\Gamma(S, \mathcal{F}).
$$
En cohomologie, on obtient l'application
...
peut aussi voir comme un recouvrement $\tau$. Ce qui précède fournit une
application de complexes doubles
$$
...
$$
Elle induit une application de suites spectrales
$$
...
applications induites ${}^\tau\! E_2^{p, q} \to E_2^{p, q}$ sont
bijectives (toutes les intersections sont séparées, car ce sont des
ouverts d'affines). Les applications
...
car $f_*$ n'envoie pas en général les faisceaux quasi-cohérents sur des
...
sur des faisceaux quasi-cohérents ; voir Schémas, Lemme
...
morphisme plat de sites annelés commute au Hom interne depuis un module
de présentation finie, par Modules sur les sites, Lemme
...
\begin{gathered}
\mathcal{F}\text{ est localement libre de rang fini} \\
\text{pour la topologie de Zariski}
\end{gathered}
\end{align*}
...
\ref{topologies-lemma-affine-big-site-fppf}, il existe une équivalence de
topos
...
topologie choisie pour étudier les modules quasi-cohérents sur $\Sch/S$
n'importe pas, et elle affirme aussi que (8) équivaut à (7). Les conditions
...
La condition (1) résulte donc de Schémas, Lemme
\ref{schemes-lemma-widetilde-pullback}.
...
\longrightarrow 0
$$
En présentant nos excuses pour cette notation peu agréable, cela achève la
preuve.
\end{proof}
...
$\tau$ standards. Voir Topologies, Définitions
\ref{topologies-definition-big-small-Zariski} et
\ref{topologies-definition-big-small-etale}. D'après Topologies, Lemmes
\ref{topologies-lemma-alternative-zariski}, respectivement\  
\ref{topologies-lemma-alternative}, il existe une équivalence de topos
...
chacun des recouvrements de $S_{affine, \etale}$ est un recouvrement
\'etale standard ; on peut donc appliquer le Lemme \ref{lemma-ff-exact}.
...
En présentant nos excuses pour cette notation peu agréable, nous avons ainsi
démontré que (1) implique (2).
...
$U \to S$ plat et localement de présentation finie. Appliquons Cohomologie
sur les sites, Lemme
\ref{sites-cohomology-lemma-cech-vanish-collection}, au sous-ensemble
...
\section{Les recouvrements fpqc sont des familles épimorphiques effectives universelles}
...
\right)
$$
\item $\{f_i : X_i \to X\}_{i \in I}$ est une famille épimorphique effective universelle
...
\begin{enumerate}
\item Si la famille est épimorphique effective universelle dans la catégorie
...
$f_i$ est épimorphique effective universelle dans la catégorie des schémas.
\end{lemma}
...
(Topology, définition \ref{topology-definition-submersive}).
Alors notre famille de morphismes est épimorphique effective universelle dans
...
Alors $\{T_i \to T\}$ est une famille épimorphique effective universelle dans la catégorie
...
\ref{lemma-universal-effective-epimorphism} pour voir que $\{X \to Y\}$ est une
famille épimorphique effective universelle.
...
(être lisse, être localement de présentation finie, etc.) descendent par les
morphismes fidèlement plats. Nous commençons par une version algébrique.
...
\medskip\noindent
Nous allons utiliser le critère d'Algebra, lemme
...
(en appliquant plusieurs fois Algebra, lemme
...
Il suffit donc de montrer que les fibres de $q$ sont lisses, voir Morphismes,
lemme \ref{morphisms-lemma-smooth-flat-smooth-fibres}.
Appliquons Varieties, lemme \ref{varieties-lemma-flat-under-smooth}, aux
...
Nous pouvons donc appliquer le résultat d'Avramov, voir Algèbre à
puissances divisées, Lemme \ref{dpa-lemma-avramov}, et conclure que
$\mathcal{O}_{Y, y}$ et
...
\medskip\noindent
Le cas $P = flat$. Dans ce cas, nous savons que $C \to A$ est plat et
...
Notons $H$ son homologie et $H'$ celle de la suite
...
\medskip\noindent
Le cas $P = locally\ finite\ type$. Dans ce cas, nous savons que
...
fini), l'homomorphisme $A \to B'$ est de type fini ; voir Algèbre,
Lemme \ref{algebra-lemma-compose-finite-type}. Par conséquent,
$C \to B'$ est de type fini, et nous concluons par le
Lemme \ref{lemma-finite-type-local-source-fppf-algebra}.

\medskip\noindent
Le cas $P = locally\ finite\ presentation$. Dans ce cas, nous savons
...
$A \to B'$ est de présentation finie ; voir Algèbre,
Lemme \ref{algebra-lemma-compose-finite-type}. Par conséquent,
...
anneaux locaux et en appliquant Algèbre, Lemme
\ref{algebra-lemma-descent-reduced}. On dit que la propriété d'être réduit
{\it descend par les morphismes plats surjectifs}. Des résultats de ce type
sont réunis dans Algèbre, Section
...
$\Spec(k[x]/(x^2)) \to \Spec(k)$. La propriété d'être réduit ne
{\it monte donc pas par les morphismes plats}. La propriété d'avoir des
corps résiduels infinis monte par les morphismes plats (mais ne descend
...
si elle monte par les morphismes plats et descend par les morphismes plats
surjectifs. Un exemple assez élémentaire est la propriété d'avoir des corps
...
Cela résulte de Topologies, Lemmes
\ref{topologies-lemma-zariski-etale},
\ref{topologies-lemma-zariski-etale-smooth},
\ref{topologies-lemma-zariski-etale-smooth-syntomic},
\ref{topologies-lemma-zariski-etale-smooth-syntomic-fppf} et
...
la topologie $\tau$ ; voir Topologies, Définitions
\ref{topologies-definition-fpqc-covering},
...
\ref{topologies-definition-syntomic-covering}, et Topologies, Lemmes
...
\ref{topologies-lemma-syntomic-affine}. Nous omettons les détails.
\end{proof}
...
localement noethérien ; c'est Morphismes, Lemme
\ref{morphisms-lemma-finite-type-noetherian}. Enfin, si $S' \to S$ est un
...
résulte d'Algèbre, Lemme \ref{algebra-lemma-descent-Noetherian}.
...
\medskip\noindent
On obtient une contradiction comme suit. Prenons d'abord un idéal maximal
$\mathfrak p \subset \mathfrak m$ de $A$. Choisissons un idéal premier
...
\begin{proof}
Nous allons vérifier les conditions (1), (2) et (3) du Lemme
\ref{lemma-descending-properties}. Comme un morphisme lisse est plat de
présentation finie (Morphismes, Lemmes \ref{morphisms-lemma-smooth-flat} et
\ref{morphisms-lemma-smooth-locally-finite-presentation}), la propriété
``être localement noethérien'' a déjà été traitée dans la preuve du
Lemme \ref{lemma-Noetherian-local-fppf}. Nous l'utiliserons sans autre
mention. Tout d'abord, $\mathcal{P}$ est locale pour la topologie de Zariski.
Cela résulte immédiatement de la définition ; voir Propriétés, Définition
\ref{properties-definition-Rk}. Ensuite, si $S' \to S$ est un morphisme
lisse entre schémas affines et si $S$ vérifie $\mathcal{P}$, alors $S'$
vérifie $\mathcal{P}$. C'est Algèbre, Lemmes
\ref{algebra-lemma-Rk-goes-up} (appliquer Morphismes, Lemme
\ref{morphisms-lemma-smooth-characterize}, ainsi qu'Algèbre, Lemmes
\ref{algebra-lemma-base-change-smooth} et
\ref{algebra-lemma-characterize-smooth-over-field}). Enfin, si $S' \to S$
...
de Nagata est locale pour la topologie de Zariski ; c'est Propriétés, Lemme
\ref{properties-lemma-locally-nagata}. Ensuite, si $S' \to S$ est un
morphisme lisse entre schémas affines et si $S$ est un schéma de Nagata,
alors $S'$ est un schéma de Nagata ; c'est Morphismes, Lemme
\ref{morphisms-lemma-finite-type-nagata}. Enfin, si $S' \to S$ est un
morphisme lisse surjectif entre schémas affines et si $S'$ est un schéma de
Nagata, alors $S$ est un schéma de Nagata ; c'est Algèbre, Lemme
\ref{algebra-lemma-descent-nagata}. Les conditions (1), (2) et (3) du
Lemme \ref{lemma-descending-properties} sont donc satisfaites, ce qui conclut.
\end{proof}
...
\begin{proof}
Le résultat s'obtient en considérant les anneaux locaux (Schémas, Définition
\ref{schemes-definition-reduced}) et en appliquant Algèbre, Lemme
\ref{algebra-lemma-descent-reduced}.
\end{proof}
...
que $A$ est noethérien d'après Algèbre, Lemme
\ref{algebra-lemma-descent-Noetherian}, et régulier d'après Algèbre, Lemme
\ref{algebra-lemma-flat-under-regular}.
\end{proof}
...
Topologie, Définition \ref{topology-definition-Krull}, comme le minimum des
...
$f$ sont discrètes (voir Morphismes, Lemme
\ref{morphisms-lemma-etale-over-field}). Ainsi, $\dim(V') \geq n$.
...
Morphismes, Lemme \ref{morphisms-lemma-fppf-open}. Ainsi,
...
$$
qui est plat et local d'après Morphismes, Lemme
\ref{morphisms-lemma-etale-flat}. Les points $\xi_i$ correspondent aux idéaux
premiers de l'anneau de gauche d'après Schémas, Lemme
\ref{schemes-lemma-specialize-points}. Le théorème de descente (voir Algèbre,
Section \ref{algebra-section-going-up}) appliqué à l'homomorphisme affiché
...
$\dim(A_{\mathfrak p}) = \dim(B_{\mathfrak q})$. C'est Plus sur l'algèbre,
Lemme \ref{more-algebra-lemma-dimension-etale-extension}.
\end{proof}
...
régulier si et seulement si $B_{\mathfrak q}$ est régulier. C'est Plus sur
l'algèbre, Lemme \ref{more-algebra-lemma-regular-etale-extension}.
\end{proof}
...
et surjectif, on dit que $\mathcal{P}$ descend par les changements de base
plats surjectifs. Si $\mathcal{P}$ est stable par changement de base plat et
descend par les changements de base plats surjectifs, on dit que
...
$\tau$, $\{Y_i \to Y\}_{i \in I}$ (voir Topologies, Section
\ref{topologies-section-procedure}), et pour tout morphisme de schémas
...
$\mathcal{P}$.
\end{enumerate}
Un tel morphisme $g$ étant ouvert (voir Morphismes, Lemme
\ref{morphisms-lemma-fppf-open}), $W \subset Y$ est un ouvert de $Y$.
...
Soit $\mathcal{P}$ une propriété des morphismes de schémas au-dessus d'une
base. Soit $\tau \in \{fpqc, fppf, \etale, smooth, syntomic\}$. Supposons que
\begin{enumerate}
...
$\tau$ vaut fpqc, fppf, \'etale, smooth ou syntomic (comparer avec Schémas,
Définition \ref{schemes-definition-preserved-by-base-change}),
...
\section{Propriétés des morphismes locales sur la cible pour la topologie fpqc}
...
nous montrerons dans Exemples, Section
\ref{examples-section-non-descending-property-projective}, que les propriétés
...
Schémas, Lemme \ref{schemes-lemma-quasi-compact-affine}. Enfin, soit
...
Schémas, Lemme \ref{schemes-lemma-separated-permanence}. La quasi-séparation
est locale sur la base pour la topologie de Zariski (cela résulte de la
définition ou de Schémas, Lemme
\ref{schemes-lemma-characterize-quasi-separated}). Enfin, soit $S' \to S$
...
topologie de Zariski (par définition ou d'après Morphismes, Lemme
\ref{morphisms-lemma-universally-closed-local-on-the-base}). Enfin, soit
$S' \to S$ un morphisme plat surjectif de schémas affines, et soit
$f : X \to S$ un morphisme. Supposons que le changement de base
...
image réciproque dans $S' \times_S T$ est fermée ; voir Morphismes, Lemme
\ref{morphisms-lemma-fpqc-quotient-topology}. Une chasse au diagramme
...
\begin{proof}
Tout changement de base d'un morphisme séparé est séparé ; voir Schémas,
Lemme \ref{schemes-lemma-separated-permanence}. La séparation est locale sur
la base pour la topologie de Zariski (par définition ou d'après Schémas,
Lemme \ref{schemes-lemma-characterize-separated}). Enfin, soit $S' \to S$ un
...
Lemme \ref{schemes-lemma-diagonal-immersion}), $\Delta$ est une immersion
...
\begin{proof}
D'après Morphismes, Lemme \ref{morphisms-lemma-base-change-dominant}, les
...
ouvert. Par conséquent, si $X \to Y$ est dominant, Morphismes, Lemme
\ref{morphisms-lemma-open-base-change-dominant}, montre que $X_E \to Y_E$
est dominant. Réciproquement, supposons que $X_E \to Y_E$ soit dominant.
Le morphisme $Y_E \to Y$ est surjectif d'après Morphismes, Lemme
\ref{morphisms-lemma-base-change-surjective} ; le composé
...
Schémas, Section \ref{schemes-section-points}, fournit une extension de corps
$K'/K$ et un morphisme $\Spec(K')
\to S'$ tels que le diagramme plein suivant soit commutatif :
$$
\xymatrix{
...
injectif, nous avons $a' = b'$ d'après Morphismes, Lemme
\ref{morphisms-lemma-universally-injective}. Cela impose évidemment $a = b$
(voir la discussion de Schémas, Section \ref{schemes-section-points}). Le
Lemme \ref{lemma-descending-properties-morphisms} s'applique et conclut.

\medskip\noindent
On peut aussi utiliser la caractérisation d'un morphisme universellement
injectif par la surjectivité de sa diagonale ; voir Morphismes, Lemme
\ref{morphisms-lemma-universally-injective}. Le résultat découle alors du fait
...
voir Morphismes, Lemme \ref{morphisms-lemma-base-change-finite-type}. Elle est
locale sur la base pour la topologie de Zariski ; voir Morphismes, Lemme
\ref{morphisms-lemma-locally-finite-type-characterize}. Enfin, soit
$S' \to S$ un morphisme plat surjectif de schémas affines, et soit
$f : X \to S$ un morphisme. Supposons que le changement de base
...
que $R \to A$ est de type fini. C'est Algèbre, Lemme
\ref{algebra-lemma-finite-type-descends}. Il s'ensuit que $f$ est localement
...
changement de base ; voir Morphismes, Lemme
\ref{morphisms-lemma-base-change-finite-presentation}. Elle est locale sur la
base pour la topologie de Zariski ; voir Morphismes, Lemme
\ref{morphisms-lemma-locally-finite-presentation-characterize}. Enfin, soit
$S' \to S$ un morphisme plat surjectif de schémas affines, et soit
$f : X \to S$ un morphisme. Supposons que le changement de base
...
montrer que $R \to A$ est de présentation finie. C'est Algèbre, Lemme
\ref{algebra-lemma-finite-presentation-descends}. Il s'ensuit que $f$ est
...
homéomorphisme. Le morphisme $f$ est donc affine par Morphismes,
Lemme \ref{morphisms-lemma-homeomorphism-affine}. Comme
$$
\mathcal{O}(S') \to
\mathcal{O}(X') =
...
voir Morphismes, Lemme \ref{morphisms-lemma-characterize-affine}.
Enfin, soit
...
voir Schémas, Lemme \ref{schemes-lemma-push-forward-quasi-coherent}.
...
(voir par exemple Cohomologie des schémas,
...
$$
au-dessus de $S$ donné par Schémas,
...
$$
voir Constructions, Lemme \ref{constructions-lemma-canonical-morphism}.
...
Supposons chaque $f_i$ une immersion quasi-compacte.
Par Schémas, Lemme \ref{schemes-lemma-immersions-monomorphisms},
chaque $f_i$ est séparé.
Les Lemmes \ref{lemma-descending-property-quasi-compact} et
\ref{lemma-descending-property-separated}
impliquent donc que $f$ est quasi-compact et séparé.
Soit $X \to Z \to Y$ la factorisation de $f$ par son
image schématique. Par
...
Par Morphismes, Lemme \ref{morphisms-lemma-quasi-compact-immersion},
...
(nous avons déjà vu qu'elle était quasi-compacte).
\end{proof}
...
sont locales pour la topologie fpqc sur la base.
\end{lemma}
...
que la propriété ``être localement de type fini'' est locale pour la topologie
fpqc sur la base; ainsi, $f$ est localement de type fini.
Il résulte alors de
...
pour la topologie fpqc sur la base
...
de dimension relative $d$'' est locale pour la topologie fpqc sur la base.
\end{lemma}

\begin{proof}
Cela résulte immédiatement du fait que la propriété d'être localement de type
fini est locale pour la topologie fpqc sur la base, et de
...
présentation finie sont locales pour la topologie fpqc sur la base (Lemmes
\ref{lemma-descending-property-flat} et
\ref{lemma-descending-property-locally-finite-presentation}).
Le résultat pour les morphismes syntomiques découle donc de
...
et la propriété d'être localement de présentation finie sont locales pour la
topologie fpqc sur la base (Lemmes
\ref{lemma-descending-property-flat} et
\ref{lemma-descending-property-locally-finite-presentation}).
Le résultat pour les morphismes lisses découle donc de
...
locales pour la topologie fpqc sur la base (Lemmes
\ref{lemma-descending-property-locally-finite-presentation},
...
Voir Morphismes, Lemme \ref{morphisms-lemma-flat-unramified-etale}.
Nous avons déjà vu que la platitude et la propriété d'être G-non ramifié
sont locales pour la topologie fpqc sur la base (Lemmes
\ref{lemma-descending-property-flat} et
...
par ex.\ Lemme \ref{lemma-fpqc-universal-effective-epimorphisms}),
...
(resp.\ $H_1$-régulière, quasi-régulière) de $A$. C'est
...
\section{Propriétés des morphismes locales pour la topologie fppf sur le but}
...
est locale pour la topologie fppf sur la base.
\end{lemma}
...
Par Topologie, Lemme \ref{topology-lemma-open-morphism-quotient-topology},
il s'ensuit que $f(X) \subset S$
est localement fermé. Soit $Z \subset S$
le fermé $Z = \overline{f(X)} \setminus f(X)$.
Une nouvelle application de Topologie,
Lemme \ref{topology-lemma-open-morphism-quotient-topology},
...
\begin{proof}
Par Morphismes,
Lemme \ref{morphisms-lemma-open-base-change-dominant},
...
\section{Application de la descente fpqc des propriétés des morphismes}
\label{section-application-descending-properties-morphisms}

\noindent
Le lemme suivant peut sembler quelque peu frivole, mais il constitue un outil
...
le chapitre des morphismes de type \'Etale, Section
...
Morphismes, Lemme \ref{morphisms-lemma-diagonal-unramified-morphism}
(et Morphismes, Lemme \ref{morphisms-lemma-flat-unramified-etale}).
Comme $f$ est universellement injectif, $\Delta_{X/Y}$ est aussi surjectif,
voir Morphismes, Lemme \ref{morphisms-lemma-universally-injective}.
...
Soit $f : X \to Y$ un morphisme \'etale universellement injectif.
Alors $f$ est ouvert (Morphismes, Lemme \ref{morphisms-lemma-etale-open});
nous pouvons donc remplacer $Y$ par $f(X)$ et supposer $f$ surjectif.
...
degré $d$, voir Morphismes, Lemme \ref{morphisms-lemma-finite-locally-free}.
...
\begin{proof}
Il faut montrer que $\Delta : X \to X \times_Y X$ est surjectif, voir
...
base, voir Morphismes, Lemme \ref{morphisms-lemma-base-change-flat}.
Les générisations se relèvent donc le long de $\text{pr}_1$, voir
...
\begin{proof}
Munissons $Z$ de la structure réduite induite de sous-schéma fermé, voir
...
Par définition (au moyen de
Cohomologie des schémas,
Lemme \ref{coherent-lemma-closed-proper-over-base}),
...
Par Morphismes, Définition \ref{morphisms-definition-relatively-ample},
...
soit surjectif sur les fibres en $x_i$. Cela résulte soit directement
...
soit surjectif sur les fibres en $x$.
Cela implique que l'ouvert $U(\psi) \subset X$ de
Constructions, Lemme
...
$r_{\mathcal{L}_i, \psi_i}$, qui est une immersion ouverte par
Morphismes, Lemme \ref{morphisms-lemma-characterize-relatively-ample}.
...
$\tau$-recouvrement $\{X_i \to X\}_{i \in I}$, on a
$$
f \text{ vérifie }\mathcal{P}
\Leftrightarrow
...
Soit $\mathcal{P}$ une propriété de morphismes qui est $\tau$-locale
sur la source. Pour tout morphisme de schémas $f : X \to Y$, il existe
...
\end{enumerate}
Comme tout tel morphisme $g$ est ouvert (voir
Morphismes, Lemme \ref{morphisms-lemma-fppf-open}),
...
$\mathcal{P}$ : nous disposons en effet d'un $\tau$-recouvrement
...
\item la propriété soit locale pour la topologie de Zariski sur la source,
\item la propriété soit locale pour la topologie de Zariski sur le but,
...
$f' : X' \to Y$.
\end{enumerate}
Alors $\mathcal{P}$ est $\tau$-locale sur la source.
\end{lemma}

\begin{proof}
Cela résulte presque immédiatement de la définition d'un
$\tau$-recouvrement, voir
...
\section{Propriétés des morphismes locales pour la topologie fpqc sur la source}
\label{section-fpqc-local-source}

\noindent
Voici quelques propriétés de morphismes qui sont locales pour la topologie
fpqc sur la source.

\begin{lemma}
\label{lemma-flat-fpqc-local-source}
La propriété $\mathcal{P}(f)=$``$f$ est plat'' est locale pour la topologie
fpqc sur la source.
\end{lemma}
...
(Voir aussi Morphismes, Lemme \ref{morphisms-lemma-flat-permanence},
...
est locale pour la topologie fpqc sur la source.
\end{lemma}
...
\section{Propriétés des morphismes locales pour la topologie fppf sur la source}
...
\begin{proof}
La propriété d'être localement de présentation finie est locale pour la
topologie de Zariski sur la source et sur le but, voir
Morphismes,
...
\begin{proof}
La propriété d'être localement de type fini est locale pour la topologie de
Zariski sur la source et sur le but, voir
...
\begin{proof}
La propriété d'être un morphisme ouvert est manifestement locale pour la
topologie de Zariski sur la source et sur le but.
Elle est préservée par composition, voir
...
et \ref{morphisms-lemma-composition-open}).
...
\section{Propriétés des morphismes locales pour la topologie syntomique sur la source}
\label{section-syntomic-local-source}

\noindent
Voici quelques propriétés de morphismes qui sont locales pour la topologie
syntomique sur la source.
...
\begin{proof}
Combiner le Lemme \ref{lemma-properties-morphisms-local-source} avec
Morphismes, Lemme \ref{morphisms-lemma-syntomic-characterize}
(localité pour la topologie de Zariski sur la source et sur le but),
...
\section{Propriétés des morphismes locales pour la topologie lisse sur la source}
\label{section-smooth-local-source}

\noindent
Voici quelques propriétés de morphismes qui sont locales pour la topologie
lisse sur la source. Notons aussi le résultat (à certains égards plus fort)
...
La propriété $\mathcal{P}(f)=$``$f$ est lisse''
est locale pour la topologie lisse sur la source.
\end{lemma}

\begin{proof}
Combiner le Lemme \ref{lemma-properties-morphisms-local-source} avec
Morphismes, Lemme \ref{morphisms-lemma-smooth-characterize}
(localité pour la topologie de Zariski sur la source et sur le but),
...
\section{Propriétés des morphismes locales pour la topologie \'etale sur la source}
...
La propriété $\mathcal{P}(f)=$``$f$ est \'etale''
est locale pour la topologie \'etale sur la source.
\end{lemma}
...
Lemme \ref{lemma-properties-morphisms-local-source}.
Par Morphismes,
Lemme \ref{morphisms-lemma-locally-quasi-finite-characterize},
la propriété d'être localement quasi-fini est locale pour la topologie de
Zariski sur la source et sur le but. Par
Morphismes, Lemmes
...
Nous allons utiliser le
Lemme \ref{lemma-properties-morphisms-local-source}.
Par Morphismes, Lemme \ref{morphisms-lemma-unramified-characterize},
la propriété d'être non ramifié (resp.\ G-non ramifié)
est locale pour la topologie de Zariski sur la source et sur le but. Par
Morphismes, Lemmes \ref{morphisms-lemma-composition-unramified} et
...
Ainsi, $X \to Y$ est non ramifié (resp.\ G-non ramifié), voir
...
\section{Propriétés des morphismes locales pour la topologie \'etale sur la source et le but}
\label{section-properties-etale-local-source-target}

\noindent
Soit $\mathcal{P}$ une propriété de morphismes de schémas. La phrase
``$\mathcal{P}$ est locale pour la topologie \'etale sur la source et le but''
a un sens intuitif. Il s'avère toutefois que cette notion ne revient pas à
demander que $\mathcal{P}$ soit à la fois locale pour la topologie \'etale
sur la source et locale pour la topologie \'etale sur le but.
...
Le lecteur vérifiera qu'elle est locale pour la topologie \'etale sur la
source et locale pour la topologie \'etale sur le but (démonstration omise,
voir Lemme \ref{lemma-Noetherian-local-fppf}).
...
Vérifions qu'elle est locale pour la topologie \'etale sur la source et
locale pour la topologie \'etale sur le but. Nous utiliserons librement
Schémas, Lemme \ref{schemes-lemma-specialize-points}.

\medskip\noindent
Localité sur le but : soit $\{g_i : Y_i \to Y\}$ un recouvrement \'etale.
...
Par conséquent, $\mathcal{O}_{Y_i, y_i}$ est noethérien par
...
\medskip\noindent
Localité sur la source : soit $\{h_i : X_i \to X\}$ un recouvrement \'etale.
...
\noindent
Quelle devrait être la bonne notion pour une propriété locale relativement à
la topologie \'etale sur la source et le but ? Par analogie avec
Morphismes, Définition \ref{morphisms-definition-property-local}, nous pensons
que ce devrait être la suivante.

\begin{definition}
\label{definition-local-source-target}
Soit $\mathcal{P}$ une propriété de morphismes de schémas.
Nous disons que $\mathcal{P}$ est {\it locale pour la topologie \'etale sur
la source et le but} si
\begin{enumerate}
\item (stabilité par précomposition avec des morphismes de type \'etale)
...
$$
dont les flèches verticales sont de type \'etale et un $u \in U$ tel que
...
En outre, une propriété locale relativement à la topologie \'etale sur la
source et le but est locale pour la topologie \'etale sur la source et locale
pour la topologie \'etale sur le but.
...
Soit $\mathcal{P}$ une propriété de morphismes de schémas qui est locale pour
la topologie \'etale sur la source et le but. Alors
\begin{enumerate}
\item $\mathcal{P}$ est locale pour la topologie \'etale sur la source,
\item $\mathcal{P}$ est locale pour la topologie \'etale sur le but,
\item $\mathcal{P}$ est stable par postcomposition avec des morphismes de type
...
Soit $\{X_i \to X\}_{i \in I}$ un recouvrement \'etale de $X$. Si chaque
...
Soit $\{Y_i \to Y\}_{i \in I}$ un recouvrement \'etale de $Y$.
...
Soit $\mathcal{P}$ une propriété de morphismes de schémas locale pour la
topologie \'etale sur la source et le but. Soit $f : X \to Y$ un morphisme de
...
$$
avec $a$, $b$ de type \'etale, le morphisme $h$ possède $\mathcal{P}$,
\item[(d)] pour un diagramme comme en (c) dans lequel $a : U \to X$ est
surjectif, le morphisme $h$ possède $\mathcal{P}$,
\item[(e)] il existe un recouvrement \'etale $\{Y_i \to Y\}_{i \in I}$ tel
...
\item[(g)] il existe un recouvrement \'etale $\{Y_i \to Y\}_{i \in I}$ et,
pour tout $i \in I$, un recouvrement \'etale
...
Lemme \ref{lemma-local-source-target-implies}.
Puisque $\{a : U \to X\}$ est un recouvrement \'etale, nous concluons que
...
\item $\mathcal{P}$ soit locale pour la topologie \'etale sur le but, et
...
\end{enumerate}
Alors $\mathcal{P}$ est locale pour la topologie \'etale sur la source et le
but.
\end{lemma}
...
stable par précomposition avec les morphismes \'etale. Le
...
$$
où $a$, $b$ sont de type \'etale et où $a$ est surjectif. Remarquons que $h$
possède $\mathcal{P}$, puisque chaque $h_x$ la possède et que $\mathcal{P}$ est
locale pour la topologie \'etale sur le but. Comme $a$ est surjectif et que
$\mathcal{P}$ est locale pour la topologie \'etale sur la source, il suffit de
...
$\mathcal{P}$, puisque $\mathcal{P}$ est locale pour la topologie \'etale sur
le but. En appliquant (AAA) au morphisme
...
$\{\coprod_{i = 1, \ldots, n} X_i \to X\}$ est un recouvrement \'etale ;
comme $\mathcal{P}$ est locale pour la topologie \'etale sur la source, nous
concluons que $X \to Z$ possède $\mathcal{P}$, comme voulu.
...
$X = \bigcup X_i$. Puisque $\mathcal{P}$ est locale pour la topologie \'etale
sur la source, chaque $f|_{X_i} : X_i \to Y$ possède $\mathcal{P}$. D'après
(AA), qui découle de (AAA) par l'Assertion 1, chaque $X_i \to Z$ possède
$\mathcal{P}$, pour tout $i$. Puisque $\{X_i \to X\}$ est un recouvrement
\'etale et que
$\mathcal{P}$ est locale pour la topologie \'etale sur la source, nous
...
Comme $\mathcal{P}$
est locale sur le but et que $X_i = (g \circ f)^{-1}(Z_i)$, nous concluons que
...
universellement bornées ; voir Morphismes,
Lemme \ref{morphisms-lemma-etale-locally-quasi-finite} et
...
des fibres de $Y \to Z$. Voir Morphismes,
Lemme \ref{morphisms-lemma-etale-universally-bounded}, pour une description de
cet entier dans le cas d'un morphisme \'etale.
Si $n = 1$, alors $Y \to Z$ est une immersion ouverte ; voir
...
$Y \times_Z Y = \Delta_{Y/Z}(Y) \amalg Y'$ ; voir Morphismes,
Lemme \ref{morphisms-lemma-diagonal-unramified-morphism}.
Puisqu'elle est obtenue par changement de base, la seconde projection
$\text{pr} : Y \times_Z Y \to Y$ a des fibres de degré borné par $n$ ; voir
...
et chacune de ses fibres possède exactement un point. En appliquant
Morphismes, Lemme \ref{morphisms-lemma-etale-universally-bounded}, nous
...
$$
Comme $\mathcal{P}$ est locale pour la topologie \'etale sur le but et donc
stable par changement de base \'etale (voir le
Lemme \ref{lemma-pullback-property-local-target}), nous voyons que $f_Y$
possède $\mathcal{P}$. Puisque $\mathcal{P}$ est locale pour la topologie
\'etale sur la source, $f' = f_Y|_{X'}$ possède $\mathcal{P}$. Par l'hypothèse
de récurrence, $X' \to Y$ possède $\mathcal{P}$. Comme $\mathcal{P}$ est locale
sur la source et que
$\{X \to X \times_Z Y, X' \to X \times_Y Z\}$ est un recouvrement \'etale,
...
de $g \circ f : X \to g(Y)$ par le recouvrement \'etale
$\{Y \to g(Y)\}$ ; puisque $\mathcal{P}$ est locale pour la topologie
\'etale sur le but, nous concluons que $g \circ f : X \to g(Y)$ possède la
...
types de morphismes dont la propriété est locale pour la topologie \'etale sur
la source et le but. Dans chaque cas, nous indiquons le lemme qui établit la
localité pour la topologie \'etale sur la source, puis celui qui établit la
localité pour la topologie \'etale sur le but. La troisième hypothèse du
...
$\mathcal{P}$ de morphismes est ``locale pour la topologie \'etale sur la
source et le but'' :
\begin{enumerate}
\item[(ST)] $\mathcal{P}$ est locale pour la topologie \'etale sur la source et
$\mathcal{P}$ est locale pour la topologie \'etale sur le but,
\item[(DM)] (définition donnée par Deligne et Mumford dans l'article
\cite[Page 100]{DM}) pour tout diagramme
$$
\xymatrix{
U \ar[d]_a \ar[r]_h & V \ar[d]^b \\
X \ar[r]^f & Y
...
$$
dont les flèches verticales sont surjectives et de type \'etale, on a
$\mathcal{P}(h) \Leftrightarrow \mathcal{P}(f)$, et
\item[(SP)] $\mathcal{P}$ est locale pour la topologie \'etale sur la source et
le but.
\end{enumerate}
...
Soit $\mathcal{P}$ une propriété de morphismes de schémas locale pour la
topologie \'etale sur la source et le but. Étant donné un diagramme commutatif
de schémas
$$
\vcenter{
\xymatrix{
...
$\mathcal{P}$ est locale pour la topologie \'etale sur la source d'après le
Lemme \ref{lemma-local-source-target-implies}.

\medskip\noindent
Supposons $x \in W(f)$. Soient $U' \subset X'$ et $V' \subset Y'$ des
voisinages ouverts de $x'$ et $y'$ tels que $f'(U') \subset V'$,
$g'(U') \subset W(f)$, et tels que $g'|_{U'}$ et $g|_{V'}$ soient tous deux de
type \'etale.
...
Supposons $x' \in W(f')$. Soient $U' \subset X'$ et $V' \subset Y'$ des
voisinages ouverts de $x'$ et $y'$ tels que $f'(U') \subset V'$,
$U' \subset W(f')$, et tels que $g'|_{U'}$ et $g|_{V'}$ soient tous deux de
type \'etale.
...
$g'|_U' : U' \to X$. Alors $\{U' \to U\}$ est un recouvrement \'etale, et
...
\begin{enumerate}
\item $\mathcal{P}$ soit locale pour la topologie \'etale sur la source,
\item $\mathcal{P}$ soit locale pour la topologie lisse sur le but,
\item $\mathcal{P}$ soit stable par postcomposition avec les immersions
ouvertes : si $f : X \to Y$ possède $\mathcal{P}$ et si $Y \subset Z$ est un
sous-schéma ouvert, alors $X \to Z$ possède $\mathcal{P}$.
\end{enumerate}
...
\medskip\noindent
Supposons $x \in W(f)$. Comme $\mathcal{P}$ est locale pour la topologie lisse
sur le but, nous voyons que $(g')^{-1}W(f) = W(f) \times_Y Y' \to Y'$ possède
$\mathcal{P}$. Ainsi, $(g')^{-1}W(f) \subset W(f')$. Nous concluons que
$x' \in W(f')$.

\medskip\noindent
Supposons $x' \in W(f')$. Pour tout voisinage ouvert $V' \subset Y'$ de $y'$,
nous pouvons remplacer $Y'$ par $V'$ et $X'$ par $U' = (f')^{-1}V'$, car
...
$Y' \to \mathbf{A}^n_Y$ au-dessus de $Y$ ; voir Morphismes,
Lemme \ref{morphisms-lemma-smooth-etale-over-affine-space}.
...
recouvrements pour la topologie \'etale sont des recouvrements lisses), nous
voyons que $\mathcal{P}$ est locale pour la topologie \'etale sur la source et
le but. Le Lemme \ref{lemma-etale-etale-local-source-target} montre que
...
$\mathcal{P}$, puisque $\mathcal{P}$ est locale pour la topologie lisse et que
$\{\mathbf{A}^n_Y \to Y\}$ est un recouvrement lisse.
...
Lemme \ref{lemma-orbits} et des morphismes \'etale d'espaces affines.
...
vu que $\mathcal{P}$ est locale pour la topologie \'etale sur la source et le
but). Puisque $T_p : \mathbf{A}^n_x \to \mathbf{A}^n_x$ est fini \'etale de
...
\section{Propriétés des morphismes de germes locales pour la topologie étale
sur la source et le but}
...
On dit que $\mathcal{Q}$ est {\it locale pour la topologie \'etale sur la
source et le but} si, pour tout diagramme commutatif
$$
\xymatrix{
...
$$
de germes dont les flèches verticales sont de type \'etale, on a
...
qui est locale pour la topologie \'etale sur la source et le but.
...
$$
Alors $\mathcal{Q}$ est locale pour la topologie \'etale sur la source et
le but au sens de la
...
locale pour la topologie \'etale sur la source et le but. Soit $Q$ la
...
$$
est locale pour la topologie \'etale sur la source et le but.
\end{lemma}
...
de type \'etale ; en particulier, elles sont essentiellement de présentation finie,
plates et non ramifiées (voir
Algèbre, Section \ref{algebra-section-etale}).
Ces homomorphismes verticaux sont donc fidèlement plats, voir
Algèbre, Lemme \ref{algebra-lemma-local-flat-ff}.
...
$\mathcal{O}_{U, u}$ ; il est donc plat d'après
...
$$
dont les flèches verticales sont de type \'etale, et un point $v' \in V'$
...
Le schéma $U'_v$ est un schéma sur $V'_v$. D'après
Morphismes, Lemme \ref{morphisms-lemma-etale-over-field}, le schéma
...
ouverte et d'un morphisme \'etale, voir
...
\begin{proof}
Étant donné un diagramme comme dans la
Définition \ref{definition-local-source-target-at-point}, nous obtenons un
...
$$
Les flèches verticales sont des localisations d'homomorphismes d'anneaux
de type \'etale ; elles sont en particulier non ramifiées (voir
Algèbre, Section \ref{algebra-section-etale}).
...
\ref{spaces-simplicial-section-simplicial-descent}, nous examinerons le lien
...
``miraculeuses''. Par exemple, le tiré en arrière de $\varphi$ par le
...
$\mathcal{U}$ raffine $\mathcal{V}$ (voir
Sites, Définition \ref{sites-definition-morphism-coverings}), mais qu'aucun
...
(Algèbre, Lemme \ref{algebra-lemma-local-flat-ff}), on en déduit que
...
et plat, il est surjectif et plat (voir
Morphismes, Lemmes \ref{morphisms-lemma-base-change-surjective}
et \ref{morphisms-lemma-base-change-flat}). Le résultat découle alors du
Lemme \ref{lemma-surjective-flat-epi}.
\end{proof}
...
$g_i : U_i \to V_{\alpha(i)}$ un morphisme de familles de morphismes de même
but ; voir Sites, Définition \ref{sites-definition-morphism-coverings}.
...
fpqc de schémas. En effet, d'après Topologies,
Lemme \ref{topologies-lemma-disjoint-union-is-fpqc-covering}, pour tout $j$,
...
$X$ pour appliquer Topologies,
Lemme \ref{topologies-lemma-recognize-fpqc-covering}, partie (3), ce qui prouve
...
Topologies, Définition \ref{topologies-definition-zariski-covering}) de
...
\begin{proof}
Le foncteur est fidèle et pleinement fidèle d'après le
...
l'ouvert correspondant de $X_{i'}$. Il résulte de Schémas,
Section \ref{schemes-section-glueing-schemes}, que l'on peut recoller
...
commutativité résulte du Lemme \ref{lemma-pullback-family}, partie (3).
...
Schémas, Section \ref{schemes-section-glueing-schemes} (preuve omise). On peut
donc supposer qu'il existe un schéma $U$ au-dessus de $S$ tel que
$U_i \subset U$ soit ouvert, que $U_{ij} = U_i \cap U_j$ et que
$\varphi_{ij} = \text{id}_{U_i \cap U_j}$ ; voir Schémas,
Lemme \ref{schemes-lemma-glue}. Après changement de base à $X$, les
...
que ce n'est pas toujours le cas. Nous le verrons dans Espaces algébriques,
Exemple \ref{spaces-example-non-representable-descent}. Même les morphismes
projectifs ne satisfont pas toujours à la descente pour les recouvrements
fpqc, d'après Exemples,
...
base. Soit
$\tau \in \{Zariski, fpqc, fppf, \etale, smooth, syntomic\}$. On dit que
{\it les morphismes de type $\mathcal{P}$ satisfont à la descente pour les
recouvrements $\tau$} si, pour tout recouvrement
$\tau$ $\mathcal{U} : \{U_i \to S\}_{i \in I}$ (voir Topologies,
Section \ref{topologies-section-procedure}), toute donnée de descente
...
base. Soit $\tau \in \{fpqc, fppf, \etale, smooth, syntomic\}$. Supposons que
\begin{enumerate}
\item $\mathcal{P}$ soit préservée par tout changement de base (voir Schémas,
Définition \ref{schemes-definition-preserved-by-base-change}),
...
$\mathcal{U} = \{\varphi_i : U_i \to S\}_{i \in I}$ un recouvrement
$\tau$ de $S$. Soit $(X_i, \varphi_{ii'})$ une donnée de descente relativement
...
\medskip\noindent
Commençons par démontrer le lemme lorsque $S$ est affine. D'après Topologies,
Lemmes \ref{topologies-lemma-fpqc-affine},
\ref{topologies-lemma-fppf-affine},
\ref{topologies-lemma-etale-affine},
\ref{topologies-lemma-smooth-affine} ou
\ref{topologies-lemma-syntomic-affine}, il existe un recouvrement
$\tau$ standard
...
que la donnée de descente au-dessus du recouvrement $\tau$ standard
...
$\mathcal{U}_V = \{V \times_S U_i \to V\}_{i \in I}$ est un recouvrement
$\tau$ de $V$. Notons $(X_i, \varphi_{ii'})_V$ la restriction, ou le
...
identifie $X_{V'}$ à l'image réciproque de $V'$ dans $X_V$. Nous omettons de
vérifier que, pour des ouverts affines $V'' \subset V' \subset V$ de $S$, on a
$\rho^V_{V''} = \rho^V_{V'} \circ \rho^{V'}_{V''}$.

\medskip\noindent
D'après Constructions, Lemme \ref{constructions-lemma-relative-glueing}, les
...
fpqc''. En voici l'énoncé formel.

\begin{lemma}
\label{lemma-affine}
Soit $S$ un schéma. Soit $\{X_i \to S\}_{i\in I}$ un recouvrement fpqc ; voir
Topologies, Définition \ref{topologies-definition-fpqc-covering}. Soit
...
et préservée par tout changement de base ; voir Morphismes,
Lemmes \ref{morphisms-lemma-characterize-affine} et
\ref{morphisms-lemma-base-change-affine}. Le
...
$A$-algèbre $B$ (voir Morphismes,
Définition \ref{morphisms-definition-affine}). L'isomorphisme $\varphi$
...
(Morphismes, Lemme \ref{morphisms-lemma-closed-immersion-affine}), si bien que
...
par tout changement de base ; voir Morphismes,
Lemmes \ref{morphisms-lemma-characterize-quasi-affine} et
\ref{morphisms-lemma-base-change-quasi-affine}. Le
Lemme \ref{lemma-descending-types-morphisms} s'applique donc, et il suffit de
démontrer l'énoncé lorsque le recouvrement fpqc est donné par un unique
morphisme plat et surjectif de schémas affines $\{X \to S\}$. Écrivons
$X = \Spec(A)$ et $S = \Spec(R)$, de sorte que $R \to A$ soit un
...
\medskip\noindent
D'après Morphismes,
Lemme \ref{morphisms-lemma-characterize-quasi-affine}, cela signifie que
$$
...
$$
par changement de base plat (Cohomologie des schémas,
Lemme \ref{coherent-lemma-flat-base-change-cohomology}). Ainsi,
...
on voit immédiatement que $V$ est l'image réciproque d'un sous-ensemble
...
aussi quasi-compact, plat et surjectif. D'après Morphismes,
Lemme \ref{morphisms-lemma-fpqc-quotient-topology}, le sous-ensemble
...
Soit $\tau \in \{Zariski, fppf, \etale, smooth, syntomic\}$\footnote{L'absence
de fpqc n'est pas une coquille. Voir la discussion dans Topologies,
Section \ref{topologies-section-fpqc}.}. Soit $\Sch_\tau$ un grand
$\tau$-site. Soit $S \in \Ob(\Sch_\tau)$. Soit
...
$(\Sch/S)_\tau$. De plus, le lemme de Yoneda (Catégories,
Lemme \ref{categories-lemma-yoneda}) garantit que les morphismes entre
