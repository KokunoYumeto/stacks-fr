...
\section{Limites filtrantes de schémas à morphismes de transition affines}
...
Soit $S$ un schéma. Soit $X = \lim X_i$ une limite filtrante de schémas sur
...
aux constructions. Nous affirmons que $S$ est la limite filtrante des
...
Soit $(A_i, \varphi_{ii'})$ un système filtrant
...
$h_X : (\Sch/S)^{opp} \to \textit{Sets}$ son foncteur des points ; voir
...
Soit $f : X \to S$ un morphisme de schémas. Si, pour toute limite filtrante
...
$(A_i, \varphi_{ii'})$ un système filtrant d'$\mathcal{O}_S(V)$-algèbres.
...
\item $S$ soit quasi-séparé.
\end{enumerate}
Alors $X = \lim X_i$ est la limite d'un système filtrant de schémas $X_i$
...
Soit $S = \lim S_i$ la limite d'un système filtrant de schémas dont les
...
\item $f_0$ est localement de présentation finie,
\end{enumerate}
alors $f_i$ est \'etale pour un certain $i \geq 0$.
\end{lemma}

\begin{proof}
La propriété d'être \'etale est locale sur la source et sur le but (Morphismes,
...
$f_i$ soit \'etale. Alors $V_i = f_i(X_i)$ est un sous-schéma ouvert
...
et quasi-séparé. Alors $X$ peut s'écrire comme une limite filtrante
...
\item $S$ est quasi-compact et quasi-séparé.
\end{enumerate}
Alors $X$ est une limite filtrante $X = \lim X_i$, dont les morphismes de
...
Soit $S = \lim S_i$ la limite d'un système filtrant de schémas
...
Soit $S = \lim S_i$ une limite filtrante de schémas à morphismes
...
une limite filtrante de schémas de type fini sur $\mathbf{Z}$.
...
\medskip\noindent
D'après la Proposition \ref{proposition-approximate}, on peut écrire
$X = \lim X_i$ comme une limite filtrante de schémas de type fini sur $\mathbf{Z}$
...
écrire $X = \lim X_i$ comme la limite d'un système filtrant
...
Écrivons $A = \colim A_j$ comme la colimite de ses $k$-sous-algèbres finies.
...
\medskip\noindent
Écrivons $S = \lim_i S_i$ comme une limite filtrante de schémas noethériens ; voir
...
quasi-séparé. Alors $X = \lim X_i$ est une limite filtrante de schémas
...
Soit $S$ un schéma. Soit $X = \lim X_i$ une limite filtrante de
...
\medskip\noindent
Pour terminer, écrivons $B$ comme colimite de
...
\medskip\noindent
D'après le Lemme \ref{lemma-proper-limit-of-proper-finite-presentation},
on peut écrire $X = \lim X_i$ comme une limite cofiltrée, où $X_i \to Y$ est propre
...
Écrivons $Y = \lim_{i \in I} Y_i$ comme une limite cofiltrée de schémas affines,
...
comme une colimite filtrante de $\mathcal{O}_X$-modules cohérents ; voir
Propriétés, Lemme
...
par l'équivalence. Si $f$ et $g$ sont séparés, propres, finis ou \'etales,
...
Si $a'$ et $b$ sont séparés, propres, finis ou \'etales,
...
par l'équivalence. Alors $f$ est séparé, propre, fini, \'etale,
...
Soit $S = \lim_{i \in I} S_i$ la limite d'un système filtrant de
