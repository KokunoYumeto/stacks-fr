...
lorsque $\Lambda$ est de torsion. Alors
$H^i_c(X, K)$ est un $\Lambda$-module fini pour tout
$i \in \mathbf{Z}$.
\end{lemma}
...
Alors $R^if_*\mathcal{F}$ est un faisceau localement constant de type fini de
$\Lambda$-modules sur $S_\etale$ pour tout $i \in \mathbf{Z}$.
\end{proposition}
...
on peut supposer $n$ inversible dans l'anneau qui définit le
schéma affine $S$.
...
fini dont le changement de base à $S$ est $f : X \to S$ ; voir le
lemme \ref{limits-lemma-descend-finite-presentation} de Limites.
Après avoir augmenté $i$, on peut supposer $f_i : X_i \to S_i$
...
il existe un $i$ et un faisceau localement constant
de type fini de $\Lambda$-modules
...
D'autre part, comme $j_!$ est adjoint à gauche de $j^{-1}$, on dispose de
la counité $\text{Tr}_j : j_!\Lambda = j_!j^{-1}\Lambda \to \Lambda$ ;
...
\medskip\noindent
Soit $f : X \to Y$ un morphisme séparé de type fini de schémas quasi-compacts
...
Rappelons que $\Hom_X(j_!\Lambda, M[n]) = H^n(U, M)$ pour tout $M$ de
...
$$
Nous omettons la vérification que les morphismes de transition sont induits par les morphismes
