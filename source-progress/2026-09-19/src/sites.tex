...
de $\mathcal{C}$ vers $\textit{Sets}$ (voir Catégories, Remarque
...
contravariant de $\mathcal{C}$ vers $\textit{Sets}$. Les {\it morphismes
...
la somme amalgamée est calculée dans $\textit{Sets}$ et induite par
...
que si $f : A \to B$ est un morphisme dans $\textit{Sets}$, alors
...
Soit $G$ un groupe. Considérons la catégorie $G\textit{-Sets}$ dont les objets
...
recouvrements sur $G\textit{-Sets}$ dont on vérifie aisément qu’elle satisfait
...
Nous remplaçons d’abord $G\textit{-Sets}$ par une sous-catégorie pleine
$G\textit{-Sets}_\alpha$ comme dans Ensembles,
...
\begin{enumerate}
\item[(a)] $\mathcal{T}_G$ est une sous-catégorie pleine de $G\textit{-Sets}$,
...
de $H^0(-, \mathcal{F}) : \mathcal{J}_U^{opp} \to \textit{Sets}$
vers la composée
$\mathcal{J}_U^{opp} \to \mathcal{J}_V^{opp}
\xrightarrow{H^0(-, \mathcal{F})} \textit{Sets}$. Par les
...
\to \textit{Sets}$ est filtrant, voir
Catégories, Définition \ref{categories-definition-directed}.
...
$\mathcal{C}^{opp}$ à valeurs dans $\textit{Sets}^{opp}$,
...
$\textit{Sets}^{opp}$. Le résultat s’en déduit formellement.
\end{proof}
...
$p = u : \mathcal{C} \to \textit{Sets}$. Aucune condition n’est nécessaire
...
par un foncteur $u : \mathcal{C} \to \textit{Sets}$ tel que
\begin{enumerate}
...
\item Le foncteur fibre $\Sh(\mathcal{C}) \to \textit{Sets}$,
...
Soit $\mathcal{C}$ un site.
Soit $p = u : \mathcal{C} \to \textit{Sets}$ un foncteur.
...
\noindent
Soit $\mathcal{C}$ un site. Soit
$p = u : \mathcal{C} \to \textit{Sets}$ un foncteur. Par analogie avec les
...
Pour tout foncteur $u : \mathcal{C} \to \textit{Sets}$, le foncteur $u^p$ est
...
Soit $\mathcal{C}$ un site. Soit
$p = u : \mathcal{C} \to \textit{Sets}$ un foncteur. Supposons que, pour tout
...
notons-le $u^sE$,
\item le foncteur fibre $\Sh(\mathcal{C}) \to \textit{Sets}$ et le foncteur
$u^s: \textit{Sets} \to \Sh(\mathcal{C})$ sont adjoints, et
...
$u : \mathcal{C} \to \textit{Sets}$. Soit $S_0$ un ensemble infini tel que
...
Soit $\mathcal{C}$ un site. Soit
$p : \Sh(pt) \to \Sh(\mathcal{C})$ un point du topos associé à
$\mathcal{C}$. Le foncteur $p_* : \textit{Sets} \to \Sh(\mathcal{C})$
...
$u : \mathcal{C} \to \textit{Sets}$ du site sous-jacent $\mathcal{C}$.
...
Soit $\mathcal{C}$ un site. Soit $p = u : \mathcal{C} \to \textit{Sets}$
...
Soit $p = u : \mathcal{C} \to \textit{Sets}$ un foncteur tel que
\begin{enumerate}
...
et, par conséquent, le foncteur fibre $\Sh(\mathcal{C}) \to \textit{Sets}$,
...
est donné par un foncteur $u : \mathcal{C} \to \textit{Sets}$ tel que
\begin{enumerate}
...
$X$ et des produits fibrés. Soit $p = u: \mathcal{C} \to \textit{Sets}$ un
foncteur tel que
\begin{enumerate}
\item $u(X) = \{*\}$ soit un singleton, et
\item pour toute paire de morphismes $U \to W$ et $V \to W$ de
...
$u : X_{Zar} \to \textit{Sets}$. Nous allons utiliser la
...
l’Exemple \ref{example-site-on-group} et la
Section \ref{section-example-sheaf-G-sets}.
Le foncteur d’oubli $u : \mathcal{T}_G \to \textit{Sets}$
...
et $G\textit{-Sets}$. Le foncteur fibre
$$
p^{-1} :
...
$v : \mathcal{D} \to \textit{Sets}$ un foncteur, et posons
...
$v : \mathcal{D} \to \textit{Sets}$ ; voir la
Définition \ref{definition-point}.
Alors le foncteur $v \circ u : \mathcal{C} \to \textit{Sets}$
...
$\mathcal{S} \subset \textit{Sets}$ est une sous-catégorie pleine contenant
...
Soit $\mathcal{C}$ un site. Soit $p$ un point de $\mathcal{C}$ donné par
$u : \mathcal{C} \to \textit{Sets}$. Soit $U$ un objet de $\mathcal{C}$
...
$v : \mathcal{C}/U \to \textit{Sets}$ tel que $j_U \circ q = p$
...
Soit $\mathcal{C}$ un site.
Soient $u, u' : \mathcal{C} \to \textit{Sets}$ deux
...
donnés par des foncteurs $u, u' : \mathcal{C} \to \textit{Sets}$.
...
et $s : \mathcal{A} \to \textit{Sets}$ est un foncteur tel
que
\begin{enumerate}
\item $s$ soit fidèle,
...
$p = u : \mathcal{C} \to \textit{Sets}$, nous pouvons définir
un foncteur fibre
$$
...
$u : \mathcal{C} \to \textit{Sets}$ tel que
\begin{enumerate}
