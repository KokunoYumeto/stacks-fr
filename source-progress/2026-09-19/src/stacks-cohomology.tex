...
\section{Modules localement quasi-cohérents possédant la propriété de changement de base plat}
...
au développement des champs algébriques adopté dans le projet Stacks
...
Dans le projet Stacks, notre convention consiste à nommer le site et à
...
$$
on voit que
$((\varphi, 0) : V \to U \times \mathbf{A}^1, \text{pr}_2^*t)$
...
et $s$ par $\text{pr}_1^*s + \text{pr}_2^*t$. Nous pouvons ainsi supposer que
...
$\QCoh(\mathcal{O}_\mathcal{X}) = \QCoh(U, R, s, t, c)$ et
$\textit{Coh}(\mathcal{O}_\mathcal{X}) = \textit{Coh}(U, R, s, t, c)$.
