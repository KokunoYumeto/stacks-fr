...
R\SheafHom_{\mathcal{O}_Y}(Rf_*L, K)
$$
où la première flèche est celle de Cohomologie, remarque \ref{cohomology-remark-projection-formula-for-internal-hom}, et la seconde est la coünité $Rf_*a(K) \to K$ de l'adjonction.
...
\section{Adjoint à droite de l'image directe et applications de trace}
\label{section-trace}
...
$$
L'application correspondante $\text{Tr}_{f, K} : Rf_*a(K) \longrightarrow K$, pour $K \in D_\QCoh(\mathcal{O}_Y)$, est parfois appelée {\it application de trace}. Elle se caractérise par le fait que la bijection
$$
...
R\SheafHom_{\mathcal{O}_Y}(Rf_*L, K)
$$
s'obtient par composition avec $\text{Tr}_{f, K}$. Toute application de trace considérée dans cette section sera un cas particulier de celle-ci. Avant d'aborder quelques cas particuliers, montrons que la formation de la trace commute au changement de base.

\begin{lemma}[Application de trace et changement de base]
...
$$
où la première flèche est l'unité pour $(g^*, g_*)$, la deuxième est $\text{Tr}_f$ et la troisième est la coünité pour $(g^*, g_*)$. Cela résulte simplement du fait que la composée $g^* \to g^* \circ g_* \circ g^* \to g^*$ de l'unité et de la coünité est l'identité. Considérons le diagramme
\begingroup\small
$$
\xymatrix{
...
a' \circ f'_* \circ (g')^*
$$
où la première flèche est l'unité pour $((g')^*, g'_*)$, la deuxième est $\eta_{f'}$ et la troisième est la coünité pour $((g')^*, g'_*)$. Cela résulte simplement du fait que la composée
$(g')^* \to (g')^* \circ (g')_* \circ (g')^* \to (g')^*$
de l'unité et de la coünité est l'identité. Considérons le diagramme
$$
\xymatrix{
...
envoie un objet $K$ de $D(A) = D_\QCoh(\mathcal{O}_Y)$ sur $R\Hom(B, K)$ dans $D(B) = D_\QCoh(\mathcal{O}_X)$. L'application de trace est l'application
$$
...
$$
(égalité d'après Catégories dérivées des schémas, lemme \ref{perfect-lemma-cohomology-base-change}), pour laquelle on utilise l'application de trace $Rf_*a(L) \to L$. Lorsque $L = \mathcal{O}_Y$, on obtient une application
\begin{equation}
...
Soit $f : X \to Y$ un morphisme propre de schémas noethériens. Soit $V \subset Y$ un ouvert tel que $f^{-1}(V) \to V$ soit un isomorphisme. Alors, pour $K \in D_\QCoh^+(\mathcal{O}_Y)$, l'application (\ref{equation-compare-with-pullback}) se restreint en un isomorphisme au-dessus de $f^{-1}(V)$.
\end{lemma}
...
$$
tensorisée par $\text{id}_K$. Ici, $\epsilon$ est l'isomorphisme de Catégories dérivées des schémas, lemme \ref{perfect-lemma-cohomology-base-change}, et $\beta$ provient de la coünité $g_*b \to \text{id}$. De même, la composée des flèches horizontales inférieures est adjointe à $\text{id}_K$ tensorisé par la composée
$$
...
\mathcal{O}_Z
$$
où $\gamma$ provient de la coünité $f_*a \to \text{id}$, et $\delta$ est l'application dont l'adjointe est la composée
$$
...
b(\mathcal{O}_Z)
$$
Les propriétés générales des foncteurs adjoints, des applications adjointes et des coünités (voir Catégories, section \ref{categories-section-adjoint}) donnent $\gamma \circ f_*\delta = \alpha \circ \epsilon^{-1}$, comme voulu.
\end{proof}
...
\begin{proof}
Pour justifier la remarque entre parenthèses, si $X$ est localement noethérien, alors $i$ est pseudo-cohérente d'après Compléments sur les morphismes, lemme \ref{more-morphisms-lemma-Noetherian-pseudo-coherent}.
...
$R\SheafHom_{\mathcal{O}_X}(i_*\mathcal{O}_Z, K)$
\hfil\par\noindent
appartient à $D_\QCoh(\mathcal{O}_X)$. Puisque $i$ est pseudo-cohérente, le faisceau $\mathcal{O}_Z$ est un $\mathcal{O}_X$-module pseudo-cohérent. Le résultat découle donc de Catégories dérivées des schémas, lemme \ref{perfect-lemma-quasi-coherence-internal-hom}.
...
Lf^*K
$$
Pour celle-ci, on peut prendre $Lf^*(can)$, où $can : Ri_* R\SheafHom(\mathcal{O}_Z, K) \to K$ est la coünité de l'adjonction.
...
$$
d'après les lemmes \ref{lemma-sheaf-with-exact-support-adjoint} et \ref{lemma-sheaf-with-exact-support-ext}. Pour cette application, on prend la coünité
\par\noindent\hfil
...
$$
est commutatif pour $K \in D_\QCoh^+(\mathcal{O}_X)$. Cela résulte du lemme \ref{lemma-finite-twisted}. La flèche horizontale inférieure est induite par l'application $\mathcal{O}_X \to f_*\mathcal{O}_Y$, et la flèche horizontale supérieure est l'application de trace de la section \ref{section-trace}.
\end{remark}
...
pour $K \in D_\QCoh(\mathcal{O}_Y)$. De plus, l'application de trace
$$
\text{Tr}_{f, \mathcal{O}_Y} : Rf_*\omega_{X/Y}^\bullet \to \mathcal{O}_Y
$$
de la section \ref{section-trace} induit l'application de trace pour tout $K$ dans $D_\QCoh(\mathcal{O}_Y)$. Plus précisément, on a le diagramme
$$
\xymatrix{
...
\noindent
Nous allons maintenant utiliser le recollement des complexes dualisants pour obtenir une théorie valable pour tous les schémas de type fini sur $S$, étant donné un couple $(S, \omega_S^\bullet)$ comme dans la situation \ref{situation-dualizing}. Cette construction est analogue à \cite[Remark on page 310]{RD}.
...
Soit $S$ un schéma noethérien et soit $\omega_S^\bullet$ un complexe dualisant. Soit $f : X \to Y$ un morphisme propre de schémas de type fini sur $S$. Soient $\omega_X^\bullet$ et $\omega_Y^\bullet$ des complexes dualisants normalisés relativement à $\omega_S^\bullet$. Dans cette situation, on a $a(\omega_Y^\bullet) = \omega_X^\bullet$ (lemme \ref{lemma-proper-map-good-dualizing-complex}) ; l'application de trace (section \ref{section-trace}) est donc une flèche canonique
$$
...
$$
dans $D_\QCoh^+(f_*\mathcal{O}_X)$ d'après les lemmes \ref{lemma-finite-twisted} et \ref{lemma-proper-map-good-dualizing-complex}, et l'application de trace de l'exemple \ref{example-trace-proper} est l'application
$$
...
\item $U = \{x \in X \mid \mathcal{O}_{X, x}\text{ is Cohen-Macaulay}\}$ est ouvert et de Cohen–Macaulay.
\end{enumerate}
...
\item $U = \{x \in X \mid \mathcal{O}_{X, x}\text{ is Gorenstein}\}$ est un sous-schéma ouvert de Gorenstein.
\end{enumerate}
...
$$
d'après les propriétés connues des foncteurs image inverse exceptionnelle (lemmes \ref{lemma-upper-shriek-composition} et \ref{lemma-shriek-affine-line}). L'équivalence de (a), (b) et (c) résulte donc du lemme \ref{lemma-affine-flat-Noetherian-gorenstein}.
\end{proof}
...
est envoyée par le morphisme de bord sur un élément $\xi_m$ de $\Ext^i_X(\mathcal{I}^m\Im(\varphi), \mathcal{G})$ qui s'envoie sur $\xi$. Puisque $\Im(\varphi)$ et $\mathcal{F}$ coïncident sur $U$, on voit que $\mathcal{F}/\mathcal{I}^m\Im(\varphi)$ est supporté sur $X \setminus U$. Il existe donc un $l \geq m$ tel que $\mathcal{I}^l\mathcal{F} \subset \mathcal{I}^m\Im(\varphi)$, voir Cohomologie des schémas, lemme \ref{coherent-lemma-power-ideal-kills-sheaf}. En prenant l'image de $\xi_m$ dans $\Ext^i_X(\mathcal{I}^l\mathcal{F}, \mathcal{G})$, on gagne.
\end{proof}
...
\begin{proof}
En effet, si $(K_n)$ est un système de Deligne, il existe un $b \in \mathbf{Z}$ tel que $H^i(K_n) = 0$ pour $i > b$. Alors $\Hom(K_n, L) = \Hom(K_n, \tau_{\leq b}L)$ et $\Hom(K, L) = \Hom(K, \tau_{\leq b}L)$. En utilisant le résultat du lemme pour $\tau_{\leq b}L$, on gagne.
\end{proof}
...
\end{align*}
fonctoriel en $L$ et $K$. Ici, on a appliqué deux fois la proposition \ref{proposition-duality-compactly-supported}, ainsi que la construction des foncteurs image inverse exceptionnelle, qui montre que $g^! = i^* \circ f^!$. La fonctorialité en $L$ montre, d'après Catégories, remarque \ref{categories-remark-pro-category-copresheaves}, que l'on obtient un morphisme canonique $Rg_!(K|_U) \to Rf_!K$ dans $\text{Pro-}D^b_{\textit{Coh}}(\mathcal{O}_Y)$, qui est fonctoriel en $K$ par la fonctorialité en $K$ de la flèche ci-dessus.
\end{enumerate}
