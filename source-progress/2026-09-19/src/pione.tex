...
La catégorie des $G$-ensembles est notée {\it $G\textit{-Sets}$}.
\end{definition}
...
Sites, Example \ref{sites-example-site-on-group}), pourvu que l'on munisse
...
Fields, Section \ref{fields-section-infinite-galois}.
...
surjectives de morphismes dans $G\textit{-Sets}$.
\end{remark}
...
trouvera dans \cite[Expos\'e V, Sections 4, 5, and 6]{SGA1}.

\medskip\noindent
Soit $F : \mathcal{C} \to \textit{Sets}$ un foncteur.
...
compacte-ouverte ; voir Topology, Example
...
$F : \mathcal{C} \to \textit{Sets}$ un foncteur. L'application
...
\end{equation}
où $\textit{Finite-}G\textit{-Sets}$ est la sous-catégorie pleine de
$G\textit{-Sets}$ dont les objets sont les $G$-ensembles finis.
La catégorie $G\textit{-Sets}$ des $G$-ensembles est définie dans la
...
$X \to Y$ dans $\textit{Finite-}G\textit{-Sets}$ commute à l'action de $G$.
...
un homéomorphisme d'après Topology, Lemma
...
$$
de $\textit{Finite-}G\textit{-Sets}$ commute à l'action de $\gamma$. Ainsi
$$
...
\begin{proof}
Soit $X$ un objet non vide de $\textit{Finite-}G\textit{-Sets}$. Le diagramme
$$
\xymatrix{
X \ar[r] \ar[d] & \{*\} \ar[d] \\
\{*\} \ar[r] & \{*\}
...
$$
est non vide (Categories, Lemma \ref{categories-lemma-nonempty-limit}).
...
Soit $\mathcal{C}$ une catégorie et soit
$F : \mathcal{C} \to \textit{Sets}$ un foncteur tel que $F(X)$ soit fini
...
\end{reference}
Soit $\mathcal{C}$ une catégorie et soit $F : \mathcal{C} \to \textit{Sets}$
...
définie comme une catégorie équivalente à $\textit{Finite-}G\textit{-Sets}$
...
$\text{Aut}(Y) = \text{Aut}_{G\textit{-Sets}}(G/U)$ opère transitivement
...
$\mathcal{C} = \textit{Finite-}G\textit{-Sets}$ et que $F$ est le
...
$\textit{Finite-}G\textit{-Sets} \to \textit{Finite-}G'\textit{-Sets}
\to \textit{Finite-}G''\textit{-Sets}$ déterminés par $h$ et $h'$.
...
la clôture algébrique séparable.
\begin{enumerate}
...
Puisque $F_{\overline{x}} : \textit{F\'Et}_X \to \textit{Sets}$ est le foncteur fibre d'une
...
aux $\pi_1(X, \overline{x})\textit{-Sets}$ finis
...
\noindent
Le lemme suivant nous dit que le groupe fondamental d'une paire hensélienne
est le groupe fondamental de la partie fermée.

\begin{lemma}
\label{lemma-gabber}
Soit $(A, I)$ une paire hensélienne. Posons $X = \Spec(A)$ et $Z = \Spec(A/I)$.
Le foncteur
$$
...
est le même que celui du schéma initial. Nous l'utiliserons
...
où $I'$ est un idéal localement nilpotent. Alors $(A', I')$ est une
paire hensélienne (Compléments d'algèbre, lemme
...
En appliquant la surjectivité essentielle à $X = U \times_X V$, on trouve
...
\ref{limits-lemma-descend-etale}). Écrivons
$$
...
Soit $(A, I)$ une paire hensélienne. Soit $X$ un schéma propre sur $A$.
...
\medskip\noindent
Surjectivité essentielle lorsque $(A, I)$ est une paire hensélienne
...
\medskip\noindent
Pleine fidélité lorsque $(A, I)$ est une paire hensélienne et que $A$ est un
G-anneau noethérien. Elle se déduit de la surjectivité essentielle exactement de la même
manière que dans le cas où $A$ est noethérien complet.

\medskip\noindent
Cas général. Soit $(A, I)$ une paire hensélienne.
...
C'est une colimite filtrante d'homomorphismes locaux qui sont
des localisés d'homomorphismes d'anneaux étales.
...
est constituée des $\text{Gal}(K^{sep}/K)\textit{-Sets}$ isomorphes
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-henselization-normal}.
...
Compléments sur l'algèbre, section \ref{more-algebra-section-group-actions-integral}.
...
$A = (A \otimes_{R^G} R)^G$, voir Compléments sur l'algèbre,
...
Une seconde application de Compléments sur l'algèbre,
lemme \ref{more-algebra-lemma-base-change-invariants},
...
D'après Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-one-orbit},
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-one-orbit-geometric}.
...
(Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-one-orbit}),
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-inertial-invariants-unramified}.
...
\noindent
Dans cette section, on poursuit la discussion de
Compléments sur l'algèbre, section \ref{more-algebra-section-ramification},
et on la relie à notre discussion des groupes fondamentaux des schémas.

\medskip\noindent
Soit $(A, \mathfrak m, \kappa)$ un anneau local normal de
corps des fractions $K$. Choisissons une clôture algébrique séparable $K^{sep}$. Soit
...
parce que $\kappa(\mathfrak m^{sh})$ est une clôture algébrique séparable
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-one-orbit-geometric-galois}.
...
et la clôture algébrique séparable de $\kappa$.
...
\noindent
Soit $X$ un schéma intègre normal de corps des fonctions rationnelles $K$.
Soit $K^{sep}$ une clôture algébrique séparable de $K$.
...
de Compléments sur l'algèbre, lemme
...
Compléments sur l'algèbre, exemple
...
démontrés dans Compléments sur l'algèbre, section \ref{more-algebra-section-ramification}.
La surjectivité de l'application $D \to \text{Aut}(\kappa(\mathfrak m)/\kappa_A)$ est le
lemme \ref{more-algebra-lemma-one-orbit-geometric-galois} de Compléments sur l'algèbre.
...
$\mathfrak m_\lambda$, voir Compléments sur l'algèbre, lemme
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-compare-inertia}.
...
(voir Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-compare-inertia}),
...
$$
voir
Compléments sur l'algèbre, remarque \ref{more-algebra-remark-canonical-inertia-character}.
...
$\lim_{n\text{ prime to }p} \mu_n(\kappa^{sep})$. Pour le voir,
...
\section{Applications de spécialisation}
\label{section-specialization-map}

\noindent
Dans cette section, nous construisons des applications de spécialisation.
Soit $f : X \to S$ un morphisme propre de schémas
...
au-dessus de $s$ et $s'$. Il existe alors une application de spécialisation
$$
sp : \pi_1(X_{\overline{s}'}) \longrightarrow \pi_1(X_{\overline{s}})
$$
La construction de cette application est la suivante. Soit $A$ le
...
$$
L'application de spécialisation est la composée
$$
...
Par construction, l'application de spécialisation s'insère dans un diagramme
commutatif
$$
\xymatrix{
...
$$
pourvu que $X$ soit connexe. L'application de spécialisation dépend du
...
et considérons une application de spécialisation
...
$$
d'applications de spécialisation, où $\overline{s}$ et $\overline{s}'$
...
comme dans la construction de l'application de spécialisation.
...
$$
et, d'après la construction de l'application de spécialisation, la commutativité
...
Une composée d'applications de spécialisation
$\pi_1(X_{\overline{s}''}) \to \pi_1(X_{\overline{s}'}) \to
\pi_1(X_{\overline{s}})$ est une application de spécialisation
...
la première application de spécialisation. Soit $\mathcal{O}_{S, s'} \to A'$
...
construire la seconde application de spécialisation. Il est alors clair que
la composée de la première et de la seconde application de spécialisation
est l'application de spécialisation
$\pi_1(X_{\overline{s}''}) \to \pi_1(X_{\overline{s}})$
construite à l'aide de $A \to A' \to \kappa(\overline{s}'')$.
\end{proof}
...
On peut alors considérer l'application de spécialisation
$$
...
une application de spécialisation. Il existe alors un anneau de valuation strictement hensélien
...
générique et fermé. On peut alors considérer l'application de spécialisation
$$
...
Soit $s' \leadsto s$ une spécialisation de points de $S$ et soit
$sp : \pi_1(X_{\overline{s}'}) \to \pi_1(X_{\overline{s}})$
une application de spécialisation. Si $S$ est noethérien, alors
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-henselization-dvr}.
...
$$
d'applications de spécialisation, par la construction de ces applications.
...
Compléments sur l'algèbre, proposition \ref{more-algebra-proposition-equivalence}.
...
Compléments sur l'algèbre, remarque \ref{more-algebra-remark-glueing-data}.
...
Compléments sur l'algèbre, proposition \ref{more-algebra-proposition-ratliff}.
...
Compléments sur l'algèbre, section \ref{more-algebra-section-properties-formal-fibres},
...
D'après Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-integral-closure-reflexive},
$B'$ est un $A$-module réflexif, ce qui implique à son tour
que $\text{depth}_A(B') \geq 2$ d'après
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-reflexive-over-normal}.
...
lemme de Compléments sur l'algèbre \ref{more-algebra-lemma-finite-etale-equivalence} ;
remarquons que $(A, fA)$ est un couple hensélien d'après le
lemme de Compléments sur l'algèbre \ref{more-algebra-lemma-complete-henselian}.
...
\label{lemma-purity}
\begin{reference}
\cite{Nagata-Purity} et \cite[Exp. X, Thm. 3.1]{SGA1}
\end{reference}
\begin{history}
...
Compléments sur l'algèbre, section \ref{more-algebra-section-permanence-etale},
et
...
module cohérent réflexif sur $X$ dont la restriction à
est $\pi_*\mathcal{O}_V$ sur $U$. Cela démontre (1).
...
D'après l'hypothèse (5), le morphisme $V \to U$ se prolonge
...
D'après Compléments sur l'algèbre, proposition \ref{more-algebra-proposition-fs-regular},
...
(Plus sur l'algèbre, lemme \ref{more-algebra-lemma-finite-type-over-excellent}).
...
Dans le cas général, il s'agit de \cite[Theorem 2.4]{Zong}, attribué à Gabber.
\end{reference}
Soit $f : X \to Y$ un morphisme de schémas localement noethériens.
Soit $x \in X$ et posons $y = f(x)$. Supposons que
\begin{enumerate}
...
$X'$ est \'etale sur $\Spec(A^\wedge)$ hors de la fibre fermée et
de sorte que $X'$ est normal hors de la fibre fermée. Puisque $X$ est normal,
...
La discussion dans \cite[Chapter IV, Section 21.12]{EGA}
...
Compléments sur l'algèbre, section \ref{more-algebra-section-permanence-completion}),
alors on voit que
$$
...
lemme de Compléments sur l'algèbre \ref{more-algebra-lemma-neighbourhood-equivalence}.
...
Le résultat découle donc du
lemme de Cohomologie locale
lemme \ref{local-cohomology-lemma-structure-torsion-D-module-regular}.
\end{proof}
...
(Compléments sur l'algèbre, exemple \ref{more-algebra-example-tor})
$$
...
\section{Applications de spécialisation dans le cas propre et lisse}
...
Soit $s \leadsto s'$ une spécialisation de points de $S$.
Alors l'application de spécialisation
$$
sp : \pi_1(X_{\overline{s}}) \longrightarrow \pi_1(X_{\overline{s}'})
$$
...
\item si la caractéristique de $\kappa(s')$ est $p > 0$, elle
...
Si $X_s$ est géométriquement réduit, alors l'application de
...
dans la construction de l'application de spécialisation, voir la
...
connexes. Soit $s' \leadsto s$ une spécialisation.
Si la caractéristique de $\kappa(s)$ est nulle, alors l'application de
spécialisation
$$
sp : \pi_1(X_{\overline{s}'}) \to \pi_1(X_{\overline{s}})
$$
...
Il reste donc à montrer qu'elle est injective.
...
considère l'application de spécialisation sur $A$.
...
Pour $\overline{K}/L/K$ finie séparable, soit $B \subset L$ la
clôture intégrale de $A$ dans $L$. C'est un anneau de
valuation discrète d'après
Compléments sur l'algèbre, remarque \ref{more-algebra-remark-finite-separable-extension}.

\medskip\noindent
Soit $X \to \Spec(A)$ comme dans le paragraphe précédent.
Pour montrer l'injectivité de l'application de spécialisation,
...
(Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-abhyankar})
nous dit donc que
$$
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-galois-conclusion}.
...
comme ci-dessus ; voir Compléments sur l'algèbre, définition
...
donné dans Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-tame-goes-up}.
\end{proof}
...
Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-characterize-tame}.
\end{example}
...
voir Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-henselization-regular}.
...
D'après Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-pull-root-uniformizer},
...
(Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-Dedekind-etale-extension}).
...
(Compléments sur l'algèbre, lemme \ref{more-algebra-lemma-pull-root-uniformizer})
...
Le schéma $V_0$ est intègre normal et s'envoie dans $\sigma(S)$
par le morphisme $\Spec(B^\wedge) \to X$ et qui s'identifie en fait
