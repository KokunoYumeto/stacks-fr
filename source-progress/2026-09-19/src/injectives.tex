...
Sets, lemme \ref{sets-lemma-map-from-set-lifts}, si la cofinalité de $\beta$
...
More on Algebra, lemme \ref{more-algebra-lemma-injective-abelian}, on voit qu'il
...
Modules on Sites, lemme \ref{sites-modules-lemma-module-quotient-flat}.
...
Categories, lemme \ref{categories-lemma-yoneda}. Il est exact à gauche entre
...
pas tout à fait aussi fort. Son résultat convient toutefois au Stacks Project,
...
si le Stacks Project en a besoin.
...
Categories, remarque \ref{categories-remark-big-categories}. L'exemple à
...
Categories, lemme \ref{categories-lemma-colimits-coproducts-coequalizers}. 
De même, si $\mathcal{A}$ satisfait AB3*, alors elle possède des limites ; voir
Categories, lemme \ref{categories-lemma-limits-products-equalizers}. 
...
Soit $\mathcal{A}$ une catégorie abélienne de Grothendieck.
Soit $F : \mathcal{A}^{opp} \to \textit{Sets}$ un foncteur.
...
la convention du Stacks project). Pour une catégorie abélienne ``grande''
