...
applicable plus loin dans le projet Stacks, en suivant de près Rim
...
$F : \mathcal{C}_\Lambda \to \textit{Sets}$ tel que $F(k)$
...
$\mathcal{C}_\Lambda \to \textit{Sets}$ est {\it pro-représentable}
...
Réciproquement, si un foncteur $F : \mathcal{C}_\Lambda \to \textit{Sets}$
...
$\Mor_\mathcal{C}(U, -): \mathcal{C} \to \textit{Sets}$ ; voir les remarques
...
existe un pseudo-foncteur associé $\mathcal{C} \to \textit{Groupoids}$.
...
Soit $F : \mathcal{C} \to \textit{Groupoids}$ un foncteur. Il
...
$\overline{\mathcal{F}} : \mathcal{C} \to \textit{Sets}$ est un
...
Soit $F: \mathcal{C} \to \textit{Sets}$ un foncteur. Nous pouvons regarder un
...
de la catégorie des foncteurs $\mathcal{C} \to \textit{Sets}$ vers la catégorie
...
Ainsi, si $F : \mathcal{C} \to \textit{Sets}$ est un foncteur, nous notons aussi
...
\textit{Sets}$. Cela définit un foncteur pleinement fidèle de $\mathcal
C^{opp}$ vers la catégorie des foncteurs $\mathcal{C} \to
\textit{Sets}$. Ainsi, si $f : U \to V$ est un morphisme, nous pouvons
...
Soit $F : \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur.
...
\noindent
Remarquons que si $F : \mathcal{C}_\Lambda \to \textit{Sets}$ est
...
On dit qu'un foncteur $F: \mathcal{C}_\Lambda \to \textit{Sets}$ est un
...
Soit $F: \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur. En identifiant
...
$\mathcal{C} \to \textit{Sets}$. Voir les remarques
\ref{remarks-cofibered-groupoids}
...
Soit $G : \widehat{\mathcal{C}}_\Lambda \to \textit{Sets}$ un foncteur qui
...
pro-représentable $U : \mathcal{C}_\Lambda \to \textit{Sets}$ et un morphisme
$U \to \mathcal{F}$.
\end{remark}
...
lisse de foncteurs due à Schlessinger ; cf.\ \cite[Definition 2.2.]{Sch}. En
...
$F, G : \mathcal{C}_\Lambda \to \textit{Sets}$ les foncteurs correspondants ;
...
\to \textit{Sets}$. Dans cette situation, la lissité de la restriction
...
\textit{Sets}$, les conditions (S1) et (S2) s'énoncent ainsi :
\begin{enumerate}
...
foncteur $F : \mathcal{C}_\Lambda \to \textit{Sets}$, cela résulte de la
...
Réciproquement, si un foncteur $\text{Mod}^{fg}_R \to \textit{Sets}$ préserve
...
Soit $L: \text{Mod}^{fg}_R \to \textit{Sets}$, respectivement
$L: \text{Mod}_R \to \textit{Sets}$, un foncteur. Supposons que $L(0)$ soit
...
$L: \text{Mod}^{fg}_R \to \textit{Sets}$. Soit $M$ un $R$-module de type
...
Soient $L_1, L_2: \text{Mod}^{fg}_R \to \textit{Sets}$ des foncteurs qui
...
$F: \mathcal{C} \to \textit{Sets}$ un foncteur. Supposons que $F(R)$ soit un
...
$\text{Mod}^{fg}_R \to \mathcal{C} \to \textit{Sets}$, c'est-à-dire
...
$F : \mathcal{C} \to \textit{Sets}$ un foncteur tel que $F(R)$ soit un
ensemble à un seul élément. L'{\it espace tangent $TF$ de $F$} est $F(R[\epsilon])$.
\end{definition}

\noindent
Lorsque $F : \mathcal{C} \to \textit{Sets}$ satisfait les hypothèses du
...
$F : \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur de prédéformation.
...
$F : \mathcal{C}_\Lambda \to \textit{Sets}$ est un foncteur
...
Soient $F, G: \mathcal{C} \to \textit{Sets}$ des foncteurs satisfaisant les
...
Soit $F: \mathcal{C} \to \textit{Sets}$ un foncteur satisfaisant les
...
$\overline{\mathcal{F}}: \mathcal{C}_\Lambda \to \textit{Sets}$. Il possède
...
Soit $F : \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur de
...
\cite[Theorem 2.11]{Sch}. Plus précisément, pour un foncteur
$F: \mathcal{C}_\Lambda \to \textit{Sets}$, la condition (RS) affirme ceci :
...
Soient $F, G: \mathcal{C}_\Lambda \to \textit{Sets}$ des foncteurs de
...
Soit $F: \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur. Alors $F$ est
...
$\mathit{Aut}(x) : U/\mathcal{C} \to \textit{Sets}$ qui envoie un objet
...
$\mathit{Aut}(x): \mathcal{C}_A \to \textit{Sets}$ satisfait à (RS).
\end{lemma}
...
$\mathit{Aut}(x) : \mathcal{C}_A \to \textit{Sets}$ et à l'élément
...
$\mathit{Aut}(x') : \mathcal{C}_{A'} \to \textit{Sets}$ et à l'élément
$\text{id}_{x_0} \in \mathit{Aut}(x)(k)$. On obtient un foncteur de
prédéformation $F = \mathit{Aut}(x')_{\text{id}_{x_0}}$. D'après les lemmes
\ref{lemma-Aut-functor-RS} et \ref{lemma-localize-RS}, $F$ est un foncteur de
...
quintuplet
$(U, R, s, t, c)$, où $U, R : \mathcal{C} \to \textit{Sets}$ sont des
...
$\mathcal{C} \to \textit{Groupoids}$, et un morphisme de groupoïdes en
foncteurs sur $\mathcal{C}$ équivaut à un morphisme des foncteurs
correspondants $\mathcal{C} \to \textit{Groupoids}$ (où l'on considère
$\textit{Groupoids}$ comme une 1-catégorie). Pour nos
...
le foncteur correspondant $\mathcal{C} \to \textit{Groupoids}$, ou, de manière
...
$\mathcal{C} \to \textit{Groupoids}$. La {\it catégorie quotient cofibrée en
...
\noindent
Les deux lemmes généraux suivants serviront à obtenir des présentations.

\begin{lemma}
\label{lemma-presentation-construction}
Soit $\mathcal{F}$ une catégorie cofibrée en groupoïdes au-dessus d'une
catégorie $\mathcal{C}$. Soit $U : \mathcal{C} \to \textit{Sets}$ un
...
\begin{enumerate}
\item $R : \mathcal{C} \to \textit{Sets}$ est le foncteur
...
présentation de $\mathcal{F}$ par un groupoïde lisse en foncteurs.

\begin{lemma}
\label{lemma-smooth-groupoid-in-functors-construction}
Soit $\mathcal{F}$ une catégorie cofibrée en groupoïdes au-dessus de
$\mathcal{C}_\Lambda$. Soit $U : \mathcal{C}_\Lambda \to \textit{Sets}$ un
...
$$
où $t, s$ sont la première et la seconde projection. Les morphismes $t, s$
...
$U : \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur de déformation. Soit
...
Soit $\mathcal{F}$ une catégorie de déformation. Soit
$U : \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur pro-représentable.
...
$\gamma : \text{Der}_\Lambda(k, k) \to TR$ de (\ref{equation-map}) est
injective, car sa composée avec $TR \to TU$ est injective, d'après le théorème
...
groupoïde lisse pro-représentable en foncteurs sur $\mathcal{C}_\Lambda$ si et
seulement si les conditions suivantes sont vérifiées :
\begin{enumerate}
\item $\mathcal{F}$ est une catégorie de déformation.
\item $\dim_k T\mathcal{F}$ est fini.
\item $\dim_k \text{Inf}(\mathcal{F})$ est fini.
\end{enumerate}
\end{theorem}
...
$\mathcal{F}$ est équivalente à un groupoïde lisse pro-représentable en
foncteurs, les conditions (1), (2) et (3) résultent du lemme
\ref{lemma-deformation-groupoid-quotient}, (1), (2) et (3).
...
$(U, R, s, t, c)$ est un groupoïde lisse en foncteurs et que
...
$\mathcal{C}_\Lambda$ qui admettent une présentation par un groupoïde lisse
pro-représentable en foncteurs. Dans cette section, nous expliquons brièvement
...
lisse pro-représentable « minimale ».

\begin{definition}
\label{definition-minimal-groupoid-in-functors}
Soit $(U, R, s, t, c)$ un groupoïde lisse pro-représentable en foncteurs sur
$\mathcal{C}_\Lambda$.
\begin{enumerate}
...
disparaît lorsque $k' \subset k$ est séparable. En outre, un groupoïde lisse
pro-représentable normalisé en foncteurs est minimal, comme le montre le lemme
...
\noindent
Le quotient d'un groupoïde minimal pro-représentable en foncteurs sur $\mathcal
...
Soit $U: \mathcal{C}_\Lambda \to \textit{Sets}$ un foncteur
...
$R \in \Ob(\widehat{\mathcal{C}}_\Lambda)$, le complété de $\varphi$ donne un
morphisme $\underline{R} \to \underline{R}$. Si $f: R \to R$ est le morphisme
correspondant de $\widehat{\mathcal{C}}_\Lambda$, alors $f$ induit un
...
nœthérien est un isomorphisme (voir Algèbre, lemme
...
Soit $(U, R, s, t, c)$ un groupoïde lisse minimal pro-représentable en
foncteurs sur $\mathcal{C}_\Lambda$. Si $\varphi : [U/R] \to [U/R]$ est une
...
Soient $(U, R, s, t, c)$ et $(U', R', s', t', c')$ des groupoïdes lisses
minimaux pro-représentables en foncteurs sur $\mathcal{C}_\Lambda$. Si
...
groupoïde lisse minimal pro-représentable en foncteurs sur
...
$(U, R, s, t, c)$ est un groupoïde lisse en foncteurs et
...
\noindent
Les présentations par des groupoïdes minimaux pro-représentables en foncteurs
...
$\mathcal{F}$ par des groupoïdes lisses minimaux pro-représentables en
foncteurs $(U, R, s, t, c)$ et $(U', R', s', t', c')$. Alors
...
\begin{enumerate}
\item $\mathcal{F}$ admet une présentation par un groupoïde lisse normalisé
pro-représentable en foncteurs sur $\mathcal{C}_\Lambda$ ;
\item $\mathcal{F}$ admet une présentation par un groupoïde lisse
pro-représentable en foncteurs sur $\mathcal{C}_\Lambda$ ;
\item $\mathcal{F}$ admet une présentation par un groupoïde lisse minimal
pro-représentable en foncteurs sur $\mathcal{C}_\Lambda$ ;
...
De plus, les groupoïdes lisses minimaux pro-représentables en foncteurs qui
...
détails. Puisque ce diagramme est une somme amalgamée dans la catégorie
...
démonstration, $\mathcal{F}$ admet une présentation par un groupoïde lisse
pro-représentable
$$
(\underline{R}, \underline{R_1}, s, t, c, e, i)|_{\mathcal{C}_\Lambda}
$$
en foncteurs sur $\mathcal{C}_\lambda$ telle que $\mathcal{F}$. Alors les
...
$h(\mathfrak p)R[[t_1, \ldots, t_r]]$ est premier minimal puisque $h$ est
...
anneau nœthérien et soit $\Lambda \to k$ un homomorphisme fini, où $k$ est un
...
Soit $\Lambda$ un anneau nœthérien, et soient $\Lambda \to k \to l$ des
...
\ref{lemma-S2-extensions}, la condition (S2) pour $\mathcal{F}$ vaut aussi pour
ce diagramme. Ainsi, (S2) vaut pour $\mathcal{F}_{l/k}$.
...
Il existe alors un isomorphisme canonique d'espaces vectoriels sur $l$
$$
...
$T\mathcal{F}$, respectivement $T\mathcal{F}_{l/k}$, possède une structure
canonique d'espace vectoriel sur $k$, respectivement sur $l$ ; voir le lemme
...
$$
la dernière égalité résultant du lemme
\ref{lemma-tangent-space-vector-space}. Plus généralement, étant donné un
espace vectoriel de dimension finie sur $l$, disons $V$, on a
$$
...
ou l'autre foncteur $l$-linéaire. Les isomorphismes sont donc compatibles aux
...
Avec les notations et les hypothèses de la situation
\ref{situation-change-of-fields}. Supposons que $\mathcal{F}$ soit une
catégorie de déformation. Il existe alors un isomorphisme canonique d'espaces
vectoriels sur $l$
$$
...
$$
d'espaces d'automorphismes infinitésimaux.
\end{lemma}
...
$\mathfrak m$.
Prenons pour $S$ le complété $\mathfrak m$-adique de l'anneau nœthérien
...
$R \to R[x_1, \ldots, x_n]$ est formellement lisse. (Comparer à la
...
envoyé sur la composée $R \to S \to B$, dont l'image est contenue dans la
...
rectangle extérieur. L'existence de ce morphisme est garantie par le choix de
$R \to S$ formellement lisse pour la topologie $\mathfrak m_S$-adique ; voir
