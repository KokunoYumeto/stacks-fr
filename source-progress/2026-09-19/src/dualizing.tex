...
$$
{\tolerance=9999\emergencystretch=3em pour certains $n, m \geq 0$. Alors
...
établit donc la dernière assertion du lemme.\par}
...
\medskip\noindent
{\tolerance=9999\emergencystretch=3em Si $\text{profondeur}_I(M) > 0$, choisissons $x \in I$ qui soit un non-diviseur de zéro sur $M$.
...
Le résultat découle donc de l'hypothèse de récurrence.\par}
\end{proof}
...
{\tolerance=9999\emergencystretch=3em Soit $A \to B$ un homomorphisme d'anneaux. Le foncteur $R\Hom(B, -)$
...
{\tolerance=9999\emergencystretch=3em Soient $A \to B \to C$ des homomorphismes d'anneaux. Alors
$R\Hom(C, -) \circ R\Hom(B, -) : D(A) \to D(C)$
est le foncteur $R\Hom(C, -) : D(A) \to D(C)$.\par}
\end{lemma}
...
$$
{\tolerance=9999\emergencystretch=3em où $\varphi_* : D(B) \to D(A)$ est la restriction. En particulier,
$R^q\Hom(B, K) = \Ext_A^q(B, K)$.\par}
\end{lemma}
...
\begin{proof}
D'après les lemmes \ref{lemma-dualizing} et \ref{lemma-equivalence-comes-from-invertible}, le foncteur
\begin{align*}
K \mapsto {} & R\Hom_A(R\Hom_A(K, \omega_A^\bullet), \\
& \qquad (\omega_A')^\bullet)
\end{align*}
...
&= \Hom_{D(A)}(R\Hom(B, \omega_A^\bullet), \omega_A^\bullet[n]) \\
&= 0
\end{align*}
