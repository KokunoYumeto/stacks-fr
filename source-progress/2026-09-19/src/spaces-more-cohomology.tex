...
$\pi_X : (\textit{Spaces}/X)_\etale \to X_{spaces, \etale}$ et
$i_X : \Sh(X_\etale) \to \Sh((\textit{Spaces}/X)_\etale)$
avec $\pi_X \circ i_X = \text{id}$ comme morphismes de topos et
$\pi_{X, *} = i_X^{-1}$.
Plus généralement, si $f : Y \to X$ est un objet de $(\textit{Spaces}/X)_\etale$,
...
$\mathcal{F}$ sur $(\textit{Spaces}/X)_\etale$. En particulier, $i_X^*$ et
...
$$
pour $f : Y \to X$ dans $(\textit{Spaces}/X)_\etale$.
...
Soit $S$ un schéma.
Soit $Y \to X$ un morphisme de $(\textit{Spaces}/S)_\etale$.
\begin{enumerate}
\item Si $\mathcal{I}$ est injectif dans
$\textit{Ab}((\textit{Spaces}/X)_\etale)$, alors
\begin{enumerate}
...
dans $\textit{Ab}((\textit{Spaces}/X)_\etale)$, alors
\begin{enumerate}
...
\end{equation}
pour tout faisceau $\mathcal{F}$ sur $(\textit{Spaces}/Y)_\etale$; si
...
Soit $f : Y \to X$ un morphisme d'espaces algébriques sur $S$.
\begin{enumerate}
\item Pour $K$ dans $D((\textit{Spaces}/Y)_\etale)$, on a
$
(Rf_{big, *}K)|_{X_\etale} = Rf_{small, *}(K|_{Y_\etale})
$
dans $D(X_\etale)$.
\item Pour $K$ dans $D((\textit{Spaces}/Y)_\etale, \mathcal{O})$, on a
$
(Rf_{big, *}K)|_{X_\etale} = Rf_{small, *}(K|_{Y_\etale})
$
...
$(\textit{Spaces}/X)_\etale$. Considérons le produit fibré
$$
\xymatrix{
...
Alors
\begin{enumerate}
\item[(3)] Pour $K$ dans $D((\textit{Spaces}/Y)_\etale)$, on a
$i_g^{-1}(Rf_{big, *}K) = Rf'_{small, *}(i_{g'}^{-1}K)$
dans $D(X'_\etale)$.
\item[(4)] Pour $K$ dans $D((\textit{Spaces}/Y)_\etale, \mathcal{O})$, on a
$i_g^*(Rf_{big, *}K) = Rf'_{small, *}(i_{g'}^*K)$
dans $D(\textit{Mod}(X'_\etale, \mathcal{O}_{X'}))$.
\item[(5)] Pour $K$ dans $D((\textit{Spaces}/Y)_\etale)$, on a
$g_{big}^{-1}(Rf_{big, *}K) = Rf'_{big, *}((g'_{big})^{-1}K)$
dans $D((\textit{Spaces}/X')_\etale)$.
\item[(6)] Pour $K$ dans $D((\textit{Spaces}/Y)_\etale, \mathcal{O})$, on a
...
$(\textit{Spaces}/X)_\etale$. Rappelons que $H^n_\etale(U, \mathcal{H})$
désigne la cohomologie de $\mathcal{H}$ sur un objet
$U$ de $(\textit{Spaces}/X)_\etale$.
...
\item Pour $M$ dans $D((\textit{Spaces}/X)_\etale)$, on a
$H^n_\etale(Y, M) = H^n(Y_\etale, i_f^{-1}M)$.
\item Pour $M$ dans $D((\textit{Spaces}/X)_\etale, \mathcal{O})$, on a
...
$\pi_X : \Sh((\textit{Spaces}/X)_\etale) \to \Sh(X_\etale)$ est défini ci-dessus.
\end{lemma}
...
comme foncteurs $\Sh(Y_\etale) \to \Sh((\textit{Spaces}/X)_\etale)$,
...
Soit $g : X' \to X$ un objet de $(\textit{Spaces}/X)_\etale$.
...
\medskip\noindent
Soient $S$ un schéma et $X$ un espace algébrique sur $S$.
Sur la catégorie $\textit{Spaces}/X$, considérons les topologies fppf
...
X_{spaces, \etale}
$$
Si $\mathcal{H}$ est un faisceau abélien sur $(\textit{Spaces}/X)_{fppf}$,
nous noterons $H^n_{fppf}(U, \mathcal{H})$ la cohomologie de
$\mathcal{H}$ sur un objet $U$ de $(\textit{Spaces}/X)_{fppf}$.
...
\medskip\noindent
Pour un faisceau abélien $\mathcal{H}$ sur $(\textit{Spaces}/X)_{fppf}$,
l'image directe supérieure $R^p\epsilon_{X, *}\mathcal{H}$ est le faisceau
associé au préfaisceau $U \mapsto H^p_{fppf}(U, \mathcal{H})$
sur $(\textit{Spaces}/X)_\etale$. Voir Cohomologie sur les sites, lemme
\ref{sites-cohomology-lemma-higher-direct-images}.
Comme tout objet de $(\textit{Spaces}/X)_\etale$ admet un recouvrement par des
...
$a_X : \Sh((\textit{Spaces}/X)_{fppf}) \to \Sh(X_\etale)$ est défini ci-dessus.
\end{lemma}
...
Soient $S$ un schéma et $X$ un espace algébrique sur $S$.
Avec $a_X : \Sh((\textit{Spaces}/X)_{fppf}) \to \Sh(X_\etale)$
défini ci-dessus:
\begin{enumerate}
...
$D^+((\textit{Spaces}/Y)_{fppf})$, telle que $R\epsilon_{Y, *}\alpha$ soit un
...
\item $\mathcal{F}^a = \pi_X^*\mathcal{F}$ sur $(\textit{Spaces}/X)_\etale$,
\item $\mathcal{F}^a = a_X^*\mathcal{F}$ sur $(\textit{Spaces}/X)_{fppf}$,
...
recouvrement fpqc dans $(\textit{Spaces}/X)_\etale$. Notons $g : U \to X$ le
...
trois sites
$(\textit{Spaces}/X)_{fppf}$, $(\textit{Spaces}/X)_\etale$ et $X_\etale$.
...
$(\Sch/X)_\etale \subset (\textit{Spaces}/X)_\etale$ et
...
\begin{proof}
Pour tout objet $f : U \to X$ de $(\textit{Spaces}/X)_\etale$, considérons la
...
la classe des objets affines de $(\textit{Spaces}/X)_{fppf}$ et soit
...
les objets affines de $(\textit{Spaces}/X)_\etale$, d'après le lemme
...
\medskip\noindent
Soient $S$ un schéma et $X$ un espace algébrique sur $S$.
Sur la catégorie $\textit{Spaces}/X$, considérons les topologies ph et étale.
...
X_{spaces, \etale}
$$
Si $\mathcal{H}$ est un faisceau abélien sur $(\textit{Spaces}/X)_{ph}$,
nous noterons $H^n_{ph}(U, \mathcal{H})$ la cohomologie de
$\mathcal{H}$ sur un objet $U$ de $(\textit{Spaces}/X)_{ph}$.
...
\medskip\noindent
Pour un faisceau abélien $\mathcal{H}$ sur $(\textit{Spaces}/X)_{ph}$,
l'image directe supérieure $R^p\epsilon_{X, *}\mathcal{H}$ est le faisceau
...
$$
sont des isomorphismes, où
$a_X : \Sh((\textit{Spaces}/X)_{ph}) \to \Sh(X_\etale)$ est défini ci-dessus.
\end{lemma}
...
Soient $S$ un schéma et $X$ un espace algébrique sur $S$.
Avec $a_X : \Sh((\textit{Spaces}/X)_{ph}) \to \Sh(X_\etale)$
défini ci-dessus:
\begin{enumerate}
...
cohomologie de torsion et $M$ dans $D^+((\textit{Spaces}/Y)_{ph})$.
