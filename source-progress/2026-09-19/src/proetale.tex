...
belle th\'eorie des faisceaux pervers. Dans un article intitul\'e ``Continuous
\'Etale Cohomology'' (\cite{Jannsen}), Uwe Jannsen examine une variante importante
...
$Y$ s'envoie bijectivement sur $T$, car $X_0 \to \pi_0(X)$ est bijective
...
(2). Par construction, $S^{-1}A$ s'envoie surjectivement sur
...
ensembles de points ferm\'es. Par hypoth\`ese, $Y_0$ s'envoie dans $X_0$ et
...
homomorphisme d'anneaux fidèlement plat et faiblement \'etale $A \to B$ possède une rétraction.
\end{definition}
...
fidèlement plat et ind-\'etale $A \to B$ possède une rétraction.
...
\item tout homomorphisme d'anneaux fidèlement plat et ind-\'etale $A \to B$ possède
une rétraction, et
...
$X' \to X$ possède une section continue $\sigma$. Alors
...
$$
Tout point de $Z \subset \Spec(A_Z^\sim)$ s'envoie dans la composante
...
lemme de Topologies \ref{topologies-lemma-recognize-fpqc-covering}.
\end{proof}

\begin{lemma}
\label{lemma-etale-proetale}
Tout recouvrement \'etale et tout recouvrement de Zariski est un recouvrement pro-\'etale.
\end{lemma}
...
\begin{proof}
Cela résulte du fait que les composées et les changements de base
...
de $\{T_i \to T\}_{i \in I}$ et dans lequel chaque $U_j$ est un schéma
...
\noindent
Nous définissons donc comme suit les recouvrements standards correspondants des schémas affines.
...
Sites, section \ref{topologies-section-procedure},
...
Un {\it grand site pro-\'etale} est un site $\Sch_\proetale$ quelconque, comme dans
Sites, définition \ref{sites-definition-site}, construit comme suit:
\begin{enumerate}
\item Choisir un ensemble quelconque de schémas $S_0$ et un ensemble de recouvrements pro-\'etales
...
\noindent
Voir les remarques qui suivent la
définition de Topologies \ref{topologies-definition-big-zariski-site}
pour la motivation et les explications relatives à la définition des grands sites.

\medskip\noindent
Il s'avérera, voir le lemme \ref{lemma-proetale-induced}, que la
...
il suffit de montrer que la collection des recouvrements pro-\'etales standards
...
Il faut donc vérifier les hypothèses (1) -- (5) du
lemme de Sites \ref{sites-lemma-equivalence}.
...
fait que tout schéma possède un recouvrement ouvert affine.
\end{proof}
...
commute aux produits fibrés. Le lemme résulte donc des
lemmes de Sites \ref{sites-lemma-when-shriek}
...
satisfait les hypothèses du lemme de Sites \ref{sites-lemma-bigger-site}
...
Le lemme découle du lemme de Sites \ref{sites-lemma-bigger-site}.
\end{proof}
...
lemme \ref{lemma-put-in-T}). Par conséquent, les
lemmes de Sites \ref{sites-lemma-when-shriek} et
...
le foncteur $v$ est un adjoint à droite, car, pour $U/T$ et $V/S$,
on a $\Mor_S(u(U), V) = \Mor_T(U, V \times_S T)$,
comme voulu. On peut donc appliquer les
lemmes de Sites \ref{sites-lemma-have-functor-other-way} et
...
De plus, $S_\proetale$ et $T_\proetale$ possèdent tous deux des objets finaux,
à savoir $S/S$ et $T/T$, et $u(S/S) = T/T$. D'après la
proposition de Sites \ref{sites-proposition-get-morphism},
le foncteur $u$ correspond donc à un morphisme de sites
$T_\proetale \to S_\proetale$. Celui-ci donne à son tour le
morphisme de topos; voir le
lemme de Sites \ref{sites-lemma-morphism-sites-topoi}. La description
...
on a, pour $\mathcal{F}$ faisceau sur le grand site pro-\'etale de $T$,
\begin{equation}
...
envoient tous deux un faisceau $\mathcal{F}$ sur $(\Sch/T)_\proetale$ sur le faisceau
$U' \mapsto \mathcal{F}(U' \times_{S'} T')$ sur $S'_\proetale$
(utiliser les lemmes \ref{lemma-put-in-T} et \ref{lemma-morphism-big}).
La seconde égalité se démontre de la même manière ou peut se
déduire du très général
lemme de Sites \ref{sites-lemma-localize-morphism}.
\end{proof}

\noindent
On peut considérer un faisceau sur le grand site pro-\'etale de $S$ comme une collection
...
lemme de Topologies \ref{topologies-lemma-characterize-sheaf-big-etale}.
\end{proof}

\begin{lemma}
\label{lemma-alternative}
Soit $S$ un schéma. Notons $S_{affine, \proetale}$ la sous-catégorie pleine
de $S_\proetale$ formée des objets affines. Les recouvrements de
$S_{affine, \proetale}$ seront les recouvrements pro-\'etales standards; voir la
...
Mais cela découle aussi immédiatement du
lemme de Sites \ref{sites-lemma-equivalence},
...
$\mathcal{O}(S) \to \mathcal{O}(U)$ soit ind-\'etale. Les recouvrements de
$S_{app}$ seront les recouvrements pro-\'etales standards; voir la
définition \ref{definition-standard-proetale}.
Alors la restriction
$$
...
$S_{affine, \proetale}$.
Le lemme découle du lemme de Sites \ref{sites-lemma-equivalence},
en vérifiant que le foncteur d'inclusion $S_{app} \to S_{affine, \proetale}$
est un foncteur spécial cocontinu; voir
Sites, définition \ref{sites-definition-special-cocontinuous-functor}.
Les conditions du lemme de Sites \ref{sites-lemma-equivalence}
découlent immédiatement de la définition et des faits suivants:
(a) tout objet $U$ de $S_{affine, \proetale}$ possède un recouvrement
...
\begin{proof}
Combiner le lemme \ref{lemma-recognize-proetale-covering} et le
lemme de Descente \ref{descent-lemma-fpqc-universal-effective-epimorphisms}.
\end{proof}
...
contiennent beaucoup d'objets faiblement contractiles. En fait, la démonstration du
...
et des $t_j \in \mathcal{F}(U_j)$ qui s'envoient sur $s|_{U_j}$.
...
est une section qui s'envoie sur $s$.
...
et le calcul de la taille des schémas dans
...
Ainsi, $A \to D$ possède une rétraction $\sigma : D \to A$.
Maintenant, si $A \to B$ est fidèlement plat et faiblement \'etale, alors
...
Choisissons des homomorphismes d'anneaux ind-\'etales et fidèlement plats $A_i \to D_i$
...
et, plus précisément, par l'application du
lemme d'Ensembles \ref{sets-lemma-coverings-site} à la dernière étape.
...
de $\Sch_\proetale$ dans la définition \ref{definition-big-proetale-site}
et, plus précisément, par l'application du
lemme d'Ensembles \ref{sets-lemma-construct-category} à l'étape (3).
Puisque $|A_i| \leq \text{size}(U_i) \leq \text{size}(T)$
d'après les lemmes d'Ensembles \ref{sets-lemma-bound-affine} et
...
qui soit la réunion disjointe d'objets faiblement contractiles $V_j$.
Puisqu'une décomposition en réunion disjointe est un recouvrement pro-\'etale,
...
Sites, définition \ref{sites-definition-w-contractible}.
Ainsi, $\mathcal{F}(V) \to \mathcal{G}(V)$ est surjectif comme produit
de morphismes surjectifs d'ensembles, et l'on conclut que $V$ est faiblement contractile.
...
(pour vérifier qu'il s'agit d'un recouvrement, utiliser le
...
\medskip\noindent
Dans ce paragraphe, que nous invitons vivement le lecteur à sauter, nous traitons
les problèmes ensemblistes. Pour savoir que la réunion disjointe
...
et du lemme d'Ensembles \ref{sets-lemma-bound-affine}.
Ainsi, $\text{size}(\coprod_{i \in I} U_i)) \leq Bound(\text{size}(U))$
d'après le lemme d'Ensembles \ref{sets-lemma-bound-size}.
Par construction du grand site pro-\'etale au moyen du
lemme d'Ensembles \ref{sets-lemma-construct-category},
...
$K$ de $U$ dans $X_\proetale$ tel que chaque terme $K_n$ consiste en un
unique objet faiblement contractile de $X_\proetale$ recouvrant $U$.
...
terme $K_n$ consiste en un unique objet affine et faiblement contractile de
$X_\proetale$ recouvrant $U$.
\end{enumerate}
\end{lemma}

\begin{proof}
Soit $\mathcal{B} \subset \Ob(X_\proetale)$ l'ensemble des objets faiblement contractiles
de $X_\proetale$. Tout objet $T$ de $X_\proetale$ possède un
recouvrement $\{T_i \to T\}_{i \in I}$ avec $I$ fini et $T_i \in \mathcal{B}$ d'après le
lemme \ref{lemma-weakly-contractible-cover}.
D'après le lemme d'Hyperrecouvrements \ref{hypercovering-lemma-w-contractible},
...
Cela est permis par la
remarque d'Hyperrecouvrements \ref{hypercovering-remark-take-unions-hypercovering-X}.
...
Alors $X_{qcqs, \proetale}$ est un site, possède des produits fibrés, et
...
d'après le lemme d'Hyperrecouvrements
...
lemme \ref{lemma-get-many-weakly-contractible}
et le fait qu'une réunion finie de schémas affines est affine,
pour tout objet $U$ de $X_{qcqs, \proetale}$, il existe un recouvrement
$\{V \to U\}$ de $X_{qcqs, \proetale}$ avec $V \in \mathcal{B}$.
D'après le lemme d'Hyperrecouvrements \ref{hypercovering-lemma-w-contractible},
on obtient un hyperrecouvrement $K$ de $U$ tel que $K_n = \{U_{n, i}\}_{i \in I_n}$,
...
$U_n = \coprod_{i \in I_n} U_{n, i}$. Cela est permis par la
remarque d'Hyperrecouvrements \ref{hypercovering-remark-take-unions-hypercovering-X}.
\end{proof}
...
abélien sur $X_\proetale$, alors la suite spectrale du
lemme d'Hyperrecouvrements \ref{hypercovering-lemma-cech-spectral-sequence}
implique que
$$
...
est un quasi-isomorphisme, car prendre les sections sur $U_n$ est exact.
Le morphisme affiché est donc un quasi-isomorphisme d'après l'une des suites
spectrales du
lemme d'Homologie \ref{homology-lemma-first-quadrant-ss}.
...
Pour le voir, choisissons un hyperrecouvrement $K$ de $X$ avec $K_n = \{U_n \to X\}$,
...
représentant $E_i$ avec $\mathcal{E}_i^p = 0$ pour $p < N$.
La somme directe terme à terme $\bigoplus \mathcal{E}_i^\bullet$ représente
$\bigoplus E_i$ dans $D(X_\proetale)$; voir le
lemme d'Injectifs \ref{injectives-lemma-derived-products}.
...
$$
d'après le lemme de Modules sur les sites
...
Soit $X$ un schéma. Puisque $X_\proetale$ possède suffisamment d'objets faiblement contractiles,
pour tout $K$ dans $D(X_\proetale)$, on a $K = R\lim \tau_{\geq -n}K$
d'après la
proposition de Cohomologie sur les sites
\ref{sites-cohomology-proposition-enough-weakly-contractibles}.
Puisque $R\Gamma$ commute à $R\lim$ d'après le
lemme d'Injectifs \ref{injectives-lemma-RF-commutes-with-Rlim},
on voit que
$$
R\Gamma(X, K) = R\lim R\Gamma(X, \tau_{\geq -n}K)
$$
dans $D(\textit{Ab})$. Cela nous permettra parfois d'étendre des résultats des
complexes bornés inférieurement à tous les complexes.
\end{remark}
...
\item $D((\Sch/S)_\proetale, \Lambda)$ est compactement engendrée, et
...
\begin{proof}
Démonstration de (3). Soit $U$ un objet affine de $S_\proetale$ qui soit
faiblement contractile. Alors $j_{U!}\mathcal{A}_U$ est un objet compact
de la catégorie dérivée $D(S_\proetale, \mathcal{A})$; voir le
lemme de Cohomologie sur les sites
\ref{sites-cohomology-lemma-quasi-compact-weakly-contractible-compact}.
Choisissons un ensemble $I$ et, pour chaque $i \in I$, un objet affine et faiblement contractile
$U_i$ de $S_\proetale$ tel que tout objet affine et faiblement contractile
...
section $s \in \mathcal{K}^n(T)$ avec $\text{d}(s) = 0$.
...
\medskip\noindent
Exactement le même argument s'applique au grand site pro-\'etale de $S$.
\end{proof}
...
qui transforme les réunions disjointes finies en produits. Alors
...
est le préfaisceau qui envoie un objet $U$ de $X_\proetale$ sur
$\colim H^0(\mathcal{U}, \mathcal{F})$, où la colimite est prise
sur tous les recouvrements pro-\'etales $\mathcal{U}$ de $U$.
Il suffit donc de démontrer que (a) $\mathcal{F}^+$ transforme les réunions
disjointes finies en produits et (b) envoie $W$ sur $\mathcal{F}(W)$.
...
$$
car $\mathcal{F}$ transforme les réunions disjointes finies en produits (cela comprend
la condition selon laquelle $\mathcal{F}$ envoie le schéma vide sur le singleton).
Cela démontre (a).
Enfin, tout recouvrement pro-\'etale de $W$ peut être raffiné par une décomposition
en réunion disjointe finie $W = W_1 \amalg \ldots W_n$ d'après le
...
\ref{sites-section-continuous-functors}; nous avons omis dans la notation
...
il suffit de démontrer que $u_p\mathcal{F}$ transforme les réunions disjointes finies
en produits.
Supposons que $U = U_1 \amalg U_2$ soit une réunion disjointe de sous-schémas
...
Le foncteur correspondant sur les catégories opposées est donc cofinal, et
le lemme de Catégories \ref{categories-lemma-cofinal} montre
que $u_p\mathcal{F}$ évalué en $U$ est la colimite des valeurs
$\mathcal{F}(V_1 \amalg V_2)$ sur la catégorie produit.
Puisque $\mathcal{F}$ est un faisceau, il transforme les réunions disjointes en produits, et
...
signifie ce qui suit. Étant donné un faisceau $\mathcal{G}$ sur $T_\proetale$ et des
sections $s_i \in \Gamma(T_i, g_{i, small}^{-1}\mathcal{G})$ dont les images inverses
...
Choisissons un morphisme surjectif faiblement \'etale $W \to T'$ avec
$W$ affine et faiblement contractile; voir le
...
Choisissons un morphisme surjectif faiblement \'etale $V \to W \times_T W$
avec $V$ affine et faiblement contractile.
Notons $a, b : V \to W$ les morphismes induits.
Puisque $a^*(s'|_W) = b^*(s'|_W)$ et que la catégorie
$\{V \to U, U \in T_\proetale\}$ est cofiltrante
(cela est clair, mais voir le
lemme de Sites \ref{sites-lemma-directed-morphism} en cas de doute),
...
soit la composée de ce morphisme avec le morphisme structural $W \to T$
...
le morphisme $s$ s'envoie sur $s'$ après image inverse sur $T'$.
\end{proof}
...
le foncteur image inverse $i_S^{-1}$ correspondant à
le morphisme de topos $i_S : \Sh(S_\proetale) \to \Sh((\Sch/S)_\proetale)$
...
\begin{proof}
Démonstration de (1)(a) et (2)(a): $i_f^{-1}$ est un adjoint à droite d'un
foncteur exact $i_{f, !}$. En effet, rappelons que $i_f$ correspond
...
lemme \ref{lemma-put-in-T} et sa démonstration. On obtient donc $i_{f, !}$ par le
lemme de Modules sur les sites \ref{sites-modules-lemma-g-shriek-adjoint}.
Le lemme de Modules sur les sites
\ref{sites-modules-lemma-exactness-lower-shriek} montre que ce foncteur est exact.
On conclut alors que (1)(a) et (2)(a) sont vérifiées d'après le
lemme d'Homologie \ref{homology-lemma-adjoint-preserve-injectives} et le
lemme de Catégories dérivées \ref{derived-lemma-adjoint-preserve-K-injectives}.
...
en choisissant un complexe K-injectif de faisceaux abéliens représentant $K$.
La troisième égalité découle de même des
lemmes de Cohomologie sur les sites \ref{sites-cohomology-lemma-cohomology-of-open} et
...
et développons les définitions. La deuxième égalité en découle,
...
\begin{proof}
Cela est vrai parce que $\pi_S^{-1}$ et $\pi_{S, *} = i_S^{-1}$ sont tous deux
des foncteurs exacts et que la composée $\pi_{S, *} \circ \pi_S^{-1}$
...
géométrique. Nous définissons un {\it voisinage pro-\'etale} de $\overline{s}$
comme un diagramme commutatif
$$
\xymatrix{
...
\begin{proof}
La démonstration est identique à celle du
lemme de Cohomologie \'Etale \ref{etale-cohomology-lemma-cofinal-etale},
mais en utilisant les faits correspondants sur les morphismes faiblement \'etales
démontrés dans
Compléments sur les morphismes, lemmes
...
géométrique $\overline{s}_i$ de $S_i$ s'envoyant sur $\overline{s}$.
\end{lemma}
...
de $\overline{s}$ avec $U \in \Ob(S_\proetale)$. Il résulte des
deux lemmes précédents que le foncteur
$$
S_\proetale \textit{Sets},\quad
U \longmapsto
\{\overline{u}\text{ point géométrique de }U\text{ s'envoyant sur }\overline{s}\}
$$
...
\noindent
Contrairement à la situation du topos \'etale de $S$, il n'est pas vrai
...
Ainsi, $\mathcal{F}$ est un faisceau abélien non nul dont la fibre au
point géométrique unique de $S$ est nulle. Puisque nous savons que $S_\proetale$
...
faiblement contractiles.
Premièrement, il y a suffisamment d'objets affines faiblement contractiles d'après le
lemme \ref{lemma-proetale-enough-w-contractible}.
Deuxièmement, si $W \in \Ob(S_\proetale)$ est affine et faiblement contractile,
alors le foncteur
$$
\Sh(S_\proetale) \longrightarrow \textit{Sets},\quad
\mathcal{F} \longmapsto \mathcal{F}(W)
$$
est un foncteur exact $\Sh(S_\proetale) \to \textit{Sets}$ qui commute
...
commute donc à toutes les limites et colimites. De plus, on peut vérifier l'exactitude d'un
...
\medskip\noindent
Une dernière remarque est que le foncteur $\mathcal{F} \mapsto \mathcal{F}(W)$,
pour $W$ affine et faiblement contractile, n'est en général pas le foncteur fibre d'un
...
\noindent
Soit $X$ un schéma. Avec des choix convenables de sites\footnote{Choisir
un grand site pro-\'etale $\Sch_\proetale$ contenant $X$ comme dans la
définition \ref{definition-big-proetale-site}. Puis choisir pour $\Sch_\etale$
le site ayant la même catégorie sous-jacente que $\Sch_\proetale$, mais
dont les recouvrements sont exactement les recouvrements pro-\'etales qui sont aussi des
recouvrements \'etales. Avec ces choix, soient $X_\etale$ et $X_\proetale$
les sous-catégories définies dans la
définition \ref{definition-big-small-proetale} et la
définition de Topologies \ref{topologies-definition-big-small-etale}.
Comparer avec la
remarque de Topologies \ref{topologies-remark-choice-sites}.},
le foncteur $u : X_\etale \to X_\proetale$ qui envoie
$U/X$ sur $U/X$ définit un morphisme de sites
$$
\epsilon : X_\proetale \longrightarrow X_\etale
$$
Cela résulte de la proposition de Sites \ref{sites-proposition-get-morphism}.
...
Ici, $f_\etale : Y_\etale \to X_\etale$ est le morphisme de
petits sites \'etales construit dans
...
d'après le lemme de Cohomologie \'Etale \ref{etale-cohomology-lemma-describe-pullback}.
Soit $a : \Sh(X_\etale) \to \Sh(X_\proetale)$ le foncteur
qui envoie $\mathcal{F}$ sur le faisceau donné par la formule du lemme.
...
Rappelons que $\epsilon_*\mathcal{G}$ est simplement donné par la restriction de
...
\medskip\noindent
Soit $\mathcal{F}$ un objet de $\Sh(X_\etale)$. Il est immédiatement
clair que $\mathcal{F} = \epsilon_*a(\mathcal{F})$.
...
$\epsilon^{-1}$ au lemme \ref{lemma-describe-pullback-from-etale}.
\end{proof}
...
est un cas particulier du théorème de Cohomologie \'Etale
\ref{etale-cohomology-theorem-colimit}.
(Lorsque $X$, $Y$ et les $Y_i$ sont tous affines, voir le
lemme de Cohomologie \'Etale
\ref{etale-cohomology-lemma-directed-colimit-cohomology}, plus facile à lire.)
La dernière assertion en découle immédiatement,
ainsi que du lemme \ref{lemma-fully-faithful}.
\end{proof}
...
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-cech-vanish-collection}
pour le démontrer. Soit $\mathcal{B} \subset \Ob(X_\proetale)$ l'ensemble des objets affines
...
Écrivons d'abord $U_i = \lim_{a \in A_i} U_{i, a}$ comme limite projective filtrante,
...
D'après les lemmes de Limites \ref{limits-lemma-descend-finite-presentation},
...
est un recouvrement \'etale; voir le
lemme de Limites \ref{limits-lemma-descend-surjective}.
Exactement comme ci-dessus, on trouve que
$$
\check{H}^p(\mathcal{U}, \epsilon^{-1}\mathcal{I}) =
...
il en est de même de celui du membre de gauche. Cela démontre la condition (3) du
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-cech-vanish-collection}.
...
à $U \mapsto H^q(U_\proetale, \epsilon^{-1}\mathcal{I})$; voir le
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-higher-direct-images}.
...
$X_\etale$ possède un recouvrement par des objets affines, il s'ensuit que
...
Soit $X$ un schéma. Soit $\mathcal{G}$ un faisceau de groupes (éventuellement
non commutatifs) sur $X_\etale$. On a
$$
H^1(X_\etale, \mathcal{G}) =
H^1(X_\proetale, \epsilon^{-1}\mathcal{G})
$$
où $H^1$ est défini comme l'ensemble des classes d'isomorphisme de
torseurs (voir
...
soit non vide. Cela est possible par la proposition \ref{proposition-weakly-etale}
et parce que les
recouvrements standards pour la topologie pro-\'etale sur $X$ sont cofinaux parmi tous les recouvrements pour la topologie pro-\'etale
...
$$
nous pouvons donc trouver un $i$ et un élément $g_i \in \mathcal{G}(U_i \times_X U_i)$
s'envoyant sur $g$ et satisfaisant la condition de cocycle.
Le cocycle $g_i$ définit alors un torseur sous $\mathcal{G}$ sur
$X_\etale$ dont l'image inverse est isomorphe à $\mathcal{F}$
par construction. Nous omettons certains détails (à savoir le lien
entre les torseurs et les 1-cocycles, qui devrait être ajouté au chapitre
sur la cohomologie des sites).
\end{proof}
...
$$
qui, d'après le lemme \ref{lemma-relative-comparison},
est la même chose que la suite exacte courte
$$
...
\medskip\noindent
L'assertion (2) découle de l'assertion (1) et de Cohomologie sur les sites,
...
sur un site. Si $M$ est un $\Lambda$-module, alors $\underline{M}$ est de
présentation finie en tant que faisceau de $\underline{\Lambda}$-modules si et seulement
si $M$ est un $\Lambda$-module de présentation finie ; voir
...
sur $X_\proetale$ et de présentation finie. Choisissons un recouvrement pour la topologie pro-\'etale
...
Comme les recouvrements pour la topologie pro-\'etale sont des recouvrements fpqc, le
lemme \ref{descent-lemma-open-fpqc-covering} de Descente
montre que $U_M = \bigcup_{i \in I_M} \Im(U_i \to X)$
est un ouvert de $X$. Alors $X = \coprod U_M$ est un recouvrement par des ouverts
...
il s'ensuit que $\mathcal{I}$ admet des sections localement pour la topologie \'etale sur $X$.
Ainsi $\mathcal{F}$ est, localement pour la topologie \'etale, un faisceau constant ; c'est
...
formées des complexes dont les faisceaux de cohomologie sont des faisceaux localement
constants de $\Lambda$-modules de type fini. Alors
$$
\epsilon^{-1} :
...
\begin{proof}
Pour $K_n$, l'assertion (2) résulte du résultat plus général donné dans le
...
et soit $I \subset \Lambda$ un idéal. Rappelons, d'après le
lemme de Modules sur les sites \ref{sites-modules-lemma-completion-flat},
que
$$
...
Soit $\mathcal{C}$ un site. Soit $\Lambda$ un anneau noethérien
et soit $I \subset \Lambda$ un idéal. Le foncteur
d'inclusion
$D_{comp}(\mathcal{C}, \Lambda) \to D(\mathcal{C}, \Lambda)$
considéré dans Géométrie algébrique et formelle, proposition
\ref{algebraization-proposition-derived-completion}, a pour adjoint à gauche le foncteur qui envoie $K$ sur
$$
...
\begin{proof}
Choisissons des générateurs $f_1, \ldots, f_r$ de $I$. D'après le
lemme de Géométrie algébrique et formelle
...
sont isomorphes. Le lemme s'ensuit.
\end{proof}
...
\begin{proof}
Nous avons établi (1) et (2) dans le
lemme de Géométrie algébrique et formelle
...
$$
la première égalité résultant du lemme \ref{lemma-naive-completion},
...
$$
Comme $Lf^*$ commute au produit tensoriel dérivé d'après le
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-pullback-tensor-product}
...
\noindent
Nous poursuivons la discussion de la
...
concernant $D_{comp}(\mathcal{C}, \Lambda)$ soit tout à fait satisfaisante,
il est difficile de donner explicitement des exemples de complexes complets au sens dérivé.
Nous savons que
\begin{enumerate}
\item tout objet $M$ de $D(\mathcal{C}, \Lambda/I^n)$ fournit par restriction un objet
de $D(\mathcal{C}, \Lambda)$ complet au sens dérivé, et
\item pour tout $K \in D(\mathcal{C}, \Lambda)$, la complétion dérivée
$K^\wedge = R\lim (K \otimes_\Lambda^\mathbf{L} \underline{\Lambda/I^n})$
est complète au sens dérivé.
\end{enumerate}
...
En effet, par définition des limites homotopiques, il
...
$D(\mathcal{C}, \Lambda)$. Ils se calculent en prenant les produits
...
$$
du préfaisceau ci-dessus ne s'annule après restriction à aucun schéma \'etale
non vide sur $X$; ainsi, le faisceau associé au préfaisceau n'est pas nul.
...
voir le lemme d'Injectifs \ref{injectives-lemma-RF-commutes-with-Rlim}.
...
$$
Puisque nous pouvons vérifier qu'un morphisme entre objets de
$D(\mathcal{C}, \Lambda)$ est un isomorphisme en l'évaluant en $U \in \mathcal{B}$,
...
pour les modules sur les anneaux; voir les
lemmes de Compléments d'Algèbre \ref{more-algebra-lemma-hom-from-Af} et
\ref{more-algebra-lemma-serre-subcategory}.
De même, (2) se déduit de la
proposition de Compléments d'Algèbre
...
$D_{comp}(X, \Lambda)$ (voir la discussion qui suit la
définition de Géométrie algébrique et formelle
...
$$
en tant qu'espaces profinis, où $G = \text{Gal}(k^{sep}/k)$. En effet, tout
...
La troisième égalité résulte du lemme de Cohomologie \'Etale
\ref{etale-cohomology-lemma-affine-only-closed-points}.
La quatrième égalité utilise
la remarque de Cohomologie \'Etale
...
À ce stade, les lemmes de Cohomologie sur les sites
...
H^i_{cont}(G, \Lambda)
$$
où le membre de droite est la cohomologie continue de Tate; voir la
section de Cohomologie \'Etale
\ref{etale-cohomology-section-continuous-group-cohomology}.
Bien entendu, c'est exactement ce que l'on devait obtenir.
...
on obtient des actions continues de $G$ sur $M_n$. D'après le théorème de Cohomologie \'Etale
...
$$
pour obtenir le faisceau promis dans le lemme. Exactement le même argument que
celui donné dans l'introduction de cette section fournit la comparaison
...
induit un morphisme de sites $f_\proetale : X_\proetale \to Y_\proetale$; voir la
proposition de Sites \ref{sites-proposition-get-morphism}.
...
f_{\proetale, *}\epsilon^{-1}\mathcal{F}$; voir la
section de Sites \ref{sites-section-pullback}.
...
tout objet de $Y_\proetale$ possède un recouvrement par des objets $V = \lim V_i$
...
$$
Voir le lemme \ref{lemma-limit-pullback} pour le membre de gauche.
D'après le lemme \ref{lemma-describe-pullback-from-etale}, on a
$$
\Gamma(X \times_Y V, \epsilon^*\mathcal{F}) =
\Gamma(X \times_Y V, g_\etale^{-1}\mathcal{F})
$$
où $g : X \times_Y V \to X$ est la projection. Le résultat découle donc du
lemme de Cohomologie \'Etale
\ref{etale-cohomology-lemma-directed-colimit-cohomology}.

\medskip\noindent
Démonstration de (2). En raisonnant exactement de la même manière que ci-dessus,
...
$$
est un isomorphisme, ce qui résulte encore une fois du lemme de Cohomologie \'Etale
\ref{etale-cohomology-lemma-directed-colimit-cohomology}.
\end{proof}
...
est affine, disons $X = \Spec(A)$. Alors $Z = \Spec(B)$ pour une certaine
...
faisceaux en l'évaluant sur un tel objet $U = \Spec(D)$. Alors
...
Tout $g \in C$ qui s'envoie dans
$C/IC = \overline{C}$ sur un élément inversible est inversible dans $C$. En effet, on a les
...
$\{u(U_i) \to u(v(V)) = V\}$ est un recouvrement de $Z_{app}$.
...
associé au foncteur cocontinu pleinement fidèle
...
La première assertion du lemme découle des
lemmes de Sites \ref{sites-lemma-have-functor-other-way} et
\ref{sites-lemma-have-functor-other-way-morphism}
...
à $v$, cela résulte de la construction; voir le
lemme de Sites \ref{sites-lemma-cocontinuous-morphism-topoi}.
...
le faisceau associé à $\mathcal{G}$ est $\mathcal{G}^+$; voir le
théorème de Sites \ref{sites-theorem-plus}. Soit ensuite $V$ un objet
...
Puisque $v$ commute aux réunions disjointes finies (en tant qu'adjoint à gauche ou par
construction) et que $\mathcal{F}$ transforme les réunions
...
comparer avec le
lemme de Cohomologie \'Etale \ref{etale-cohomology-lemma-cech-complex}).
...
Il s'ensuit que la valeur de $\mathcal{G}^+ = i^{-1}\mathcal{F}$
en $V$ est simplement $\mathcal{F}(v(V))$, ce qui démontre (2).

\medskip\noindent
Démonstration de (3). Tout objet de $Z_{app}$ possède un recouvrement par des objets
...
$i^{Sh}_!$ découle alors du
lemme de Sites \ref{sites-lemma-existence-lower-shriek}.
...
\medskip\noindent
Les assertions de pleine fidélité de (6) résultent alors du
lemme de Catégories \ref{categories-lemma-adjoint-fully-faithful}.
\end{proof}
...
Alors
$i_{\proetale, *}$ et $i_\proetale^{-1}$ sont des équivalences de catégories
...
le théorème de Cohomologie \'Etale
...
Après avoir remplacé $B$ par un localisé, nous pouvons supposer que $B/IB = A/I$.
...
ensemblistement; voir le lemme d'Algèbre \ref{algebra-lemma-qc-open}.
...
L'égalité résulte de la discussion de la section \ref{section-morphism}.
\end{proof}
...
\begin{proof}
Soit $\{V_i \to V\}$ un recouvrement pro-\'etale. Le lemme est démontré si nous
pouvons raffiner ce recouvrement en un recouvrement dont les membres n'admettent aucun
...
Pour tout $i$, écrivons $V_i = \Spec(A_i)$ et soit $Z_i \subset V_i$ le
...
\begin{proof}
Voir le lemme de Modules sur les sites \ref{sites-modules-lemma-restrict-back}
(et le lemme de Sites \ref{sites-lemma-restrict-back} pour le cas
des faisceaux d'ensembles), ainsi que le
lemme de Catégories \ref{categories-lemma-adjoint-fully-faithful}.
\end{proof}
...
Il suffit de montrer que $X_\proetale$ possède suffisamment d'objets
...
tel que $A$ soit w-contractile (il existe suffisamment d'objets de ce type).
...
Nous obtenons un homomorphisme d'anneaux ind-Zariski fidèlement plat
$$
...
$$
la première égalité d'après le lemme \ref{lemma-jshriek-zero}
et la seconde d'après les
...
et achèvera la démonstration (détails omis).
...
la troisième égalité du lemme \ref{lemma-fully-faithful}, et la
quatrième égalité, à nouveau, du lemme \ref{lemma-jshriek-comparison}.
De même, on a
$$
...
i_*i^{-1}\mathcal{F}
$$
cette fois en utilisant le lemme \ref{lemma-morphism-comparison}.
...
\begin{proof}
Notons que $H^i(K \otimes_\Lambda^\mathbf{L} L)$ s'identifie à
...
peut-être pas standard.} s'il existe un module qui soit
$I$-adiquement complet et qui soit un $\Lambda$-module noté $M$, tel que $M/IM$ soit fini
et que $\mathcal{F}$ soit localement isomorphe à
$$
\underline{M}^\wedge = \lim \underline{M/I^nM}.
$$
\item Nous disons que $\mathcal{F}$ est
{\it constructible adique}\footnote{Cette notation n'est peut-être pas standard.}
...
ni la catégorie des faisceaux constructibles adiques, ni
...
Cependant, le conoyau du morphisme ci-dessus n'est pas constructible adique, car
...
$R$-modules d'après la proposition de Cohomologie \'etale
...
Nous affirmons qu'il s'agit d'une partition comme dans le lemme.
...
constant. Il s'ensuit que chacun des faisceaux
 $I^n\mathcal{F}/I^{n + 1}\mathcal{F}$
est localement constant. Les suites exactes courtes
$$
0 \to I^n\mathcal{F}/I^{n + 1}\mathcal{F} \to
\mathcal{F}/I^{n + 1}\mathcal{F} \to \mathcal{F}/I^n\mathcal{F} \to 0
$$
permettent de conclure par récurrence, en utilisant le lemme de Modules sur les sites
\ref{sites-modules-lemma-kernel-finite-locally-constant}
...
lemme d'Algèbre \ref{algebra-lemma-limit-complete}.
...
nous voyons que $M$ est un $\Lambda^\wedge$-module fini.
...
à $\underline{M_n}$. Il résulte du
lemme de Compléments d'algèbre \ref{more-algebra-lemma-hom-systems-ML}
...
transition sont surjectifs. Puisque $*(X)$ est un singleton (non vide)
...
il existe un $\Lambda/I^n$-module fini $M_n$ tel que
...
pour un certain $\Lambda^\wedge$-module fini $N_t$. Notons que
...
Il se trouve que l'analogue naïf du
lemme de Compléments d'algèbre \ref{more-algebra-lemma-derived-complete-finite}
est faux dans ce contexte. Cependant, voici
l'analogue du lemme de Compléments d'algèbre
...
soit isomorphe au faisceau constant associé à un $\Lambda/I$-module fini
...
possède des faisceaux de cohomologie constructibles et est localement de dimension de Tor finie.
\end{enumerate}
Nous notons $D_{cons}(X, \Lambda)$ la sous-catégorie pleine des objets constructibles
...
lemme \ref{lemma-naive-completion}.
Par définition de la propriété d'être localement de dimension de Tor finie, nous pouvons trouver
...
$X = \coprod U_i$ et, pour chaque $i$, une collection finie de
$\Lambda^\wedge$-modules projectifs de type fini $M^a, \ldots, M^b$
...
Remarquons que chaque $\overline{M}_i$ est un $\Lambda/I$-module fini.
...
Alors $M_i = \Im(p_i)$ est complet pour la topologie $I$-adique et
est un $\Lambda^\wedge$-module fini vérifiant $M_i/IM_i = \overline{M}_i$.
...
$$
est exacte par l'annulation de $\text{Tor}_1$ mentionnée au début de ce
paragraphe ; utilisons aussi le fait que
$\underline{\Lambda}^\wedge/I\overline{\Lambda}^\wedge$ d'après le
lemme \ref{sites-modules-lemma-completion-flat} de Modules sur les sites,
pour voir que
...
soit d'amplitude de Tor contenue dans $[a, b - 1]$ (détails omis).
...
pas standard} s'il existe un complexe fini de
$\Lambda^\wedge$-modules projectifs de type fini $M^\bullet$ tel que
$K$ soit localement isomorphe à
$$
\underline{M^a}^\wedge \to \ldots \to \underline{M^b}^\wedge
$$
\item Nous disons que $K$ est {\it constructible adique}\footnote{Cette notation n'est peut-être
...
Puisque $K \otimes_\Lambda^\mathbf{L} \underline{\Lambda/I}$ est de
...
il existe un complexe fini $M^\bullet$ de modules projectifs de type fini sur
...
On a $K_n = \epsilon^{-1}L_n$ pour un $L_n$ de
...
objet de $D_{cons}(X, \Lambda)$ (détails omis) tel que
...
$D_{cons}(X, \Lambda)$. Alors $K$ est constructible adique
(définition \ref{definition-adic-constructible}).
\end{proposition}
...
R\lim Lg^* Rf_* K_n
$$
la dernière égalité résultant du fait que $\Lambda/I^n$ est un module parfait et de
...
R\lim Rf'_* L(g')^* K_n
$$
la dernière égalité résultant du fait que $Rf'_*$ commute à $R\lim$
(Cohomologie sur les sites, lemme
...
condition, on peut écrire $K_n = \epsilon^{-1}L_n$ pour un
...
\noindent
Nous conseillons au lecteur d'omettre cette section : nous montrons ici que la cohomologie
...
peuvent chacun être raffinés par un recouvrement ouvert fini disjoint
$V_k = V_{k, 1} \amalg \ldots \amalg V_{k, n_k}$, voir le
lemme \ref{lemma-w-contractible-proetale-cover}.
Alors $\{V_{k, i} \to T\}$ est un recouvrement de $\Sch_\proetale$
...
$$
dont la composée est isomorphe à l'identité et tels que $f_* = g^{-1}$.
De plus,
\begin{enumerate}
...
et des objets finals, et que notre foncteur commute à ces constructions
(lemme \ref{lemma-fibre-products-proetale}).
Il résulte du lemme \ref{lemma-proetale-induced}
que le foncteur d'inclusion est cocontinu.
L'existence de $f$ et $g$ résulte donc du
lemme de Sites \ref{sites-lemma-bigger-site}.
L'égalité de (1) est donnée par le
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-cohomology-bigger-site}.
...
les hypothèses du lemme de Sites \ref{sites-lemma-bigger-site}.
...
$(\Sch/S)_\proetale \to (\Sch'/S)_\proetale$ vérifie lui aussi
les hypothèses du lemme de Sites \ref{sites-lemma-bigger-site}
également. On obtient donc de même des morphismes
\begin{eqnarray*}
...
$(\Sch/S)_\proetale \to (\Sch'/S)_\proetale$ vérifie les hypothèses du
lemme de Sites \ref{sites-lemma-bigger-site}. Cela implique (2), et (1)
résulte du
lemme de Cohomologie sur les sites \ref{sites-cohomology-lemma-cohomology-bigger-site}.
\end{proof}
