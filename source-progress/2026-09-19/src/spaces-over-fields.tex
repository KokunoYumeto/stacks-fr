...
\noindent
Nous supposons toujours que tous les schémas sont contenus dans
un grand site fppf $\Sch_{fppf}$. Et tous les anneaux $A$ considérés
...
\noindent
Cette section poursuit la discussion dans
Espaces décents, Section \ref{decent-spaces-section-generically-finite}
et l'analogue, pour les morphismes d'espaces algébriques, de
...
localement noethérien. Soit $y \in |Y|$ un point de codimension $\leq 1$
sur $Y$. Soit $X^0 \subset |X|$ l'ensemble des points de codimension $0$
sur $X$. Supposons de plus que l'une des conditions suivantes soit satisfaite
\begin{enumerate}
...
\item $f$ est quasi-fini en un ensemble dense de points de $|X|$,
...
$$
où $U$, $V$ sont des schémas et où les flèches horizontales sont \'etales
et surjectives. Choisissons $v \in V$ qui s'envoie sur $y$. Observons que
...
La fibre $U_v$ de $U \to V$ au-dessus de $v$ se surjecte sur
...
\medskip\noindent
Dans le cas (4), puisque $|U| \to |X|$ est ouvert, nous voyons que
...
noethérien. Soit $y \in Y$ un point de codimension $\leq 1$ dans $Y$.
Soit $X^0 \subset |X|$ l'ensemble des points de codimension $0$ sur $X$.
...
un point géométrique au-dessus de $y$. Alors $|X_{\overline{y}}|$ est un espace
discret (Espaces décents, Lemme
\ref{decent-spaces-lemma-conditions-on-fibre-and-qf}).
Puisque $X_{\overline{y}}$ est quasi-compact car $f$ est propre, nous concluons
...
\item tout $x \in |X|$ de codimension $1$ sur $X$ tel que l'anneau local
...
\begin{proof}
La partie (1) découle du lemme des Espaces décents,
...
La partie (2) découle de la partie (3) et du lemme \ref{lemma-finite-in-codim-1}
(et du fait que les morphismes finis ont des fibres finies).
Soit $x \in |X|$ comme dans (3). D'après
Cohomologie des espaces, Lemme
\ref{spaces-cohomology-lemma-proper-finite-fibre-finite-in-neighbourhood}
(qui s'applique par le lemme des Espaces décents,
\ref{decent-spaces-lemma-conditions-on-fibre-and-qf})
nous pouvons supposer que $f$ est fini. Choisissons un schéma affine $X'$ et
un morphisme \'etale $X' \to X$ et un point $x' \in X$ dont l'image est $x$.
...
(auquel cas $U$ contient l'image de $U'$ dans $X$, voir Espaces, Lemme
\ref{spaces-lemma-descent-representable-transformations-property}).
Alors $Y \times_X X' \to X$ est un morphisme fini birationnel
(Espaces décents, Lemme \ref{decent-spaces-lemma-birational-etale-localization})
Puisqu'un morphisme fini est affine, nous nous ramenons
...
\noindent
Nous n'avons pas encore défini la notion d'un espace algébrique intègre. Le
problème est que le fait d'être intègre n'est pas une propriété locale pour la topologie \'etale des schémas.
Nous pourrions utiliser la propriété que $X$ est réduit et que $|X|$ est irréductible,
donnée dans Propriétés, Lemme \ref{properties-lemma-characterize-integral}
pour définir les espaces algébriques intègres. Dans ce cas, l'espace algébrique
décrit dans Espaces, Exemple \ref{spaces-example-infinite-product}
serait intègre, ce qui ne semble pas correct.
Pour éviter ce type de pathologie, nous supposerons en outre que $X$ est
un espace algébrique décent, bien qu'il existe peut-être une alternative plus faible.

\begin{definition}
\label{definition-integral-algebraic-space}
Soit $S$ un schéma. Nous disons qu'un espace algébrique $X$ sur $S$ est
{\it intègre} s'il est réduit, décent et si $|X|$ est irréductible.
\end{definition}
...
Il possède donc un unique point générique $x$. En fait, dans
Espaces décents, Lemme
\ref{decent-spaces-lemma-finitely-many-irreducible-components}
nous avons caractérisé les espaces algébriques décents ayant un nombre fini de
composantes irréductibles. En appliquant ce lemme, nous voyons qu'un
espace algébrique $X$ est intègre s'il est
réduit, s'il possède un sous-schéma ouvert dense irréductible $X'$
ayant pour point générique $x'$ et si le morphisme $x' \to X$ est quasi-compact.
...
Il existe des identifications canoniques
$$
R(X) = \mathcal{O}_{X, \eta}^h = \kappa(\eta)
$$
où $R(X)$ est l'anneau des fonctions rationnelles défini dans
Morphismes d'espaces, Définition
...
\begin{proof}
Puisque $X$ est un schéma dans un voisinage ouvert de $\eta$ (voir la discussion
ci-dessus), cela découle immédiatement du résultat correspondant pour
les schémas, voir Morphismes, Lemme
\ref{morphisms-lemma-integral-scheme-rational-functions}.
Nous utilisons également : l'hensélisation d'un corps est le corps lui-même
et nos définitions de ces objets
pour les espaces algébriques sont compatibles avec celles pour les schémas.
Détails omis.
\end{proof}
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre sur $S$.
Alors $\Gamma(X, \mathcal{O}_X)$ est un domaine.
\end{lemma}

\begin{proof}
Posons $R = \Gamma(X, \mathcal{O}_X)$. Si $f, g \in R$ sont non nuls et
$fg = 0$ alors $X = V(f) \cup V(g)$ où $V(f)$ désigne le sous-espace fermé
de $X$ défini par $f$. Comme $X$ est irréductible, on voit que soit
$V(f) = X$ soit $V(g) = X$. Alors soit $f = 0$ soit $g = 0$ d'après
Propriétés des espaces, Lemme \ref{spaces-properties-lemma-reduced-space}.
\end{proof}

\noindent
Voici un lemme concernant les espaces algébriques intègres normaux.

\begin{lemma}
\label{lemma-normal-integral-cover-by-affines}
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre normal sur $S$.
Pour tout $x \in |X|$, il existe un schéma affine intègre normal $U$
et un morphisme \'etale $U \to X$ tels que $x$ appartienne à l'image.
\end{lemma}

\begin{proof}
Choisissons un schéma affine $U$ et un morphisme \'etale $U \to X$ tels que
$x$ appartienne à l'image. Soient $u_i$, $i \in I$, les points génériques des
composantes irréductibles de $U$. Alors chaque $u_i$ s'envoie sur le point générique de $X$
...
Ainsi $A = \prod_{i \in I} A_i$ est un produit de domaines normaux d'après
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre normal sur $S$.
Alors $\Gamma(X, \mathcal{O}_X)$ est un domaine normal.
\end{lemma}

\begin{proof}
Posons $R = \Gamma(X, \mathcal{O}_X)$. Alors $R$ est un domaine d'après
...
Alors il existe un unique $f_U \in \Gamma(U, \mathcal{O}_U)$
tel que $b|_U f_U = a|_U$, puisque
$\Gamma(U, \mathcal{O}_U)$ est un domaine normal d'après
Propriétés, Lemme \ref{properties-lemma-normal-integral-sections}.
D'après l'unicité mentionnée ci-dessus ces $f_U$
se recollent et définissent une section globale $f$ du faisceau
structural, c'est-à-dire de $R$.
\end{proof}
...
\item l'ensemble des sous-ensembles fermés irréductibles de $|X|$,
\item l'ensemble des sous-espaces fermés intègres de $X$.
\end{enumerate}
La bijection de (1) vers (2) envoie $x$ sur $\overline{\{x\}}$.
La bijection de (3) vers (2) envoie $Z$ sur $|Z|$.
\end{lemma}

\begin{proof}
Notre application définit une bijection entre (1) et (2) puisque $|X|$ est
sobre d'après 
Espaces décents, Proposition \ref{decent-spaces-proposition-reasonable-sober}.
Étant donné $T \subset |X|$ fermé et irréductible, il existe un
unique sous-espace fermé réduit $Z \subset X$ tel que
$|Z| = T$, à savoir $Z$ est la structure de sous-espace induite
réduite sur $T$, voir Propriétés des espaces, Définition
...
\item $x$ est le seul point de $|X|$ qui s'envoie sur $y$.
\end{enumerate}
...
\begin{proof}
Par topologie élémentaire, on voit que $f(x) = y$ puisque $f$ est dominant.
...
qui est localement de type fini. On en déduit que (1), (2), (3)
...
D'autre part, la condition (3) implique la condition (4) puisqu'un morphisme
fini est quasi-fini et puisque $x \in U$ car $x$
est le point générique. Ainsi (1) -- (4) sont équivalentes.

\medskip\noindent
Supposons vérifiées les conditions équivalentes (1) -- (4). Supposons que
...
Cela découle d'Espaces décents, Lemme
...
Soit $f : X \to Y$ localement de type fini et dominant.
Supposons vérifiée l'une quelconque des conditions équivalentes (1) -- (5) du
Lemme \ref{lemma-finite-degree}. Soient $x \in |X|$ et $y \in |Y|$
...
Soient $f : X \to Y$ et $g : Y \to Z$ des morphismes dominants localement
de type fini. Supposons vérifiée l'une quelconque des conditions équivalentes
(1) -- (5) du Lemme \ref{lemma-finite-degree} pour $f$ et $g$. Alors
$$
...
au-dessus de $S$. Si $T \subset |X|$ est un sous-ensemble fermé,
...
Le lemme découle donc de la définition de localement fini
(Topologie, définition \ref{topology-definition-locally-finite}).
\end{proof}

\noindent
Soit $S$ un schéma. Soit $X$ un espace algébrique décent au-dessus de $S$.
...
Soit $S$ un schéma.
Soit $X$ un espace algébrique intègre localement noethérien au-dessus de $S$.
\begin{enumerate}
...
n'est peut-être pas un domaine, voir l'exemple \ref{example-not-a-domain}.
...
au-dessus de $S$. Soit $Z \subset X$ un diviseur premier et soit $\xi \in |Z|$
...
Cependant, $\mathcal{O}_{X, \xi}^h$ est l'hensélisé de
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soit $f \in R(X)^*$. Pour tout diviseur premier
...
\noindent
Si $\mathcal{O}_{X, \xi}^h$ est un domaine, nous obtenons
$$
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soit $f \in R(X)^*$. Si le diviseur premier
...
$\mathcal{O}_{X, \xi}^h$ est l'hensélisé de $\mathcal{O}_{X, \xi}$
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soit $f \in R(X)^*$. Alors les familles
$$
...
Soit $S$ un schéma.
Soit $X$ un espace algébrique intègre localement noethérien au-dessus de $S$. Le
...
Ceci est la suite de l'exemple de Morphismes des espaces,
voir l'exemple de Morphismes d'espaces \ref{spaces-morphisms-example-universal-homeomorphism}.
...
$\Delta' : U \to U \times_k U$ est l'application $u \mapsto (u, \sigma(u))$
...
de $X$ en $x$ n'est pas un domaine, car c'est l'hensélisé
...
\medskip\noindent
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soit $\mathcal{L}$ un $\mathcal{O}_X$-module inversible.
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soit $\mathcal{L}$ un
...
$\mathcal{L}$ au-dessus de $U$. Par conséquent, les diviseurs premiers qui peuvent apparaître
...
Soit $S$ un schéma. Soit $X$ un espace algébrique intègre localement noethérien
au-dessus de $S$. Soient $\mathcal{L}$, $\mathcal{N}$ deux modules inversibles
