...
$$
{\emergencystretch=3em soit un complexe. Comme il s'agit d'un cas particulier d'un complexe $2$-périodique,
nous disposons de ses {\it modules de cohomologie} $H^0(M, \varphi, \psi)$,
$H^1(M, \varphi, \psi)$ et d'une notion d'exactitude.\par}
\end{enumerate}
\end{definition}
...
Homologie, Lemme \ref{homology-lemma-cat-chain-abelian}.\par}
...
$$
e_R(M, N, \varphi, \psi) =
\text{long}_R(H^0(M, N, \varphi, \psi))
-
\text{long}_R(H^1(M, N, \varphi, \psi))
$$
...
e_R(M, 0, \psi) = \text{long}_R(\Coker(\psi)) - \text{long}_R(\Ker(\psi))
\end{equation}
...
$e_R(M, N, \varphi, \psi) = \text{long}_R(M) - \text{long}_R(N)$.
...
sont de longueur finie, la quantité « $\text{long}_R(M) - \text{long}_R(N)$ »
...
\begin{proof}
{\emergencystretch=3em Appliquons l'additivité du Lemme \ref{lemma-additivity-periodic-length}
et observons que $(\Ker(f), \varphi, \psi)$ et
$(\Coker(f), \varphi', \psi')$ ont une multiplicité nulle d'après le
Lemme \ref{lemma-finite-periodic-length}.\par}
\end{proof}
...
\text{long}_{R_{\mathfrak q_i}}(M_{\mathfrak q_i}).
$$
\end{lemma}
...
\text{long}_R(M/xM)
=
\sum\nolimits_i \text{long}_R(R/(x, \mathfrak q_i))
\text{long}_{R_{\mathfrak q_i}}(M_{\mathfrak q_i}).
$$
...
=
\sum\nolimits_i \text{long}_R(R/(x, \mathfrak q_i))
\text{long}_{R_{\mathfrak q_i}}((R/I)_{\mathfrak q_i})
$$
...
$$
a_i = \text{long}_R(K_i/t^{n - i}M),\quad
b_i = \text{long}_R(K_i/tK_{i + 1}),\quad
c_i = \text{long}_R(K_1/t^iK_{i + 1})
$$
...
$$
\ell = \min(\text{long}_A(A/a_1A),\ \text{long}_A(A/a_2A)).
$$
...
Choisissons $x \in \{a_1, a_2\}$ tel que $\ell = \text{long}_A(A/xA)$.
D'après le Lemme \ref{lemma-compare-periodic-lengths} 
et (\ref{equation-multiplicity-coker-ker}),
nous avons $\text{long}_A(A/xA) = \text{long}_A(A'/xA')$.
D'autre part,
$$
\text{long}_A(A'/xA') =
\sum [\kappa(\mathfrak m_i) : \kappa(\mathfrak m)]
\text{long}_{A'_i}(A'_i/xA'_i)
$$
...
$\text{long}_A(A/cA) < \ell$, car $cA$ est un idéal strictement
...
\end{equation}
où $m_j = \text{long}_{B_j}(B_j/\pi_j B_j)$ et le produit
...
$m = \text{long}_B(B/\pi B)$ et $k = \text{long}_B(B/\theta B)$,
on obtient $e_2 m = \text{long}_B(B/a_2 B) = f_2 k$.
...
$C_k = C_{\mathfrak m_k}$,
(5) $m = \text{long}_B(B/\pi B)$, et
(6) $m_k = \text{long}_{C_k}(C_k/\pi C_k)$.
...
Premièrement, nous avons $m = \text{long}_B(B/xB) = \text{long}_B(C/\pi C)$
d'après le Lemme \ref{lemma-compare-periodic-lengths} 
et (\ref{equation-multiplicity-coker-ker}).
Enfin, $\text{long}_B(C/\pi C) = \sum m_k[\kappa_k:\kappa]$
d'après Algèbre, Lemme \ref{algebra-lemma-pushdown-module}.
\end{proof}
...
&= (-1)^{e_{1, j}e_{3, j}}u_{1, j}^{e_{3, j}}u_{3, j}^{-e_{1, j}} \\
&\quad (-1)^{e_{2, j}e_{3, j}}u_{2, j}^{e_{3, j}}u_{3, j}^{-e_{2, j}}
\end{aligned}
$$
...
\begin{proof}
{\emergencystretch=2em Choisissons $B_j \subset C_j$ comme dans le
...
au-dessus de $\mathfrak m_j$. Soient\par}
$$
...
\begin{proof}
{\emergencystretch=3em Si $a_1, a_2$ sont tous deux des unités, alors $\partial_A(a_1, a_2) = 1$,
...
\bmod \mathfrak m_j)^{m_j}
$$
où $m_j = \text{long}_{B_j}(B_j/\pi_j B_j)$.
...
$$
où $m'_{j, l} = \text{long}_{B'_{j, l}}(B'_{j, l}/\pi_j B'_{j, l})$.
...
$$
{\emergencystretch=3em de $A'$-modules. En posant $M'_l = B'_{j, l}/\pi_j B'_{j, l}$, on voit que
$\text{Norme}_{\kappa'_{j, l}/\kappa'}(u \bmod \mathfrak m'_{j, l})^{m'_{j, l}}
...
du déterminant, voir Compléments sur l'algèbre, Lemme \ref{more-algebra-lemma-ses}.\par}
\end{proof}
...
Soit $M \subset t^{-n}A$, respectivement $M' \subset t^{-2n}A$, le
sous-$A$-module formé des éléments qui s'envoient dans $B_i$ dans
$t^{-n}A_{\mathfrak q_i}$, respectivement dans $t^{-2n}A_{\mathfrak q_i}$.
Alors $M \subset M'$ sont des $A$-modules finis puisque $A$ est noethérien,
...
$$
{\emergencystretch=3em dans $A_{\mathfrak q_i}$. En posant
$m_i = \text{long}_{A_{\mathfrak q_i}}(A_{\mathfrak q_i}/\pi_i)$,
nous avons
$$
...
Nous avons alors une application\par}
$$
...
ne devrait prêter à aucune confusion). Dans ce cas, on obtient\par}
\begin{align*}
e_A(M_i, a, b) & =
...
-\text{ord}_{A/\mathfrak q_i}(\partial_{A_{\mathfrak q_i}}(a, b))
\end{align*}
{\emergencystretch=3em La deuxième égalité résulte du Lemme \ref{lemma-multiply-period-length}.
Observons que
...
la fonction ordre et de la définition de notre symbole modéré.\par}
...
Alors
$$
\text{long}_{\mathcal{O}_{X, \xi}} \mathcal{O}_{Z, \xi} < \infty
$$
...
l'entier
$m_{Z', Z} = \text{long}_{\mathcal{O}_{X, \xi}} \mathcal{O}_{Z, \xi}$
...
Alors
$$
\text{long}_{\mathcal{O}_{X, \xi}} \mathcal{F}_\xi < \infty
$$
...
$m_{Z', \mathcal{F}} = \text{long}_{\mathcal{O}_{X, \xi}} \mathcal{F}_\xi$
...
$$
{\emergencystretch=2em où la somme porte sur les composantes irréductibles de
$\text{Supp}(\mathcal{F})$ de $\delta$-dimension $k$.
(C'est un $k$-cycle d'après le Lemme \ref{lemma-length-finite}.)\par}
\end{enumerate}
\end{definition}
...
est manifestement $\text{long}_{R_{\mathfrak p}}(M_{\mathfrak p})$.
...
\text{long}_{A_{\mathfrak q_i}}(M_{\mathfrak q_i})
$$
...
$$
n = \text{long}_C(C/\mathfrak m_AC),
\quad
\text{et}
\quad
m = \text{long}_C(C/\mathfrak m_BC) \text{long}_B(B/\mathfrak m_AB)
$$
...
\begin{enumerate}
\item {\emergencystretch=6em Soit $Z \subset Y$ un sous-schéma fermé tel que
$\dim_\delta(Z) \leq k$. Alors
$\dim_\delta(f^{-1}(Z)) \leq k + r$
et $[f^{-1}(Z)]_{k + r} = f^*[Z]_k$ dans $Z_{k + r}(X)$.\par}
\item {\emergencystretch=6em Soit $\mathcal{F}$ un faisceau cohérent sur $Y$ tel que
$\dim_\delta(\text{Supp}(\mathcal{F})) \leq k$.
...
\begin{proof}
{\emergencystretch=2em D'après le Lemme \ref{lemma-exact-sequence-closed}, la flèche
...
\begin{proof}
{\emergencystretch=2em Soit $\{W_i\}_{i \in I}$ la famille des composantes
...
$q_*\gamma_Z = \sum_{j \in J_Z} n_j\deg(W'_j/W)[W] = 0$.\par}
\item $J_Z \not = \emptyset$ et $\dim_\delta(Z) < k$.
Dans ce cas, nous choisissons un sous-schéma fermé intègre $W \subset Y$
qui s'envoie birationnellement sur $Z$ (voir ci-dessus). En considérant les points
génériques, nous voyons que $W_j \times_Z W$ possède une unique composante
...
\begin{proof}
{\emergencystretch=2em La définition d'une sous-catégorie de Serre est la
Définition \ref{homology-definition-serre-subcategory} d'Homologie.
La démonstration du lemme est immédiate et omise.\par}
\end{proof}
...
$$
{\emergencystretch=3em alors $\mathcal{F}'$ est le sous-module cohérent maximal de $\mathcal{F}$
...
\begin{proof}
{\emergencystretch=3em Nous devons montrer qu'un élément $\alpha$ de $Z_k(X)$ qui est rationnellement
...
Soit $\mathcal{L}$ un faisceau inversible sur $Y$.
Soit $s$ une section méromorphe non nulle, notée $s$, de $\mathcal{L}$ sur $Y$.
...
\begin{proof}
{\emergencystretch=2em La dernière égalité résulte de la première puisque $f_*[X] = [R(X) : R(Y)][Y]$
...
nous cherchons simplement à démontrer\par}
$$
...
\begin{aligned}
0 \to{}&
\bigoplus\nolimits_{y \in Y, \delta(y) = k + i}' K_i^M(\kappa(y)) \\
{}\to{}&
\bigoplus\nolimits_{x \in X, \delta(x) = k + i}' K_i^M(\kappa(x)) \\
{}\to{}&
...
$\text{length}_A(M/fM) < \infty$. De plus, $\text{length}_A(M/fM)$
...
[(p|_D)^{-1}(W)]_{k + r}.
$$
{\emergencystretch=3em et la seconde découle de l'égalité $D \cap p^{-1}(W) = (p|_D)^{-1}(W)$ de schémas.
...
on applique le lemme \ref{lemma-support-cap-effective-Cartier}.\par}
\end{proof}
...
intègres de $L$ de $\delta$-dimension $k + 1$. Ainsi\par}
\begin{align*}
i_1^*p^*[W]
& =
i_1^*[p^{-1}(W)]_{k + 1} \\
...
\begin{proof}
{\emergencystretch=3em Remarquons que $(i')^*$ est défini, car on dispose du triplet
$(\mathcal{O}_{X'}, s', i' : D' \to X')$ comme dans la
définition \ref{definition-gysin-homomorphism}. L'énoncé a donc un sens.\par}
...
\pi_D^*(i^*\alpha_{r - i}) = 0
$$
{\emergencystretch=2em dans le groupe de Chow de $P_D$.
...
\noindent
Soit $(S, \delta)$ comme dans la Situation \ref{situation-setup}. Soit $X$ localement
de type fini sur $S$. Soient $\mathcal{E}_i$ une famille finie de faisceaux
...
associés à des modules localement libres de rang constant fini, et
...
Dans la Situation \ref{situation-setup}, soit $X$ localement de type fini sur $S$.
Soient $\mathcal{E}$ et $\mathcal{F}$ un
...
classe de Chern supérieure d'un module localement libre de type fini au moyen du
lieu d'annulation d'une « section régulière.
...
$$
{\emergencystretch=3em où $\mathcal{E}$, $\mathcal{E}'$, $\mathcal{F}$, $\mathcal{F}'$ sont
localement libres de type fini. Problème : montrer que
$c(\mathcal{E})c(\mathcal{E}')^{-1} \cap [X] =
c(\mathcal{F})c(\mathcal{F}')^{-1} \cap [X]$.\par}
...
\item $E_1 \to E_2$ peut être représenté être par une application de complexes localement bornés
...
lecteur de sauter le reste de la section.\par}
...
soient isomorphes à des $\mathcal{O}_{X_1}$-modules localement libres de rang fini
de rangs $< p$ et $< q$, placés en degré cohomologique $0$.
\end{enumerate}
...
isomorphes à des modules localement libres de rang fini
de rangs $< p$ et $< p'$.} pour $i = 1, 2$. Notons
\begin{enumerate}
...
Un second choix de $\beta$ diffère de $\beta$ par un cycle
...
\begin{proof}
{\emergencystretch=3em Rappelons que l'image de $c'_i(Q)$ dans $A^p(X)$ est égale à
...
l'additivité du lemme \ref{lemma-additivity-on-perfect}.\par}
...
$\mathcal{O}_U$-module localement libre de rang fini de rang $< p$. Voir Compléments d'algèbre, lemme
...
\begin{enumerate}
\item {\emergencystretch=4em pour $i = 1, \ldots, n$, l'hypothèse du
lemme \ref{lemma-localized-chern-pre} soit satisfaite pour
$b, Z_i, Q_i, T_i, p_i$,\par}
\item $Q_i|_{W \setminus W_i}$ soit nul, resp.\ soit isomorphe à un module
localement libre de rang fini de rang $< p_i$ placé en degré cohomologique $0$,
...
{}\circ{}&
c'_{p_{n - 1}}(Q_{n - 1}|_{W_{n - 1}, \infty}) \\
{}\circ{}& \ldots \circ
...
(W_{n - 1, \infty} \cap \ldots \cap W_{1, \infty} \to
W_{n - 1} \cap \ldots \cap W_1)^* \\
&\qquad {}\Bigl(c_{p_{n - 1}}(W_{n - 1} \to W, Q_{n - 1}) \cap \ldots \\
&\qquad {}\cap c_{p_1}(W_1 \to W, Q_1) \cap \beta\Bigr)
\end{align*}
...
$$
{\emergencystretch=2em Bien sûr, $b^{-1}(\mathbf{A}^1)$ s'envoie isomorphiquement sur $\mathbf{A}^1_X$.
...
lemme \ref{lemma-localized-chern-pre-sum-c}.\par}
\end{proof}
...
classes construites ci-dessus.\par}
...
\ref{more-morphisms-lemma-NL-immersion}, nous avons une suite exacte\par}
$$
...
\begin{proof}
{\emergencystretch=2em Une première remarque est que, si $\alpha = \sum n_i[X_i]$
...
dimensions $n + m$, et nous pouvons effectivement considérer\par}
$$
...
Par symétrie, il en va de même pour l'autre variable, et nous concluons.\par}
\end{proof}
...
\alpha \cdot (\beta \cdot \gamma)
\end{align*}
{\emergencystretch=2em Toutes les égalités ressortent clairement de ce qui précède, sauf peut-être
la deuxième et l'avant-dernière. L'égalité
$\Delta_{23}^!(\alpha \times (\beta \times \gamma)) =
\alpha \times \Delta^!(\beta \times \gamma)$ résulte de la
Remarque \ref{remark-commuting-exterior}. Il en va de même pour la deuxième
...
\item {\emergencystretch=4em les applications $g^* : \CH_k(X) \to \CH_{k + c}(X')$ et
...
du Lemme \ref{lemma-pullback-base-change}.\par}
\end{enumerate}
\end{lemma}

\begin{proof}
Soient $s \in S$ et $s'' \in S''$ un point générique d'une composante
...
Ainsi, $\alpha_i$ s'annule dans la colimite comme voulu.\par}
...
\noindent
Soulignons que l'on a toujours $l = \text{length}_R(M)$. Soulignons aussi qu'il
...
Nous allons montrer par récurrence sur $l = \text{length}(M)$
...
multipliée par $(-1)^{cg}$, avec $c = \text{length}_R(C)$ et $g = \text{length}_R(G)$,
et
...
\end{eqnarray*}
{\emergencystretch=2em La propriété (2) signifie qu'étant donnés un symbole
...
$$
{\emergencystretch=2em Cet isomorphisme est fonctoriel en $M$ et compatible avec les
isomorphismes $\gamma_{K \to L \to M}$ du Lemme \ref{lemma-det-exact-sequences}
définis pour $\det_{R, \kappa}$ et $\det_{R', \kappa}$.\par}
\end{lemma}
...
est un $\kappa$-espace vectoriel de dimension $l_i = \text{length}_R(M_i)$ et,
...
\det\nolimits_\kappa(M)
$$
est la multiplication par $\overline{u}^l$, où $l = \text{length}_R(M)$
...
Supposons que $\text{length}_R(I_\varphi) = a$ et que
$\text{length}_R(I_\psi) = b$, de sorte que $a + b = \text{length}_R(M)$
...
exact. Alors $\text{length}_R(M) = 2 \text{length}_R(\Im(\varphi))$
et
$$
...
$$
{\emergencystretch=8em Les carrés supérieur et inférieur sont commutatifs au signe près
...
le rectangle extérieur du diagramme ci-dessus commute au signe près\par}
\begin{eqnarray*}
\epsilon & = &
...
\medskip\noindent
Introduisons $r = \text{length}_R(K_\alpha)$,
$s = \text{length}_R(K_\beta)$ et $t = \text{length}_R(K_\gamma)$.
D'après les suites exactes ci-dessus et nos hypothèses, nous avons
$\text{length}_R(I_\alpha) = s + t$, $\text{length}_R(I_\beta) = r + t$,
$\text{length}_R(I_\gamma) = r + s$, et
$\text{length}(M) = r + s + t$.
Choisissons
\begin{enumerate}
...
Introduisons $k = \text{length}_R(K)$, $a = \text{length}_R(I_\varphi)$
et $b = \text{length}_R(I_\psi)$. Nous voyons alors que $\text{length}_R(M) = a + b$,
ainsi que $\text{length}_R(N) = a + b - k$, $\text{length}_R(Q) = k$
et $\text{length}_R(M') = a + b$. Les suites exactes ci-dessous montreront
qu'également $\text{length}_R(I_{\varphi'}) = a$ et
...
Alors $\text{length}_A(M/\varphi\psi M) < \infty$
...
\begin{proof}
{\emergencystretch=2em La première assertion résulte du lemme \ref{lemma-multiplicativity-determinant}
appliqué à $M/abcM$ et aux endomorphismes $\alpha, \beta, \gamma$ donnés par
la multiplication par $a, b, c$.
La seconde provient du lemme \ref{lemma-periodic-determinant-shift}.\par}
\end{proof}
...
Alors $d_M(a, a) = (-1)^{\text{length}_A(M/aM)}$.
\end{lemma}
...
\begin{proof}
{\emergencystretch=5em Si $a \in A^*$, l'égalité résulte de
l'égalité $\text{length}(M/bM) = \text{length}(M'/bM')$
...
au cas où $\text{length}_A(M'/M) = 1$.
Enfin, dans ce cas, considérons l'application\par}
$$
...
$\text{length}_{A_{\mathfrak q_i}}(M_{\mathfrak q_i})$, d'après
...
$d^A_{A'}(b', b') = (-1)^{\text{length}_A(A'/(b'))}$, et le lemme
est démontré.
\end{proof}
...
que $\text{length}_R(M/M') = (l - 1)\text{ord}_{R/\mathfrak q}(a)$
et, de même,
...
$$
{\emergencystretch=4em d'après la troisième des relations d'admissibilité qui définissent
...
\cong R/\mathfrak q$ pour $i = 1, \ldots, l$.
\end{enumerate}
De plus, dans ce cas, $l = \text{length}_{R_\mathfrak q}(M_\mathfrak q)$.
\end{lemma}

\begin{proof}
La condition selon laquelle $\mathfrak q$ est un idéal premier minimal du support
de $M$ implique que $l = \text{length}_{R_\mathfrak q}(M_\mathfrak q)$
...
$\mathfrak q$. Remarquons que $l = \text{length}_{R_{\mathfrak q}}(M_{\mathfrak q})$
est égal à $l = a + b$.
...
\begin{proof}
{\emergencystretch=2em Rappelons que $H^0(M, 0, \psi) = \Coker(\psi)$ et
...
lemme \ref{lemma-periodic-determinant-easy-case}.\par}
...
\begin{proof}
{\emergencystretch=2em Soit $\{W_j\}_{j \in J}$ la famille des composantes irréductibles
...
\begin{proof}
{\emergencystretch=6em D'après le lemme \ref{lemma-cycles-k-group}, nous avons
