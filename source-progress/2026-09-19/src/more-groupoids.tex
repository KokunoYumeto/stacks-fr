...
\noindent
Voici le second résultat. Il faut penser à l'ouvert $R$-invariant $W \subset U$ comme à
...
tel que $G_W \to W$ possède la propriété $\mathcal{P}$. Alors $W$ est $R$-invariant
(voir
...
Soit $t^{-1}(U') \subset R$ est le plus grand sous-schéma ouvert de $R$
...
est locale sur le but pour la topologie $\tau$. Si
\begin{enumerate}
...
pour voir que la propriété d'être de Cohen-Macaulay est locale pour la topologie fppf sur la cible).
\end{proof}
...
finis. Il existe une suite de sous-schémas fermés $R$-invariants
$$
...
que $s^{-1}(Z_r) = t^{-1}(Z_r)$ ; autrement dit, $Z_r$ est $R$-invariant.
Le morphisme
$s^{-1}(Z_{r - 1}) \setminus s^{-1}(Z_r) \to Z_{r - 1} \setminus Z_r$
...
$W' \subset W$ tels que
\begin{enumerate}
\item $W$ et $W'$ soient $R$-invariants,
...
soient finies localement libres.
\end{enumerate}
\end{lemma}
...
entre des sous-schémas $R$-invariants de $U$, de sorte que les morphismes
$s_r', t_r'$ des restrictions $(W_r', R_r', s_r', t_r', c_r')$
soient finis localement libres de rang $r$. Pour commencer, posons
$W_1 = W'_1 = U \setminus Z_1$. C'est un sous-schéma ouvert $R$-invariant
...
finies localement libres de rang $1$.
...
\item $W_r$ et $W_r'$ soient $R$-invariants,
...
$(W_r', R_r', s_r', t_r', c_r')$ soient finies localement libres de rang $r$.
\end{enumerate}
Posons alors
$$
W_{n + 1} = Z_n \setminus
...
\right)
$$
schématiquement. Alors $W_{n + 1}$ est un sous-schéma ouvert $R$-invariant
...
Ainsi, $W' \subset W$ vérifie (1), (2), (3) et (4).
\end{proof}
...
Comme les sous-schémas fermés $Z_k$ sont $R$-invariants et que
...
de composantes irréductibles de $U$. Il existe alors des sous-schémas $R$-invariants
...
on a $u_j \in W$ et $W' \to W$ est un épaississement de présentation finie.
D'après Limites, Lemme \ref{limits-lemma-affines-glued-in-closed-affine},
il suffit de trouver un sous-schéma ouvert affine $R$-invariant
...
composantes irréductibles de $U$. Il existe alors des sous-schémas $R$-invariants
$V' \subset V \subset U$ tels que
\begin{enumerate}
\item $u_1, \ldots, u_m \in V'$,
\item $V$ soit ouvert dans $U$,
...
cela implique que $u_{ji} \in W$. Comme $W$ est $R$-invariant, on
...
il suffit de trouver un sous-schéma ouvert affine $R$-invariant $V'$
...
ouvert affine $R'$-invariant, alors l'image $u \in U$ appartient
dans un ouvert affine $R$-invariant de $U$.
\end{lemma}

\begin{proof}
Soit $W' \subset U'$ un ouvert affine $R'$-invariant.
...
Comme $W'$ est $R'$-invariant, l'image
$W = h(\tilde W)$ est ensemblistement $R$-invariante et
...
soit un sous-schéma ouvert affine $R$-invariant de $U$ contenant
$u_1, \ldots, u_m$.
\end{lemma}
...
que $g$ est une fonction $R$-invariante ; ainsi, $D(g) \subset U$
est $R$-invariant. Comme $f$ ne s'annule pas sur l'orbite de $u_j$,
...
de l'orbite de $u$.
\end{enumerate}
Alors $u$ est contenu dans un ouvert affine $R$-invariant de $U$.
\end{lemma}
...
Alors $t(s^{-1}(U \setminus U'))$ est une partie fermée et $R$-invariante
de $U$ qui ne contient pas $u$. Ainsi
$U \setminus t(s^{-1}(U \setminus U'))$ est un ouvert $R$-invariant
de $U'$ contenant $u$.
...
et supposer que $U$ est réduit. Cela signifie que les sous-schémas $R$-invariants
...
un sous-schéma ouvert dense $R$-invariant $W \subset U$ tel que
...
pour trouver $f \in J$ tel que $(j|_W)^{-1}D(f)$ soit un ouvert $R_W$-invariant
affine contenant $\xi_j$ pour tout $j$. Puisque $f \in J$,
nous voyons que $j^{-1}D(f) \subset W$, c'est-à-dire que $j^{-1}D(f)$ est
un ouvert affine $R$-invariant de $U$ contenu dans $W$
...
$R$-invariant (mais il peut ne pas être $R$-invariant au sens schématique).
...
$j^{-1}D(h) \cap Z$ soit un ouvert affine $R_Z$-invariant contenant $u$.

\medskip\noindent
Considérons la fonction $R_W$-invariante (Groupoïdes, lemme
...
$$
est un ouvert affine $R$-invariant de $U$, ce qui achève la démonstration du lemme.
Il est $R$-invariant du point de vue ensembliste par construction. Comme $V$ est une
...
$R$-invariant et que $u_2$ lui appartient. Ainsi $V$ est ouvert.
...
où $W$ est ouvert et $R'$-invariant, et où
$E$ est $R'$-invariant du point de vue ensembliste et
