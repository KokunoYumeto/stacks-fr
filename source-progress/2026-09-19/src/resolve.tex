...
\begin{proof} Soit $n$ le degré de l'extension de corps des fractions $L/K$. Soit $\text{Trace}_{L/K} : L \to K$ la trace. Puisque l'extension est finie séparable, l'accouplement trace $(h, g) \mapsto \text{Trace}_{L/K}(fg)$ est une forme bilinéaire non dégénérée sur $L$ relativement à $K$. Voir Corps, lemme \ref{fields-lemma-separable-trace-pairing}. Choisissons $b_1, \ldots, b_n \in B$ qui forment une base de $L$ sur $K$. D'après ce qui précède, $d = \det(\text{Trace}_{L/K}(b_ib_j)) \in A$ est non nul.
...
\noindent Dans cette section, nous nous ramenons des points singuliers rationnels aux points singuliers rationnels de Gorenstein. Voir \cite{Lipman-rational} et \cite{Mattuck}.
...
\medskip\noindent D'après la discussion d'Exemples, section \ref{examples-section-bad}, il existe un anneau local noethérien normal intègre $A$ dont le complété est isomorphe à $\mathbf{C}[[x, y, z]]/(z^2)$. Dans ce cas, on pourrait dire que $A$ présente une singularité en point double rationnel ; en revanche, $\Spec(A)$ n'admet pas de résolution des singularités. Ce type de comportement ne peut pas se produire si $A$ est un anneau de Nagata ; voir Algèbre, lemme \ref{algebra-lemma-local-nagata-domain-analytically-unramified}.
...
$$ a un noyau engendré par deux éléments $f_2, f_3 \in \kappa[y_2, y_3]$ (voir par exemple Algèbre, exemple \ref{algebra-example-spec-kxy}, ou la démonstration d'Algèbre, lemme \ref{algebra-lemma-dim-affine-space}). Soit $z_2, z_3 \in A'$ un élément qui s'envoie sur $f_2, f_3$ dans $\kappa[y_2, y_3]$. On voit alors que $\mathfrak m' = (x_1, z_2, z_3)$, car $x_2$ et $x_3$ deviennent divisibles par $x_1$ dans $A'$.
...
\noindent Dans cette section, nous montrons que, pour un schéma intègre noethérien, l'existence d'une altération régulière a un assez grand nombre de conséquences. Les lecteurs qui ne s'intéressent pas aux « mauvais » anneaux noethériens peuvent omettre cette section.
...
\noindent Soit $X$ un schéma noethérien. Soit $E \subset X$ un sous-schéma fermé possédant les propriétés suivantes : \begin{enumerate} \item $E$ est un diviseur de Cartier effectif sur $X$ ; \item il existe un corps $k$ et un isomorphisme de schémas $\mathbf{P}^1_k \to E$ ; \item le faisceau normal $\mathcal{N}_{E/X}$ se tire en arrière sur $\mathcal{O}_{\mathbf{P}^1}(-1)$. \end{enumerate} Un tel sous-schéma fermé est appelé une {\it courbe exceptionnelle de première espèce}.
...
$$ du lemme \ref{coherent-lemma-formal-functions-stalk} de Cohomologie des schémas. Soit $E_n = nE$ comme dans le lemme \ref{lemma-exceptional-first-kind-local}. Observons que $$
