...
\item $G^0 \subset G$ est la composante connexe de l'élément neutre,
...
\begin{proof}
Soit $G^0$ la composante connexe de l'élément neutre, munie de sa structure canonique
...
Cela achèvera la preuve, car $g^{-1}U \subset W$ en vertu de l'invariance
sous les translations à gauche par $G^0$ de $W$.
...
la composante connexe de $G$ comme dans la
Proposition \ref{proposition-connected-component}.
...
\section{Espaces homogènes principaux}
...
\begin{enumerate}
\item Nous disons que $X$ est un {\it pseudo-$G$-torseur} ou que $X$ est
...
\item {\emergencystretch=1em Un pseudo-$G$-torseur $X$ est dit {\it trivial} s'il existe
...
le changement de base $X_{S'}$ d'un pseudo-$G$-torseur sur $S$ est un
pseudo-$G_{S'}$-torseur sur $S'$.
...
\begin{enumerate}
\item Le schéma $X$ est un pseudo-$G$-torseur si et seulement si, pour tout schéma
$T$ sur $S$, l'ensemble $X(T)$ est vide ou l'action du groupe $G(T)$
sur $X(T)$ est simplement transitive.
\item Un pseudo-$G$-torseur $X$ est trivial si et seulement si le morphisme
...
Soit $S$ un schéma.
Soit $(G, m)$ un schéma en groupes sur $S$.
Soit $X$ un pseudo-$G$-torseur sur $S$.
\begin{enumerate}
\item Nous disons que $X$ est un {\it espace homogène principal}
ou un {\it $G$-torseur} s'il existe un recouvrement fpqc\footnote{Cela signifie
...
$X_{S_i} \to S_i$ possède une section (c'est-à-dire qu'il est un pseudo-$G_{S_i}$-torseur trivial).
\item Soit $\tau \in\allowbreak \{Zariski,\allowbreak \etale,\allowbreak smooth,\allowbreak syntomic,\allowbreak fppf\}$.
Nous disons que $X$ est un {\it $G$-torseur pour la topologie $\tau$}, ou un
{\it $\tau$-$G$-torseur}, ou simplement un {\it $\tau$-torseur},
s'il existe un recouvrement $\tau$ $\{S_i \to S\}_{i \in I}$
tel que chaque $X_{S_i} \to S_i$ possède une section.
\item Si $X$ est un $G$-torseur, nous disons qu'il est
...
\noindent
Nous disons parfois « soit $X$ un $G$-torseur sur $S$ » pour indiquer que
...
Alors $X$ est un $G$-torseur pour la topologie $\tau$ si et seulement si
$\underline{X}$ est un $\underline{G}$-torseur sur $(\Sch/S)_\tau$.
\end{lemma}
...
\begin{enumerate}
\item Si $X \to S$ est un pseudo-$G$-torseur et si $X \to S$ est surjectif,
alors $X$ est-il nécessairement un $G$-torseur ?
\item Tout $\underline{G}$-torseur sur $(\Sch/S)_{fppf}$ est-il
représentable ? Autrement dit, tout $\underline{G}$-torseur
provient-il d'un $G$-torseur fppf ?
\item Tout $G$-torseur est-il un
...
et, en fait, tout $\text{GL}_{n, S}$-torseur est localement trivial
(cela résulte de
...
$$
à partir du morphisme d'anneaux $\mu : A \to A \otimes \mathbf{Z}[x, x^{-1}]$,
...
\noindent
Soit $S$ un schéma. Soit $(U, R, s, t, c)$ un groupoïde en schémas sur $S$.
...
Soit $(U, R, s, t, c)$ un groupoïde en schémas sur $S$. Soit $\kappa$ un
...
On obtient alors un groupoïde en schémas $(U, R, s, t, c, e, i)$ sur $S$
...
le groupoïde en schémas construit dans le lemme \ref{lemma-groupoid-from-action}.
La règle
...
\noindent
À partir d'un groupoïde en schémas, on obtient un schéma en groupes comme suit.
...
est appelé le {\it stabilisateur du groupoïde en schémas
$(U, R, s, t, c)$}.
\end{definition}
...
Soit $S$ un schéma. Soit $(U, R, s, t, c)$ un groupoïde en schémas
...
telle que $(U', R', s', t', c')$ soit un groupoïde en schémas sur
...
Soit $(U, R, s, t, c)$ un groupoïde en schémas sur le schéma de base $S$.
\begin{enumerate}
...
finies localement libres. Si $\varphi : A \to B$ est une application d'anneaux finie localement libre, alors
...
$$
{\emergencystretch=2em du Lemme \ref{lemma-diagram}.
...
(\ref{equation-invariants}). Soit $C \to C'$ une application d'anneaux, et posons
$U' = \Spec(A \otimes_C C')$,
$R' = \Spec(B \otimes_C C')$.
Alors
\begin{enumerate}
...
\medskip\noindent
Dans ce paragraphe, nous démontrons (2)(a) lorsque les applications d'anneaux
$s^\sharp, t^\sharp : A \to B$ sont finies localement libres d'un rang fixé $r$.
...
\medskip\noindent
Soit $k$ un corps algébriquement clos et soit $C \to k$ une application d'anneaux.
...
\medskip\noindent
Soient $a_0, a_1 : A \to k$ deux applications d'anneaux. S'il existe une application d'anneaux
...
il existe un nombre fini d'applications d'anneaux
...
et l'application d'anneaux \'etale
...
\medskip\noindent
On effectuera des changements de base par des morphismes plats d'anneaux $C \to C'$, comme dans
...
un morphisme local plat d'anneaux $C_{\mathfrak p} \to C_{\mathfrak p}'$
...
supposer que les morphismes d'anneaux $s, t : A \to B$ sont finis
localement libres d'un rang fixé $r$.

\medskip\noindent
Les morphismes locaux d'anneaux $C_{\mathfrak p} \to C_{\mathfrak p}'$ que l'on va
...
au morphisme d'anneaux $C \to A$, au $A$-module $M = B$ (considéré comme un $A$-module
