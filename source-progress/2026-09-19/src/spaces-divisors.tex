...
\noindent
Dans ce chapitre, nous étudions les diviseurs sur les espaces algébriquess et les sujets connexes.
Une référence de base pour les espaces algébriquess est \cite{Kn}.
...
des points associés. À savoir, les points associés habituels
...
Pour un espace algébrique général la notion d'un point associé
est fondamentalement inutile et nous n'avons même pas la peine de le présenter.
Si l'espace algébrique est localement noethérien, alors nous nous autorisons
pour utiliser l'expression « point associé » au lieu de « point associé »
''point faiblement associé'', car les notions sont les mêmes pour
schémas noethériens (Diviseurs, lemme \ref{divisors-lemma-ass-weakly-ass}).
Avant de faire notre définition, nous avons besoin d'un lemme.

\begin{lemma}
\label{lemma-associated}
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Soit $x \in |X|$. Les éléments suivants sont équivalents
\begin{enumerate}
\item pour un morphisme \'étale $f : U \to X$ avec $U$ un schéma
et la application $u \in U$ à $x$, le point $u$ est faiblement associé
à $f^*\mathcal{F}$,
\item pour chaque morphisme \'étale $f : U \to X$ avec $U$
et la application $u \in U$ à $x$, le point $u$ est faiblement associé
à $f^*\mathcal{F}$,
\item l'idéal maximal de $\mathcal{O}_{X, \overline{x}}$
est un idéal premier faiblement associé de le faisceau $\mathcal{F}_{\overline{x}}$.
\end{enumerate}
Si $X$ est localement noethérien, alors ceux-ci sont également équivalents à
\begin{enumerate}
\item[(4)] pour un morphisme \'étale $f : U \to X$ avec $U$ un schéma
et la application $u \in U$ à $x$, le point $u$ est associé
à $f^*\mathcal{F}$,
\item[(5)] pour chaque morphisme \'étale $f : U \to X$ avec $U$
et la application $u \in U$ à $x$, le point $u$ est associé
à $f^*\mathcal{F}$,
\item[(6)] l'idéal maximal de $\mathcal{O}_{X, \overline{x}}$
est un idéal premier associé de le faisceau $\mathcal{F}_{\overline{x}}$.
\end{enumerate}
\end{lemma}

\begin{proof}
Choisissons un schéma $U$ avec un point $u$ et un morphisme \'étale
Carte $f : U \to X$ $u$ à $x$. Relevons $\overline{x}$ à un géométrique
le point de $U$ au-dessus de $u$.
$\mathcal{O}_{X, \overline{x}} = \mathcal{O}_{U, u}^{sh}$
où la stricte henselisation est par rapport au relèvement choisi
ascenseur de $\overline{x}$, voir
...
On obtient ainsi l'équivalence des points 1), 2) et 3)
Compléments sur la platitude, lemme \ref{flat-lemma-weakly-associated-henselization}.
Si $X$ est localement noethérien, alors
n'importe quel $U$ comme ci-dessus est localement noethérien,
Par conséquent, nous voyons que (1), resp.\ (2) sont équivalents à (4), resp.\ (5) par
Diviseurs, lemme \ref{divisors-lemma-ass-weakly-ass}.
D'autre part, dans l'affaire locale noethérien
anneau local $\mathcal{O}_{X, \overline{x}}$ est noethérien aussi
(Propriétés des espaces, lemme
\ref{spaces-properties-lemma-Noetherian-local-ring-Noetherian}).
On obtient ainsi l'équivalence de (3) et (6) par le même lemme
...
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Soit $\mathcal{F}$ un faisceau quasi-cohérente sur $X$.
Soit $x \in |X|$.
\begin{enumerate}
\item Nous disons que $x$ est {\it  faiblement associé} à $\mathcal{F}$
si les conditions équivalentes (1), (2) et (3)
lemme \ref{lemma-associated} sont satisfaits.
\item Nous indiquons $\text{WeakAss}(\mathcal{F})$ l'ensemble de faiblement associés
les points $\mathcal{F}$.
\item Les points {\it  faiblement associés de $X$} sont les points faiblement associés
les points $\mathcal{O}_X$.
\end{enumerate}
Si $X$ est localement noethérien nous dirons
{\it $x$ est associé à $\mathcal{F}$}
si et seulement si $x$ est faiblement associé à $\mathcal{F}$ et que nous définissons
$\text{Ass}(\mathcal{F}) = \text{WeakAss}(\mathcal{F})$.
Enfin (en supposant que $X$ soit localement noethérien),
nous dirons {\it $x$ est un point associé de $X$} si et seulement si
$x$ est un point faiblement associé à $X$.
\end{definition}

\noindent
À ce stade, nous pouvons prouver les lemmess obligatoires.
...
\begin{proof}
Ceci découle immédiatement des définitions. L'appui d'un faisceau d'abélien
...
une suite exacte courte de faisceaux quasi-cohérents sur $X$.
Alors
...
\begin{proof}
Pour chaque point géométrique $\overline{x} \in X$
la suite des tiges
$0 \to \mathcal{F}_{1, \overline{x}} \to
\mathcal{F}_{2, \overline{x}} \to
\mathcal{F}_{3, \overline{x}} \to 0$
est une suite exacte courte de $\mathcal{O}_{X, \overline{x}}$-modules.
...
\begin{proof}
Choisissons un schéma $U$ et un surjectif \'étale morphisme $f : U \to X$.
Alors, $\mathcal{F}$ est zéro si et seulement si $f^*\mathcal{F}$ est zéro.
Ainsi, le lemme découle de la définition et le lemme dans le
cas de schémas, voir
...
\item $x$ est une codimension $0$ point de $X$
...
est un module de type fini $\mathcal{O}_X$ avec support schématique $Z$
(Morphismes des espaces, Définition
\ref{spaces-morphisms-definition-scheme-theoretic-support})
et $x$ est une codimension $0$ point de $Z$, alors
$x \in \text{WeakAss}(\mathcal{F})$.
\end{lemma}

\begin{proof}
Depuis $x \in \text{Supp}(\mathcal{F})$ le faisceau
$\mathcal{F}_{\overline{x}}$ n'est pas zéro.
...
est un singleton. Par conséquent, $x$ est un point faiblement associés de
$\mathcal{F}$ par définition. L'énoncé final suit
en tant que $\mathcal{O}_{X, \overline{x}} \to \mathcal{O}_{Z, \overline{z}}$
est une surjection, le spectre de $\mathcal{O}_{Z, \overline{z}}$
est un singleton, et $\mathcal{F}_{\overline{x}}$ est un non nul
module sur $\mathcal{O}_{Z, \overline{z}}$.
\end{proof}
...
Soit $x \in |X|$. Si
\begin{enumerate}
\item $X$ est décent (par exemple quasi séparé ou séparé localement),
\item $x \in \text{Supp}(\mathcal{F})$
...
Un espace algébrique séparé localement est décent, voir
...
$f(u') \not = x$. Nous voyons donc que $u \in \text{Supp}(f^*\mathcal{F})$
n'est pas une spécialisation d'un autre point de
$\text{Supp}(f^*\mathcal{F})$.
Ainsi, $u \in \text{WeakAss}(f^*\mathcal{F})$ par
Diviseurs, lemme \ref{lemma-minimal-support-in-weakly-ass}.
\end{proof}

\begin{lemma}
\label{lemma-finite-ass}
Soit $S$ un schéma. Soit $X$ un espace algébrique local noethérien sur $S$.
Soit $\mathcal{F}$ un module cohérent $\mathcal{O}_X$.
Alors $\text{Ass}(\mathcal{F}) \cap W$ est fini pour
chaque quasi-compact ouvert $W \subset |X|$.
\end{lemma}

\begin{proof}
Choisissons un schéma quasi compact $U$ et un morphisme \'étale $U \to X$
tel que $W$ est l'image de $|U| \to |X|$. Ensuite $U$ est un
Régime noetherien et nous pouvons appliquer
Diviseurs, lemme \ref{divisors-lemma-finite-ass} pour conclure.
\end{proof}
...
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Si $U \to X$ est un morphisme \'tale tel que
$\text{WeakAss}(\mathcal{F}) \subset \Im(|U| \to |X|)$, puis
$\Gamma(X, \mathcal{F}) \to \Gamma(U, \mathcal{F})$ est injectable.
\end{lemma}

\begin{proof}
Soit $s \in \Gamma(X, \mathcal{F})$ une section qui se limite à zéro sur $U$.
Soit $\mathcal{F}' \subset \mathcal{F}$ être l'image de la carte
$\mathcal{O}_X \to \mathcal{F}$ défini par $s$. Ensuite $\mathcal{F}'|_U = 0$.
Cela implique que
$\text{WeakAss}(\mathcal{F}') \cap \Im(|U| \to |X|) = \emptyset$
(par la définition de points faiblement associés).
D'un autre côté,
$\text{WeakAss}(\mathcal{F}') \subset \text{WeakAss}(\mathcal{F})$
par lemme \ref{lemma-ses-weakly-ass}. Nous concluons
$\text{WeakAss}(\mathcal{F}') = \emptyset$.
Ainsi, $\mathcal{F}' = 0$ par lemme \ref{lemma-weakly-ass-zero}.
\end{proof}

\begin{lemma}
\label{lemma-weakass-pushforward}
Soit $S$ un schéma. Soit $f : X \to Y$ un quasi-compact et quasi-séparé
morphisme des espaces algébriques sur $S$. Que $\mathcal{F}$ un quasi-cohérent
$\mathcal{O}_X$-module. Soit $y \in |Y|$ un point qui n'est pas dans le
l'image de $|f|$. Alors, $y$ n'est pas faiblement associé à $f_*\mathcal{F}$.
\end{lemma}

\begin{proof}
Par Morphismes d'espaces, lemme \ref{spaces-morphisms-lemma-pushforward}
le $\mathcal{O}_Y$-module $f_*\mathcal{F}$ est donc quasi-cohérent
La lemme a du sens.
Choisissons un schéma affine $V$, un point $v \in V$ et un morphisme \'tale
$V \to Y$ application $v$ à $y$. Nous pouvons remplacer
$f : X \to Y$, $\mathcal{F}$, $y$ par
$X \times_Y V \to V$, $\mathcal{F}|_{X \times_Y V}$, $v$.
Ainsi, nous pouvons supposer que $Y$ est un schéma affine.
Dans ce cas, $X$ est quasi-compact, donc nous pouvons choisir
un schéma affine $U$ et un morphisme surjectif \'étale $U \to X$.
Noter $g : U \to Y$ la composition.
Alors $f_*\mathcal{F} \subset g_*(\mathcal{F}|_U)$.
Par lemme \ref{lemma-ses-weakly-ass}
nous réduisons au cas des schémas qui est
...
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Soit $\varphi : \mathcal{F} \to \mathcal{G}$ une carte de
quasi-cohérent $\mathcal{O}_X$-modules. Supposons que pour chaque
$x \in |X|$ au moins l'un des suivants se produit
\begin{enumerate}
\item $\mathcal{F}_{\overline{x}} \to \mathcal{G}_{\overline{x}}$
est injectable, ou
\item $x \not \in \text{WeakAss}(\mathcal{F})$.
\end{enumerate}
Alors, $\varphi$ est injectable.
\end{lemma}

\begin{proof}
Les hypothèses impliquent que $\text{WeakAss}(\Ker(\varphi)) = \emptyset$
et donc $\Ker(\varphi) = 0$ par lemme \ref{lemma-weakly-ass-zero}.
\end{proof}

\begin{lemma}
\label{lemma-weakass-reduced}
Soit $S$ un schéma. Soit $X$ un espace algébrique réduit sur $S$.
Alors, le point faiblement associés de $X$ sont exactement la codimension $0$
les points $X$.
\end{lemma}

\begin{proof}
\'étale travaillant localement
...
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme affine des espaces algébriques sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Alors nous avons
$$
...
\begin{proof}
Choisissons un schéma $V$ et un surjectif \'étale morphisme $V \to Y$.
Set $U = X \times_Y V$. Alors, $U \to V$ est un morphisme affine
Par notre définition des points faiblement associés
le problème est réduit au morphisme des schémas $U \to V$.
...
Que $f : X \to Y$ un morphisme affine des espaces algébriques sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Si $X$ est localement noethérien, alors nous avons
$$
...
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme fini des espaces algébriques sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
...
\begin{proof}
Choisissons un schéma $V$ et un surjectif \'étale morphisme $V \to Y$.
Set $U = X \times_Y V$. Ensuite $U \to V$ est un morphisme fini
Par notre définition des points faiblement associés
le problème est réduit au morphisme des schémas $U \to V$.
...
Que $S$ un schéma. Que $f : X \to Y$ un morphisme des espaces algébriques
au-dessus de $S$. Soit $\mathcal{G}$ un module quasi-cohérent $\mathcal{O}_Y$.
Soit $x \in |X|$ et $y = f(x) \in |Y|$. Si
\begin{enumerate}
\item $y \in \text{WeakAss}_S(\mathcal{G})$,
\item $f$ est plat à $x$, et
\item la dimension de l'anneau local de la fibre de $f$ à $x$
est zéro (Morphismes des espaces, Définition
\ref{spaces-morphisms-definition-dimension-fibre}),
\end{enumerate}
puis $x \in \text{WeakAss}(f^*\mathcal{G})$.
\end{lemma}

\begin{proof}
Choisissons un schéma $V$, un point $v \in V$ et un morphisme \'étale $V \to Y$
envoyant $v$ to $y$. Choisissons un schéma $U$, un point $u \in U$, et un
Morphisme \'étale Morphisme $U \to V \times_Y X$ application $v$ jusqu'à un point situé au-dessus
$v$ et $x$. Ceci est possible parce qu'il y a un $t \in |V \times_Y X|$
application vers $(v, y)$ par Propriétés des espaces, lemme
\ref{spaces-properties-lemma-points-cartesian}.
Par définition, nous voyons que la dimension de $\mathcal{O}_{U_v, u}$ est zéro.
$u$ est donc un point générique de la fibre $U_v$.
Par notre définition des points faiblement associés
le problème est réduit au morphisme des schémas $U \to V$.
Cette affaire est traitée dans
Diviseurs, lemme \ref{divisors-lemma-weakly-ass-pullback}.
\end{proof}

\begin{lemma}
\label{lemma-weakly-ass-change-fields}
Soit $K/k$ une extension de champ. Soit $X$ un espace algébrique sur $k$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Soit $y \in X_K$ avec l'image $x \in X$. Si $y$ est un faible
point associé du retrait $\mathcal{F}_K$, puis $x$
...
dans le langage des espaces algébriques. Nous omettons les détails de la
traduction.
\end{proof}
...
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme plat fini des espaces algébriques.
Que $\mathcal{G}$ un module quasi-cohérent $\mathcal{O}_Y$.
Soit $x \in |X|$ un point avec l'image $y \in |Y|$.
$$
...
\begin{proof}
S'inscrit immédiatement dans le cas des schémas
...
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme \'étale des espaces algébriques.
Que $\mathcal{G}$ un module quasi-cohérent $\mathcal{O}_Y$.
Soit $x \in |X|$ un point avec l'image $y \in |Y|$.
$$
...
\begin{proof}
Ceci est immédiat à partir de la définition des points faiblement associés
et, en fait, le lemme correspondant dans le cas des schémas
(Compléments sur la platitude, lemme \ref{flat-lemma-etale-weak-assassin-up-down})
est la base de notre définition.
\end{proof}
...
\noindent
Il nous faut quelques lemmes pour définir ce gadget.

\begin{lemma}
\label{lemma-locally-noetherian-fibre}
Que $S$ un schéma. Que $f : X \to Y$ un morphisme des espaces algébriques
sur $S$. Soit $y \in |Y|$. Les éléments suivants sont équivalents
\begin{enumerate}
\item pour certains schémas $V$, $v \in V$ et \'étale morphisme $V \to Y$
envoyant $v$ à $y$, l'espace algébrique $X_v$ est localement noethérien,
\item pour chaque schéma $V$, point $v \in V$, et \'étale morphisme $V \to Y$
application de $v$ à $y$, l'espace algébrique $X_v$ est localement noetherien, et
\item existe un corps $k$ et un morphisme $\Spec(k) \to Y$ représentant
$y$ de telle sorte que $X_k$ soit localement noethérien.
\end{enumerate}
S'il existe un corps $k_0$ et un monomorphisme $\Spec(k_0) \to Y$
représentant $y$, alors ceux-ci sont également équivalents à
\begin{enumerate}
...
\begin{proof}
Observez que $X_v = v \times_Y X = \Spec(\kappa(v)) \times_Y X$.
Ainsi, les implications (2) $\Rightarrow$ (1) $\Rightarrow$ (3) sont claires.
Supposons que $\Spec(k) \to Y$ un morphisme à partir du spectre d'un corps
de telle sorte que $X_k$ soit localement noethérien. Soit $V \to Y$ un morphisme \'tale
à partir d'un schéma $V$ et laisser $v \in V$ une application ponctuelle à $y$.
Alors, le schéma $v \times_Y \Spec(k)$ n'est pas vide.
point $w \in v \times_Y \Spec(k)$. Considérez les morphismes
$$
X_v \longleftarrow X_w \longrightarrow X_k
$$
Alorsque $V \to Y$ est \'étale et que $w$ peut être vu comme un point de
$V \times_Y \Spec(k)$, nous voyons que $\kappa(w)/k$
est une extension de champs séparables finie
(Morphismes, lemme \ref{morphisms-lemma-etale-over-field}).
Ainsi, $X_w \to X_k$ est un morphisme fini \'tale comme un changement de base de
$w \to \Spec(k)$. Par conséquent $X_w$ est localement noethérien
(Morphismes des espaces, lemme
\ref{spaces-morphisms-lemma-locally-finite-type-locally-noetherian}).
Le morphisme $X_w \to X_v$ est un morphisme surjectif, affine et plat
comme un changement de base du surjectif, affine, morphisme plat $w \to v$.
Alors, le fait que $X_w$ soit localement noethérien implique que
$X_v$ est localement noethérien.
surjectif \'tale morphisme $U \to X$ puis en utilisant
$U_w \to U_v$ est surjectif, affine et plat. Travail
Nous concluons sur place sur le plan $U_v$
...
dans le cas où nous avons un monomorphisme $\Spec(k_0) \to Y$ dans la classe de $y$.
Alors, $\Spec(k) \to Y$ facteurs comme $\Spec(k) \to \Spec(k_0) \to Y$.
L'argument donné ci-dessus montre alors que $X_k$ est
localement noethérien implique que $X_{k_0}$ soit localement noethérien.
\end{proof}

\begin{definition}
\label{definition-locally-Noetherian-fibre}
Que $S$ un schéma. Que $f : X \to Y$ un morphisme des espaces algébriques
sur $S$. Soit $y \in |Y|$. Nous disons {\it la fibre de $f$ sur $y$ est
local noethérien} si les conditions équivalentes (1), (2) et (3)
de lemme \ref{lemma-locally-noetherian-fibre} sont satisfaits.
Nous disons que {\it les fibres de $f$ sont localement noethérien} si cela
détient pour chaque $y \in |Y|$.
\end{definition}

\noindent
Bien sûr, la façon habituelle de garantir localement les fibres de noethérien est
à supposer que le morphisme est localement de type fini.

\begin{lemma}
\label{lemma-locally-finite-type-locally-Noetherian-fibres}
Que $S$ un schéma. Que $f : X \to Y$ un morphisme des espaces algébriques
sur $S$. Si $f$ est de type fini localement, alors 
les fibres de $f$ sont localement noetheriennes.
\end{lemma}

\begin{proof}
Cela fait suite aux Morphismes d'espaces, lemme
\ref{spaces-morphisms-lemma-locally-finite-type-locally-noetherian}
et le fait que le spectre d'un corps est noethérien.
\end{proof}
...
Que $\mathcal{F}$ un quasi-cohérent
$\mathcal{O}_X$-module. Considérer les diagrammes commutatifs
$$
\xymatrix{
...
sont \'tale, $v \in V$, $x' \in |X_v|$, $u \in U$ sont des points
comme dans le dernier diagramme.
Note $\mathcal{F}|_{X_v}$ et $\mathcal{F}|_{U_v}$
les retraits de $\mathcal{F}$.
Les éléments suivants sont équivalents:
\begin{enumerate}
\item pour certains $V, v, x'$ comme ci-dessus $x'$ est faiblement associé
le point $\mathcal{F}|_{X_v}$,
\item pour chaque $V \to Y, v, x'$ comme ci-dessus $x'$ est faiblement associé
le point $\mathcal{F}|_{X_v}$,
\item pour certains $U, V, u, v$ comme ci-dessus $u$ est faiblement associé
le point $\mathcal{F}|_{U_v}$,
\item pour chaque $U, V, u, v$ comme ci-dessus $u$ est faiblement associé
le point $\mathcal{F}|_{U_v}$,
\item pour certains champs $k$ et le morphisme $\Spec(k) \to Y$ représentant $y$
et un certain $t \in |X_k|$ envoyant à $x$, le point $t$ est un faible
le point associé à $\mathcal{F}|_{X_k}$.
\end{enumerate}
S'il existe un corps $k_0$ et un monomorphisme $\Spec(k_0) \to Y$
représentant $y$, alors ceux-ci sont également équivalents à
\begin{enumerate}
\item[(6)] $x_0$ est un point faiblement associé à $\mathcal{F}|_{X_{k_0}}$
où $x_0 \in |X_{k_0}|$ est la application ponctuelle unique à $x$.
\end{enumerate}
Si la fibre de $f$ au-dessus de $y$ est localement noétherienne, alors dans
conditions (1), (2), (3), (4) et (6) nous pouvons remplacer
``faiblement associé'' avec ``associé''.
\end{lemma}

\begin{proof}
Observez que vu $V, v, x'$ comme dans le lemme nous pouvons trouver
Application $U \to X \times_Y V$ et $u \in U$ vers $x'$
puis le morphisme $U_v \to X_v$ est \'étale.
Il est donc clair que les paragraphes 1 et 3 sont équivalents.
ainsi que les paragraphes (2) et (4). Chacun d'eux implique (5).
Nous montrerons que (5) implique (2).
Supposons que $V, v, x'$ soit administré, ainsi que $\Spec(k) \to X$ et $t \in |X_k|$
si bien que le point $t$ est faible
le point associé à $\mathcal{F}|_{X_k}$.
Nous pouvons choisir un point $w \in v \times_Y \Spec(k)$.
Ensuite nous obtenons les morphismes
$$
X_v \longleftarrow X_w \longrightarrow X_k
$$
Alorsque $V \to Y$ est \'étale et que $w$ peut être vu comme un point de
...
(Morphismes, lemme \ref{morphisms-lemma-etale-over-field}).
Ainsi, $X_w \to X_k$ est un morphisme fini \'tale comme un changement de base de
$w \to \Spec(k)$. Ainsi, tout point $x''$ de $X_w$ se trouvant au-dessus de $t$
est un point faiblement associés de $\mathcal{F}|_{X_w}$ par
lemme \ref{lemma-etale-weak-assassin-up-down}.
Nous pouvons choisir la application $x''$ vers $x'$
(Propriétés des espaces, lemme \ref{spaces-properties-lemma-points-cartesian}).
Alors lemme \ref{lemma-weakly-ass-change-fields}
implique que $x'$ est faiblement associé
le point $\mathcal{F}|_{X_v}$.

\medskip\noindent
Pour finir la preuve il suffit de montrer que l'équivalent
conditions (1) -- (5) impliquent (6) si nous sommes donnés
$\Spec(k_0) \to Y$ comme dans (6). Dans ce cas le morphisme
$\Spec(k) \to Y$ de (5) facteurs uniques comme $\Spec(k) \to \Spec(k_0) \to Y$.
Alors, $x_0$ est l'image de $t$ sous le morphisme $X_k \to X_{k_0}$.
Ainsi, le même lemme que ci-dessus montre que (6) est vrai.
\end{proof}

\begin{definition}
\label{definition-relative-weak-assassin}
Que $S$ un schéma. Que $f : X \to Y$ un morphisme des espaces algébriques
au-dessus de $S$. Soit $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
L'assassin faible relatif {\it  de $\mathcal{F}$ dans $X$ sur $Y$}
est le jeu $\text{WeakAss}_{X/Y}(\mathcal{F}) \subset |X|$
consistant en ces $x \in |X|$ de telle sorte que les conditions équivalentes de
lemme \ref{lemma-relative-assassin} sont satisfaits.
Si les fibres de $f$ sont localement noethérien
(Définition \ref{definition-locally-Noetherian-fibre})
puis nous utilisons la notation $\text{Ass}_{X/Y}(\mathcal{F})$.
\end{definition}

\noindent
Avec cette notation, nous pouvons formuler quelques-uns des résultats
déjà prouvé pour les schémass.

\begin{lemma}
\label{lemma-bourbaki}
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme des espaces algébriques sur $S$.
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$.
Que $\mathcal{G}$ un module quasi-cohérent $\mathcal{O}_Y$.
Supposons
\begin{enumerate}
\item $\mathcal{F}$ est plat sur $Y$,
\item $X$ et $Y$ sont localement noetheriens, et
\item les fibres de $f$ sont localement noetheriennes.
\end{enumerate}
Alors
$$
...
\begin{proof}
Via la localisation \'étale, c'est une conséquence immédiate du résultat
pour les schémass, voir
Diviseurs, lemme \ref{divisors-lemma-bourbaki}.
Le résultat pour les schémass est plus général seulement parce que
nous n'avons pas défini de points associés pour
espaces algébriques non-noétheriens (d'où la nécessité de supposer $X$
et les fibres de $X \to Y$ sont localement noethérien à
être en mesure de formuler ce résultat).
\end{proof}
...
Que $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$
et définir $\mathcal{F}' = (g')^*\mathcal{F}$.
Si $f$ est de type fini localement, alors
\begin{enumerate}
\item $x' \in \text{Ass}_{X'/Y'}(\mathcal{F}')
\Rightarrow g'(x') \in \text{Ass}_{X/Y}(\mathcal{F})$
\item si $x \in \text{Ass}_{X/Y}(\mathcal{F})$, alors donné
$y' \in |Y'|$ avec $f(x) = g(y')$, il existe un
...
\begin{proof}
Cela découle du cas des schémas par localisation \'étale.
Nous écrivons les détails complètement. Choisissons un schéma
$V$ et un morphisme surjectif \'étale $V \to Y$.
Choisissons un schéma $U$ et un surjectif
\'tale morphisme $U \to V \times_Y X$. Choisissons un schéma $V'$
et un morphisme surjectif \'tale $V' \to V \times_Y Y'$.
Alors $U' = V' \times_V U$ est un schéma et le morphisme
$U' \to X'$ est surjectif et \'étale.

\medskip\noindent
Preuve de (1). Choisissons la application $u' \in U'$ vers $x'$.
Noter $v' \in V'$ l'image de $u'$.
Alors $x' \in \text{Ass}_{X'/Y'}(\mathcal{F}')$ est
équivalent à $u' \in \text{Ass}(\mathcal{F}|_{U'_{v'}})$
par définition (écrivonsure $\text{Ass}$ au lieu de $\text{WeakAss}$
ce qui est légitime puisque $U'_{v'}$ est localement noethérien).
Application des diviseurs, lemme \ref{divisors-lemma-base-change-relative-assassin}
nous voyons que l'image $u \in U$ de $u'$ est dans
$\text{Ass}(\mathcal{F}|_{U_v})$ où $v \in V$ est l'image de $u$.
Cela signifie à son tour $g'(x') \in \text{Ass}_{X/Y}(\mathcal{F})$.

\medskip\noindent
Preuve de (2). Choisissons la application $u \in U$ vers $x$.
Noter $v \in V$ l'image de $u$.
Alors $x \in \text{Ass}_{X/Y}(\mathcal{F})$ est
équivalent à $u \in \text{Ass}(\mathcal{F}|_{U_v})$
par définition. Choisissons un point $v' \in V'$ envoyant
à $y' \in |Y'|$ et à $v \in V$ (possible par
Propriétés des espaces, lemme \ref{spaces-properties-lemma-points-cartesian}).
Soit $t \in \Spec(\kappa(v') \otimes_{\kappa(v)} \kappa(u))$
un point générique d'une composante irréductible.
Que $u' \in U'$ soit l'image de $t$.
Application des diviseurs, lemme \ref{divisors-lemma-base-change-relative-assassin}
nous voyons ce $u' \in \text{Ass}(\mathcal{F}'|_{U'_{v'}})$.
Cela signifie à son tour $x' \in \text{Ass}_{X'/Y'}(\mathcal{F}')$
où $x' \in |X'|$ est l'image de $u'$.
\end{proof}

\begin{lemma}
\label{lemma-base-change-relative-assassin-quasi-finite}
Avec notation et hypothèses comme dans
lemme \ref{lemma-base-change-relative-assassin}.
Supposons que $g$ est localement quasi-fini, ou plus généralement que
pour chaque $y' \in |Y'|$ le degré de transcendance de $y'/g(y')$ est $0$.
Alors, $\text{Ass}_{X'/Y'}(\mathcal{F}')$ est l'image inverse de
...
Morphismes des espaces, Définition
\ref{spaces-morphisms-definition-dimension-fibre}.
Soit $x' \in |X'|$ avec l'image $x \in |X|$.
Choisissons un schéma $V$ et un surjectif \'étale morphisme $V \to Y$.
Choisissons un schéma $U$ et un surjectif
\'tale morphisme $U \to V \times_Y X$. Choisissons un schéma $V'$
et un morphisme surjectif \'tale $V' \to V \times_Y Y'$.
Alors $U' = V' \times_V U$ est un schéma et le morphisme
$U' \to X'$ est surjectif et \'étale.
Choisissons la application $u \in U$ vers $x$.
Noter $v \in V$ l'image de $u$.
Alors $x \in \text{Ass}_{X/Y}(\mathcal{F})$ est
équivalent à $u \in \text{Ass}(\mathcal{F}|_{U_v})$
par définition. Choisissons un point $u' \in U'$ envoyant
à $x' \in |X'|$ et à $u \in U$ (possible par
Propriétés des espaces, lemme \ref{spaces-properties-lemma-points-cartesian}).
Que $v' \in V'$ soit l'image de $u'$.
Alors $x' \in \text{Ass}_{X'/Y'}(\mathcal{F}')$ est
équivalent à $u' \in \text{Ass}(\mathcal{F}'|_{U'_{v'}})$
par définition.
Maintenant, le lemme fait suite à la discussion en
Diviseurs, remarquez \ref{divisors-remark-base-change-relative-assassin}
...
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme des espaces algébriques sur $S$.
Que $i : Z \to X$ un morphisme fini.
Que $\mathcal{G}$ un module quasi-cohérent $\mathcal{O}_Z$.
Alors $\text{WeakAss}_{X/Y}(i_*\mathcal{G}) =
i(\text{WeakAss}_{Z/Y}(\mathcal{G}))$.
\end{lemma}

\begin{proof}
S'inscrit dans le cas des schémas
(Diviseurs, lemme \ref{divisors-lemma-relative-weak-assassin-finite})
par localisation \'étale. Détails omis.
\end{proof}

\begin{lemma}
\label{lemma-relative-assassin-constructible}
Soit $Y$ un schéma. Soit $X$ un espace algébrique de présentation finie
au-dessus de $Y$. Soit $\mathcal{F}$ un module quasi-cohérent $\mathcal{O}_X$
de présentation finie. Que $U \subset X$ un sous-espace ouvert
de telle sorte que $U \to Y$ soit quasi-compact.
$$
E = \{y \in Y \mid \text{Ass}_{X_y}(\mathcal{F}_y) \subset |U_y|\}
$$
est constructible localement dans $Y$.
\end{lemma}

\begin{proof}
Notez que puisque $Y$ est un schéma, il est logique de prendre les fibres
$X_y = \Spec(\kappa(y)) \times_Y X$. (En outre, selon nos définitions,
le jeu $\text{Ass}_{X_y}(\mathcal{F}_y)$ est exactement la fibre de
$\text{Ass}_{X/Y}(\mathcal{F}) \to Y$ sur $y$, mais nous n'en aurons pas besoin.)
La question est locale sur $Y$, en effet, nous devons montrer que
$E$ est constructible si $Y$ est affine.
Dans ce cas, $X$ est quasi-compact. Choisissons un schéma affine $W$
et un morphisme surjectif \'tale $\varphi : W \to X$.
Alors, $\text{Ass}_{X_y}(\mathcal{F}_y)$ est l'image de
$\text{Ass}_{W_y}(\varphi^*\mathcal{F}_y)$ pour tous les $y \in Y$.
C'est pourquoi le lemme découle du cas des schémas de
le $\varphi^{-1}(U) \subset W$ ouvert et le morphisme $W \to Y$.
Le cas des schémas est le suivant:
En savoir plus sur les morphismes, lemme
...
\noindent
La présente section fait suite à la discussion qui s'est tenue en
Diviseurs, section \ref{divisors-section-fitting-ideals}.
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Soit $\mathcal{F}$ un de type fini et quasi-cohérent $\mathcal{O}_X$-module.
Dans cette situation, nous pouvons construire les idéaux d'aménagement
$$
0 = \text{Fit}_{-1}(\mathcal{F}) \subset \text{Fit}_0(\mathcal{F}) \subset
\text{Fit}_1(\mathcal{F}) \subset \ldots \subset \mathcal{O}_X
$$
comme la suite d'idéaux quasi-cohérents de faisceaux caractérisés par le
propriété suivante: pour chaque affine $U = \Spec(A)$ \'étale au-dessus de $X$
si $\mathcal{F}|_U$ correspond au module $A$ $M$, alors
$\text{Fit}_i(\mathcal{F})|_U$
correspond à l'idéal $\text{Fit}_i(M) \subset A$.
C'est bien défini et un faisceau quasi-cohérent d'idéaux parce que
si $A \to B$ est une homomorphisme d'anneaux \'étale, alors le $i$th Montage idéal
de $M \otimes_A B$ au-dessus de $B$ est égal à $\text{Fit}_i(M) B$ par
Compléments d'algèbre, partie lemme \ref{more-algebra-lemma-fitting-ideal-basics} (3).
Plus précisément (peut-être), l'existence des faisceaux quasi-cohérents des idéaux
$\text{Fit}_0(\mathcal{O}_X)$ suit (par exemple)
la description des faisceaux quasi-cohérents en
Propriétés des espaces, lemme
\ref{spaces-properties-lemma-characterize-quasi-coherent-small-etale}
et la propriété de retrait donnée dans
Diviseurs, lemme \ref{divisors-lemma-base-change-fitting-ideal}.

\medskip\noindent
L'avantage de construire les idéaux d'aménagement de cette manière
est que nous voyons immédiatement que la construction des idéaux de Fitting
commute avec \'étale localisation d'où de nombreuses propriétés de
les idéaux de Fitting réduisent immédiatement au correspondant
propriétés dans le cas de schémas. Souvent, nous utiliserons le
discussion dans Propriétés des espaces, section
\ref{spaces-properties-section-properties-modules}
pour faire la traduction entre les propriétés des faisceaux quasi-cohérents
sur les plans et sur les espaces algébriquess.

\begin{lemma}
\label{lemma-base-change-fitting-ideal}
Soit $S$ un schéma.
Que $f : X \to Y$ un morphisme des espaces algébriques sur $S$.
Soit $\mathcal{F}$ un de type fini et quasi-cohérent $\mathcal{O}_Y$-module.
Alors
...
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Soit $\mathcal{F}$ un de type fini et quasi-cohérent $\mathcal{O}_X$-module.
Que $Z_0 \subset X$ soit le sous-espace fermé coupé par
$\text{Fit}_0(\mathcal{F})$.
Que $Z \subset X$ soit le support schématique de $\mathcal{F}$.
Alors
\begin{enumerate}
\item $Z \subset Z_0 \subset X$ en tant que sous-espaces fermés,
\item $|Z| = |Z_0| = \text{Supp}(\mathcal{F})$ en tant que sous-ensembles fermés de $|X|$,
\item existe un de type fini et quasi-cohérent $\mathcal{O}_{Z_0}$-module
$\mathcal{G}_0$ avec
$$
...
\begin{proof}
Rappelons que la construction de $Z$ commute avec \'étale localisation, voir
Morphismes des espaces, Définition
\ref{spaces-morphisms-definition-scheme-theoretic-support}
(qui utilise les morphismes des espaces, lemme
\ref{spaces-morphisms-lemma-scheme-theoretic-support}
à définir $Z$C'est pourquoi les points 1) et 2) découlent du cas des schémas, cf.
...
que nous avons $\mathcal{G}$ sur $Z$ comme dans Morphismes d'espaces, lemme
\ref{spaces-morphisms-lemma-scheme-theoretic-support}
et définir $\mathcal{G}_0 = (Z \to Z_0)_*\mathcal{G}$.
\end{proof}

\begin{lemma}
\label{lemma-fitting-ideal-generate-locally}
Soit $S$ un schéma. Soit $X$ un espace algébrique sur $S$.
Soit $\mathcal{F}$ un de type fini et quasi-cohérent
